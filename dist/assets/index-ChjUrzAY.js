(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))a(s);new MutationObserver(s=>{for(const r of s)if(r.type==="childList")for(const o of r.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&a(o)}).observe(document,{childList:!0,subtree:!0});function n(s){const r={};return s.integrity&&(r.integrity=s.integrity),s.referrerPolicy&&(r.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?r.credentials="include":s.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function a(s){if(s.ep)return;s.ep=!0;const r=n(s);fetch(s.href,r)}})();let Gt=new Date,Ze=[],je=[],Ne=[],Qa=[],da=[],nt=[],_e=[],Pt=[],At=[],xs=!1,Ta=[],Ot=[],ie=[];window.att_multiSelectMode=!1;window.att_selectedDates=new Set;window.att_selectedCategories=new Set;window.att_selectedGroups=new Set;function bn(e){let t=e.target_group_ids||[];if(t.length===0&&e.target_group_id&&(t=[e.target_group_id]),t.length===0)return{ids:[],name:"全体",color:"#e5e7eb"};const n=je.filter(a=>t.includes(a.id));return n.length===0?{ids:[],name:"全体",color:"#e5e7eb"}:{ids:t,name:n.map(a=>a.name).join(", "),color:n[0].color||"#e5e7eb",groups:n}}function Ws(e){return e.ids.length===0?'<span class="text-xs border border-gray-300 text-gray-800 px-2 py-0.5 rounded shadow-sm" style="background-color: #e5e7eb">全体</span>':e.groups.map(t=>`<span class="text-xs border border-gray-300 text-gray-800 px-2 py-0.5 rounded shadow-sm" style="background-color: ${t.color||"#e5e7eb"}">${t.name}</span>`).join(" ")}function zs(e){if(!e)return"-";try{const t=new Date(e);if(isNaN(t.getTime()))return"-";const n=t.getMonth()+1,a=t.getDate(),s=String(t.getHours()).padStart(2,"0"),r=String(t.getMinutes()).padStart(2,"0");return`${n}/${a} ${s}:${r}`}catch{return"-"}}async function Aa(){try{xs||(Pr(),xs=!0),await en(),He(),Or(),Mr()}catch(e){console.error("Attendance App Init Error:",e),typeof qe=="function"&&qe()}}function Pr(){var n,a,s,r,o,i,d,l,c,m,u,f,b,y,p,g,v,x,h,k,L,w,I,E,$;const e=document.getElementById("btn-logout-att");if(e&&!document.getElementById("btn-change-password-att")){const _=document.createElement("button");_.id="btn-change-password-att",_.className="text-xs md:text-sm bg-gray-500 hover:bg-gray-600 text-white px-2 py-1 md:px-3 md:py-1 rounded shadow mr-2 font-bold",_.textContent="パスワード変更",_.onclick=()=>ms(),e.parentNode.insertBefore(_,e)}(n=document.getElementById("tab-calendar"))==null||n.addEventListener("click",()=>vs("calendar")),(a=document.getElementById("tab-list"))==null||a.addEventListener("click",()=>{vs("list"),kt()}),(s=document.getElementById("cal-prev-month"))==null||s.addEventListener("click",()=>{Gt.setMonth(Gt.getMonth()-1),He()}),(r=document.getElementById("cal-next-month"))==null||r.addEventListener("click",()=>{Gt.setMonth(Gt.getMonth()+1),He()}),(o=document.getElementById("cal-today"))==null||o.addEventListener("click",()=>{Gt=new Date,He()}),(i=document.getElementById("btn-add-event"))==null||i.addEventListener("click",()=>Bn()),(d=document.getElementById("btn-export-events"))==null||d.addEventListener("click",to),(l=document.getElementById("btn-import-events"))==null||l.addEventListener("click",()=>document.getElementById("input-import-events-csv").click()),(c=document.getElementById("btn-import-ics-events"))==null||c.addEventListener("click",()=>document.getElementById("input-import-events-ics").click()),(m=document.getElementById("btn-download-events-sample"))==null||m.addEventListener("click",so),(u=document.getElementById("input-import-events-csv"))==null||u.addEventListener("change",no),(f=document.getElementById("input-import-events-ics"))==null||f.addEventListener("change",oo),(b=document.getElementById("btn-close-ics-modal-x"))==null||b.addEventListener("click",Ca),(y=document.getElementById("btn-close-ics-modal"))==null||y.addEventListener("click",Ca),(p=document.getElementById("btn-execute-ics-import"))==null||p.addEventListener("click",lo),(g=document.getElementById("ics-import-list"))==null||g.addEventListener("change",_=>{if(_.target&&_.target.classList.contains("ics-row-location-select")){const C=_.target,P=C.closest(".ics-row").querySelector(".ics-row-location");C.value&&C.value!=="custom"?P.value=C.value:C.value==="custom"&&(P.value="",P.focus())}});const t=document.getElementById("btn-group-manage");t&&(t.style.display="none"),(v=document.getElementById("btn-toggle-multiselect"))==null||v.addEventListener("click",function(){window.att_multiSelectMode=!window.att_multiSelectMode,window.att_multiSelectMode||window.att_selectedDates.clear(),this.classList.toggle("bg-blue-600",window.att_multiSelectMode),this.classList.toggle("text-white",window.att_multiSelectMode),this.classList.toggle("bg-blue-50",!window.att_multiSelectMode),this.classList.toggle("text-blue-600",!window.att_multiSelectMode),He(),window.att_updateMultiselectBar()}),(x=document.getElementById("btn-cal-filter"))==null||x.addEventListener("click",ws),(h=document.getElementById("btn-cal-filter-clear"))==null||h.addEventListener("click",xa),(k=document.getElementById("btn-list-filter"))==null||k.addEventListener("click",ws),(L=document.getElementById("btn-list-filter-clear"))==null||L.addEventListener("click",xa),(w=document.getElementById("btn-close-filter-modal-x"))==null||w.addEventListener("click",Jn),(I=document.getElementById("btn-close-filter-modal"))==null||I.addEventListener("click",Jn),(E=document.getElementById("btn-apply-filter"))==null||E.addEventListener("click",Rr),($=document.getElementById("btn-filter-clear"))==null||$.addEventListener("click",xa)}function vs(e){const t=document.getElementById("tab-calendar"),n=document.getElementById("tab-list");e==="calendar"?(document.getElementById("calendar-container").classList.remove("hidden"),document.getElementById("list-container").classList.add("hidden"),t.classList.replace("bg-white","bg-green-600"),t.classList.replace("text-green-600","text-white"),n.classList.replace("bg-green-600","bg-white"),n.classList.replace("text-white","text-green-600")):(document.getElementById("calendar-container").classList.add("hidden"),document.getElementById("list-container").classList.remove("hidden"),n.classList.replace("bg-white","bg-green-600"),n.classList.replace("text-green-600","text-white"),t.classList.replace("bg-green-600","bg-white"),t.classList.replace("text-white","text-green-600"))}async function en(){U("イベント・出欠データ読み込み中...");try{const{data:e}=await S.from("groups").select("*").order("sort_order",{ascending:!0}).order("created_at",{ascending:!0});e&&(je=e);const{data:t}=await S.from("event_categories").select("*").order("sort_order",{ascending:!0}).order("created_at",{ascending:!0});t&&t.length>0?Ne=t:Ne=[{id:"1",name:"練習"},{id:"2",name:"試合"},{id:"3",name:"イベント"}];const n=new Date,a=new Date(n.getFullYear(),n.getMonth()-6,1),s=new Date(n.getFullYear(),n.getMonth()+7,1),{data:r}=await S.from("events").select("*").gte("start_time",a.toISOString()).lt("start_time",s.toISOString()).order("start_time");r&&(Ze=r);const{data:o}=await S.from("app_users").select("email, name, attribute_id, can_use_attendance");o&&(Pt=o);const{data:i}=await S.from("user_attributes").select("*").order("sort_order",{ascending:!0}).order("created_at",{ascending:!0});i&&(At=i);try{const{data:c}=await S.from("event_locations").select("*").order("sort_order",{ascending:!0}).order("created_at",{ascending:!0});c&&(Ot=c)}catch(c){console.warn("event_locations table might not exist yet:",c),Ot=[]}const{data:d}=await S.from("user_groups").select("*");d&&(nt=d,V&&(Qa=d.filter(c=>c.user_email===V.email)));let l=[];if(Ze&&Ze.length>0){const c=Ze.map(u=>u.id),{data:m}=await S.from("attendances").select("*").in("event_id",c);m&&(l=m)}_e=l,V&&(da=l.filter(c=>c.user_email===V.email));try{const{data:c}=await S.from("master_data").select("data").eq("key","ATTENDANCE_DELEGATIONS").single();c&&c.data&&V&&(Ta=c.data[V.email]||[])}catch{Ta=[]}}catch(e){console.error("Attendance DB Error:",e)}finally{typeof qe=="function"?qe():F()}}function Or(){const e=document.getElementById("filter-group");e&&(e.innerHTML='<option value="">すべてのグループ</option>'+je.map(t=>`<option value="${t.id}">${t.name}</option>`).join(""))}function Mr(){const e=document.getElementById("filter-category");if(!e)return;const t=e.value;e.innerHTML='<option value="">すべてのカテゴリ</option>'+Ne.map(n=>`<option value="${n.name}">${n.name}</option>`).join(""),Ne.some(n=>n.name===t)&&(e.value=t)}function ws(){const e=document.getElementById("filter-modal-content");if(!e)return;let t="";t+='<div class="mb-5">',t+='<h4 class="text-xs font-bold text-gray-400 tracking-wider uppercase mb-2">カテゴリ（複数選択）</h4>',Ne.length===0?t+='<p class="text-xs text-gray-400">カテゴリが登録されていません</p>':(t+='<div class="grid grid-cols-2 gap-2">',Ne.forEach(a=>{const s=window.att_selectedCategories.has(a.name)?"checked":"";t+=`
    ...
    `,je.length>0&&je.forEach(a=>{const s=window.att_selectedGroups.has(a.id)?"checked":"";t+=`
        ...
        `,d.forEach(m=>{const u=bn(m);u.name,u.color;const f=Ne.find(L=>L.name===m.category),b=(f==null?void 0:f.color)||"#bfdbfe",y=da.find(L=>L.event_id===m.id);let p=y&&y.status?y.status:"未回答";p==="未定"&&(p="保留");const g=qr(m.start_time,m.end_time,m.is_all_day);let v="不明";if(m.created_by){const L=Pt.find(w=>w.email===m.created_by);v=L&&L.name||m.created_by.split("@")[0]}const x=jr(m.created_at),h=m.created_by?`
            ...
            `}),o+="</div>"}),t.innerHTML=o}function Bn(e="",t=null,n=!1,a=null){let s="";t&&(s=t.title.replace(/"/g,"&quot;"),n||(s+=" (コピー)"));const r=!t&&!n;a&&a.length>0?ie=[...a]:t?ie=[t.start_time.split("T")[0]]:e?ie=[e]:ie=[new Date().toISOString().split("T")[0]],window.att_modalSelectedDates=ie,window.att_isEditingModal=n;const o=t&&t.start_time&&!t.is_all_day?t.start_time.split("T")[1].substring(0,5):r?"12:30":"",i=t&&t.end_time&&!t.is_all_day?t.end_time.split("T")[1].substring(0,5):r?"17:00":"",d=t?t.is_all_day:!1,l=t?t.category:"",c=t?(t.location||"").replace(/"/g,"&quot;"):"",m=t?(t.description||"").replace(/</g,"&lt;").replace(/>/g,"&gt;"):"",u=t?t.requires_attendance:!0,f=t?t.require_detailed_attendance:!1,b=o?o.split(":")[0]:"",y=o?o.split(":")[1]:"",p=i?i.split(":")[0]:"",g=i?i.split(":")[1]:"",v=!c||!Ot.some(T=>(T.url?`${T.name} ${T.url}`:T.name)===c);let x="",h="";if(c){const T=c.match(/(https?:\/\/[^\s\<\>\"]+)/);T?(h=T[0],x=c.replace(h,"").trim()):x=c}const k=!!(t&&t.attendance_deadline&&t.attendance_deadline!==vt(t.start_time.split("T")[0])),L=vt(ie[0])?vt(ie[0]).split("T")[0]:"",w=t&&t.attendance_deadline?t.attendance_deadline.split("T")[0]:L,I=t&&t.attendance_deadline?t.attendance_deadline.split("T")[1].substring(0,5):"12:00",E=I?I.split(":")[0]:"12",$=I?I.split(":")[1]:"00",_=T=>'<option value="">--</option>'+Array.from({length:24},(M,W)=>String(W).padStart(2,"0")).map(M=>`<option value="${M}" ${M===T?"selected":""}>${M}</option>`).join(""),C=T=>'<option value="">--</option>'+Array.from({length:60},(M,W)=>String(W).padStart(2,"0")).map(M=>`<option value="${M}" ${M===T?"selected":""}>${M}</option>`).join(""),P=t?t.target_group_ids&&t.target_group_ids.length>0?t.target_group_ids:t.target_group_id?[t.target_group_id]:[]:[],D=n?"イベントを編集":t?"イベントを複製":"新規イベント登録",O=`
    ...
    `;let q="";n?q=`
            <div>
                ...
                <div id="ev-dates-container" class="mt-1">
                    <input type="date" value="${ie[0]}" class="w-full border p-1.5 rounded text-xs px-1" onchange="window.att_onSingleDateChange(this.value)">
                </div>
            </div>
        `:q=`
            <div>
    ...
    </div>`;document.getElementById("attendance-modals").innerHTML=B,Mt(),n||Tn();const A=document.getElementById("ev-group-all"),N=document.querySelectorAll('input[name="ev-group-cb"]');A.addEventListener("change",function(){this.checked&&N.forEach(T=>T.checked=!1)}),N.forEach(T=>{T.addEventListener("change",function(){this.checked&&(A.checked=!1)})})}async function Hr(e=null){var C,P,D,O,q,B;const t=document.getElementById("ev-title").value.trim(),n=document.querySelectorAll('#ev-dates-container input[type="date"]'),a=Array.from(n).map(A=>A.value).filter(Boolean);if(!t)return alert("イベント名は必須です");if(a.length===0)return alert("日付を1つ以上指定してください");const s=document.getElementById("ev-time-h").value,r=document.getElementById("ev-time-m").value;let o="";(s||r)&&(o=`${s||"00"}:${r||"00"}`);const i=document.getElementById("ev-end-time-h").value,d=document.getElementById("ev-end-time-m").value;let l="";(i||d)&&(l=`${i||"00"}:${d||"00"}`);const c=document.getElementById("ev-all-day").checked,m=document.getElementById("ev-category").value,u=document.getElementById("ev-description").value,f=document.getElementById("ev-location-select");let b="";if(f)if(f.value==="custom"){const A=((C=document.getElementById("ev-location-custom-name"))==null?void 0:C.value.trim())||"",N=((P=document.getElementById("ev-location-custom-url"))==null?void 0:P.value.trim())||"";b=N?`${A} ${N}`:A}else b=f.value;const y=document.getElementById("ev-attendance-type").value,p=y!=="none",g=y==="detailed",v=((D=document.getElementById("ev-deadline-custom-cb"))==null?void 0:D.checked)||!1,x=(O=document.getElementById("ev-deadline-date"))==null?void 0:O.value,h=(q=document.getElementById("ev-deadline-time-h"))==null?void 0:q.value,k=(B=document.getElementById("ev-deadline-time-m"))==null?void 0:B.value,L=document.getElementById("ev-group-all").checked;let w=[];L||document.querySelectorAll('input[name="ev-group-cb"]:checked').forEach(A=>w.push(A.value));const I=w.length>0?w[0]:null,E=A=>new Promise(N=>setTimeout(N,A)),$=3;let _=0;for(;_<$;){_++,_>1?(U(`通信リトライ中 (${_-1}/${$-1}回目)...`),await E(1500)):U("イベント保存中...");try{if(e){const A=a[0],N=`${A}T${o||"00:00"}:00`;let T=null;l&&(T=`${A}T${l}:00`);let M=null;p&&(v?x&&h&&k&&(M=`${x}T${h}:${k}:00`):M=vt(A));const W={title:t,category:m,description:u,location:b,start_time:N,end_time:T,is_all_day:c,requires_attendance:p,require_detailed_attendance:g,attendance_deadline:M,target_group_id:I,target_group_ids:w.length>0?w:null},{error:j}=await S.from("events").update(W).eq("id",e);if(j)throw j;await ee("UPDATE_EVENT",`イベント「${t}」を更新しました`)}else{const A=a.map(T=>{const M=`${T}T${o||"00:00"}:00`;let W=null;l&&(W=`${T}T${l}:00`);let j=null;return p&&(v?x&&h&&k&&(j=`${x}T${h}:${k}:00`):j=vt(T)),{title:t,category:m,description:u,location:b,start_time:M,end_time:W,is_all_day:c,requires_attendance:p,require_detailed_attendance:g,attendance_deadline:j,target_group_id:I,target_group_ids:w.length>0?w:null,created_by:V==null?void 0:V.email}}),{error:N}=await S.from("events").insert(A);if(N)throw N;await ee("CREATE_EVENT",`イベント「${t}」を${A.length}件作成しました`)}await en(),He(),document.getElementById("list-container").classList.contains("hidden")||kt(),!e&&typeof window.att_clearDateSelection=="function"&&window.att_clearDateSelection(),window.att_closeModal();break}catch(A){console.error(`Save Event Attempt ${_} Error:`,A);const N=A.message==="Load failed"||A.message==="Failed to fetch"||!A.code;if(_<$&&N)continue;{let T=A.message||String(A);A.details&&(T+=`
Details: `+A.details),A.hint&&(T+=`
Hint: `+A.hint),A.code&&(T+=`
Code: `+A.code),alert("保存エラー: "+T+`
（ネットワーク接続をご確認のうえ、再度お試しください）`);break}}}F()}async function Ur(e){if(confirm("このイベントを削除しますか？")){U("イベント削除中...");try{await S.from("events").delete().eq("id",e),await ee("DELETE_EVENT",`イベント(ID:${e})を削除しました`),await en(),He(),kt(),window.att_closeModal()}catch(t){console.error(t)}finally{F()}}}function Ks(e,t,n=!1){const a=t.ids.length===0||Qa.some(l=>t.ids.includes(l.group_id)),s=Pt.find(l=>l.email===V.email),r=s?s.can_use_attendance!==!1:!0,o=[];r&&o.push({email:V.email,name:"自分 ( "+((V==null?void 0:V.name)||V.email.split("@")[0])+" )",canAttend:a}),Ta.forEach(l=>{const c=Pt.find(m=>m.email===l);if(c&&c.can_use_attendance!==!1){const m=t.ids.length===0||nt.some(u=>t.ids.includes(u.group_id)&&u.user_email===l);o.push({email:l,name:c.name||l.split("@")[0],canAttend:m})}});let i=!1;return{formsHtml:o.map((l,c)=>{if(!l.canAttend)return`<div class="p-3 bg-gray-50 border rounded mb-2">
    ...
    `,v=u?"※出欠の入力があるグループのみ表示しています。":"※出欠の入力がある属性のみ表示しています。";let x=`
    ...
    `,k=m.filter(B=>{const A=_e.find(T=>T.event_id===n.id&&T.user_email===B.email),N=(A==null?void 0:A.status)||"未回答";return window.att_statusFilter==="all"?!0:window.att_statusFilter==="answered"?N==="出席"||N==="欠席"||N==="保留"||N==="未定":window.att_statusFilter==="attending"?N==="出席":window.att_statusFilter==="absent"?N==="欠席":window.att_statusFilter==="pending"?N==="保留"||N==="未定":window.att_statusFilter==="unanswered"?N==="未回答"||!N:!0});window.att_statusSortKey||(window.att_statusSortKey="default"),window.att_statusSortOrder||(window.att_statusSortOrder="asc");const L=window.att_statusSortKey,w=window.att_statusSortOrder;L!=="default"&&k.sort((B,A)=>{var M,W;let N,T;if(L==="name")N=B.name||B.email.split("@")[0],T=A.name||A.email.split("@")[0];else if(L==="group")if(u){const j=nt.filter(R=>R.user_email===B.email).map(R=>R.group_id);N=je.filter(R=>j.includes(R.id)).map(R=>R.name).join(", ")||"-";const H=nt.filter(R=>R.user_email===A.email).map(R=>R.group_id);T=je.filter(R=>H.includes(R.id)).map(R=>R.name).join(", ")||"-"}else N=((M=At.find(j=>j.id===B.attribute_id))==null?void 0:M.name)||"-",T=((W=At.find(j=>j.id===A.attribute_id))==null?void 0:W.name)||"-";else if(L==="status"){const j=H=>{const R=_e.find(le=>le.event_id===n.id&&le.user_email===H.email),z=(R==null?void 0:R.status)||"未回答";return z==="出席"?1:z==="保留"||z==="未定"?2:z==="未回答"?3:z==="欠席"?4:5};N=j(B),T=j(A)}else if(L==="comment"){const j=H=>{const R=_e.find(be=>be.event_id===n.id&&be.user_email===H.email);if(!R||!R.status||R.status==="未回答")return"";let z=R.comment||"",le="否";z.startsWith("[荷物車:可]")?(le="可",z=z.substring(8)):z.startsWith("[荷物車:否]")&&(le="否",z=z.substring(8));let te=[];return n.require_detailed_attendance&&(R.accompanying_persons&&te.push(`同伴: ${R.accompanying_persons}`),R.car_capacity&&R.car_capacity>0&&(te.push(`車出: 可[${R.car_capacity}人]`),te.push(`荷物車: ${le}`))),z.trim()&&te.push(`メモ: ${z.trim()}`),te.join(", ")};N=j(B),T=j(A)}else if(L==="updated_at"){const j=_e.find(R=>R.event_id===n.id&&R.user_email===B.email),H=_e.find(R=>R.event_id===n.id&&R.user_email===A.email);N=j?new Date(j.updated_at).getTime():0,T=H?new Date(H.updated_at).getTime():0}return N<T?w==="asc"?-1:1:N>T?w==="asc"?1:-1:0});const E=[{type:"all",label:"すべて"},{type:"answered",label:"回答済"},{type:"attending",label:"出席"},{type:"absent",label:"欠席"},{type:"pending",label:"保留"},{type:"unanswered",label:"未回答"}].map(B=>{const A=window.att_statusFilter===B.type;return`<button onclick="window.att_setStatusFilter('${n.id}', '${B.type}')" class="px-2.5 py-1 rounded-full text-xs font-bold transition ${A?"bg-blue-600 text-white shadow-sm":"bg-gray-100 text-gray-600 hover:bg-gray-200"}">${B.label}</button>`}).join(" ");let $="";k.length===0?$=`
            <tr>
                <td colspan="5" class="px-4 py-6 text-center text-gray-400 text-xs">該当するメンバーはいません。</td>
            </tr>
        `:$=k.map(B=>{var te;const A=B.name||B.email.split("@")[0];let N="-";if(u){const be=nt.filter(Ee=>Ee.user_email===B.email).map(Ee=>Ee.group_id);N=je.filter(Ee=>be.includes(Ee.id)).map(Ee=>Ee.name).join(", ")||"-"}else N=((te=At.find(be=>be.id===B.attribute_id))==null?void 0:te.name)||"-";const T=_e.find(be=>be.event_id===n.id&&be.user_email===B.email);let M=(T==null?void 0:T.status)||"未回答";M==="未定"&&(M="保留");let W="";M==="出席"?W='<span class="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-bold bg-green-100 text-green-800 border border-green-200">🟢 参加</span>':M==="欠席"?W='<span class="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-bold bg-red-100 text-red-800 border border-red-200">❌ 不参加</span>':M==="保留"?W='<span class="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-bold bg-yellow-100 text-yellow-800 border border-yellow-200">❓ 未定/その他</span>':W='<span class="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-bold bg-gray-100 text-gray-500 border border-gray-200">❓ 未定/その他</span>';let j=(T==null?void 0:T.comment)||"",H="否";j.startsWith("[荷物車:可]")?(H="可",j=j.substring(8)):j.startsWith("[荷物車:否]")&&(H="否",j=j.substring(8));let R=[];T&&T.status&&T.status!=="未回答"&&(n.require_detailed_attendance&&(T.accompanying_persons&&R.push(`同伴: ${T.accompanying_persons}`),T.car_capacity&&T.car_capacity>0&&(R.push(`車出: 可[${T.car_capacity}人]`),R.push(`荷物車: ${H}`))),j.trim()&&R.push(`メモ: ${j.trim()}`));const z=R.length>0?R.join(", "):"-",le=T?zs(T.updated_at):"-";return`
                <tr class="hover:bg-gray-50/50 border-b border-gray-100 transition">
                    ...
                    <td class="px-4 py-2 text-gray-400 text-[11px] font-semibold">${le}</td>
                </tr>
            `}).join("");const _=B=>L!==B?'<span class="text-gray-300 ml-1">⇅</span>':w==="asc"?'<span class="text-blue-600 ml-1">▲</span>':'<span class="text-blue-600 ml-1">▼</span>',C=`
    ...
    </div>`;document.getElementById("attendance-modals").innerHTML=q};function Vr(e){const t=Ze.find(i=>i.id===e);if(!t)return;const n=bn(t),a=t.attendance_deadline?new Date>new Date(t.attendance_deadline):!1,{formsHtml:s,hasAnyForm:r}=Ks(t,n,a),o=`
...
`&&r++):a+=o}return(a||n.length>0)&&(n.push(a),t.push(n)),t}window.att_importCsv=async function(e){const t=e.target.files[0];if(!t)return;const n=new FileReader;n.onload=async a=>{const s=a.target.result,r=zr(s);if(r.length<2)return alert("インポートするデータがありません。");const o=[];for(let c=1;c<r.length;c++){const m=r[c];if(m.length<2||!m[0]||!m[1])continue;const u=m[0],f=m[1],b=m[2],y=m[3],p=(m[4]||"").toUpperCase()==="TRUE",g=m[5],v=m[6]||"",x=m[7],h=(m[8]||"").toUpperCase()==="TRUE"||(m[8]||"").trim()==="",k=(m[9]||"").toUpperCase()==="TRUE",L=m[10],w=m[11]||null,I=`${f}T${b||"00:00"}:00`;let E=null;y&&(E=`${f}T${y}:00`);const $=v.split(",").map(D=>D.trim()),_=je.filter(D=>$.includes(D.name)),C=_.length>0?_.map(D=>D.id):null,P=C?C[0]:null;o.push({title:u,category:g||null,description:L||null,location:x||null,start_time:I,end_time:E,is_all_day:p,requires_attendance:h,require_detailed_attendance:k,attendance_deadline:w,target_group_id:P,target_group_ids:C,created_by:V==null?void 0:V.email})}if(o.length===0)return alert(`インポート可能なイベントがありませんでした。
...
`).replace(/\\r/g,"\r").replace(/\\t/g,"	").replace(/\\,/g,",").replace(/\\;/g,";").replace(/\\\\/g,"\\")}function _s(e,t){if(!e)return null;const n=parseInt(e.substring(0,4)),a=parseInt(e.substring(4,6))-1,s=parseInt(e.substring(6,8));if(t||e.indexOf("T")===-1)return new Date(n,a,s);const r=e.indexOf("T"),o=parseInt(e.substring(r+1,r+3)),i=parseInt(e.substring(r+3,r+5)),d=parseInt(e.substring(r+5,r+7))||0;return e.endsWith("Z")?new Date(Date.UTC(n,a,s,o,i,d)):new Date(n,a,s,o,i,d)}async function oo(e){const t=e.target.files[0];if(!t)return;U("ICSファイルを解析中...");const n=new FileReader;n.onload=async a=>{try{const s=a.target.result,r=ro(s);if(F(),r.length===0){alert("ICSファイルから有効な予定が見つかりませんでした。");return}io(r)}catch(s){console.error(s),alert("ICSファイルの解析に失敗しました: "+s.message),F()}},n.readAsText(t),e.target.value=""}function io(e){const t=document.getElementById("ics-import-list");if(!t)return;const n=document.getElementById("ics-import-count");n&&(n.textContent=e.length),t.innerHTML=e.map((r,o)=>{var y,p;const i=r.title||"",d=i.toLowerCase();let l=((y=Ne[0])==null?void 0:y.name)||"練習";for(let g of Ne)if(d.includes(g.name)){l=g.name;break}l===((p=Ne[0])==null?void 0:p.name)&&(d.includes("試合")||d.includes("vs")||d.includes("戦")?Ne.find(v=>v.name==="試合")&&(l="試合"):(d.includes("イベント")||d.includes("会")||d.includes("式")||d.includes("フェス"))&&Ne.find(v=>v.name==="イベント")&&(l="イベント"));let c="";if(r.location){const g=Ot.find(v=>v.name===r.location);g?c=g.name:c="custom"}const m=va(r.start),u=va(r.end||(r.start?new Date(r.start.getTime()+2*60*60*1e3):null));let f=r.start?new Date(r.start.getTime()-3*24*60*60*1e3):null;f&&f.setHours(12,0,0,0);const b=va(f);return`
        ...
        `}).join("");const a=document.getElementById("ics-select-all");a&&(a.checked=!0,a.onclick=r=>{document.querySelectorAll(".ics-row-select").forEach(o=>o.checked=r.target.checked)});const s=document.getElementById("ics-import-modal");s&&s.classList.remove("hidden")}function Ca(){var e;(e=document.getElementById("ics-import-modal"))==null||e.classList.add("hidden")}async function lo(){const e=document.querySelectorAll(".ics-row"),t=[];if(e.forEach(n=>{var p,g,v,x,h,k,L,w,I,E;const a=n.querySelector(".ics-row-select");if(!a||!a.checked)return;const s=((p=n.querySelector(".ics-row-title"))==null?void 0:p.value)||"";if(!s)return;const r=((g=n.querySelector(".ics-row-category"))==null?void 0:g.value)||"",o=((v=n.querySelector(".ics-row-location"))==null?void 0:v.value)||"",i=((x=n.querySelector(".ics-row-start"))==null?void 0:x.value)||"",d=((h=n.querySelector(".ics-row-end"))==null?void 0:h.value)||"",l=((k=n.querySelector(".ics-row-allday"))==null?void 0:k.checked)||!1,c=[];n.querySelectorAll(".ics-row-group-cb:checked").forEach($=>{c.push($.value)});const m=((L=n.querySelector(".ics-row-req-att"))==null?void 0:L.checked)||!1,u=((w=n.querySelector(".ics-row-req-det"))==null?void 0:w.checked)||!1,f=((I=n.querySelector(".ics-row-deadline"))==null?void 0:I.value)||"",b=((E=n.querySelector(".ics-row-description"))==null?void 0:E.value)||"",y=$=>$?new Date($).toISOString():null;t.push({title:s,category:r,location:o,start_time:y(i),end_time:y(d),is_all_day:l,target_group_ids:c.length>0?c:null,requires_attendance:m,require_detailed_attendance:m?u:!1,attendance_deadline:m?y(f):null,description:b,created_by:V==null?void 0:V.email})}),t.length===0){alert("登録対象の予定が選択されていません。");return}U("予定データを登録中...");try{const{error:n}=await S.from("events").insert(t);if(n)throw n;await ee("IMPORT_ICS_EVENTS",`${t.length}件の予定をICSファイルから一括登録しました`),alert(`${t.length}件の予定を登録しました。`),Ca(),await en(),He(),document.getElementById("list-container").classList.contains("hidden")||kt()}catch(n){console.error(n),alert("予定の登録に失敗しました: "+n.message)}finally{F()}}const co="modulepreload",uo=function(e){return"/"+e},Is={},mo=function(t,n,a){let s=Promise.resolve();if(n&&n.length>0){document.getElementsByTagName("link");const o=document.querySelector("meta[property=csp-nonce]"),i=(o==null?void 0:o.nonce)||(o==null?void 0:o.getAttribute("nonce"));s=Promise.allSettled(n.map(d=>{if(d=uo(d),d in Is)return;Is[d]=!0;const l=d.endsWith(".css"),c=l?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${d}"]${c}`))return;const m=document.createElement("link");if(m.rel=l?"stylesheet":co,l||(m.as="script"),m.crossOrigin="",m.href=d,i&&m.setAttribute("nonce",i),document.head.appendChild(m),l)return new Promise((u,f)=>{m.addEventListener("load",u),m.addEventListener("error",()=>f(new Error(`Unable to preload CSS for ${d}`)))})}))}function r(o){const i=new Event("vite:preloadError",{cancelable:!0});if(i.payload=o,window.dispatchEvent(i),!i.defaultPrevented)throw o}return s.then(o=>{for(const i of o||[])i.status==="rejected"&&r(i.reason);return t().catch(r)})};let $s=!1,Xs=[],er=[],tr=[],rt=[],ve={homeTeamNames:["ありんこアントス@A軍"],defaultFilterDate:{from:"",to:"",teamRegex:"",category:"",outcome:"all"}},ks=!1,vn={},Ct={},Ea={},_a={},Da={batter:[],pitcher:[]},Vt={batter:{key:"ops",order:"desc"},pitcher:{key:"era",order:"asc"}},nn={batter:{key:"ops",order:"desc"},pitcher:{key:"era",order:"asc"}},bt=null,pt={};async function Na(){$s||(go(),$s=!0);const e=document.querySelector('button[data-tab="import-data"]');e&&(he==="admin"?e.classList.remove("hidden"):e.classList.add("hidden")),await nr()}function po(e){const t=[];let n=[],a="",s=!1;for(let r=0;r<e.length;r++){const o=e[r];s?o==='"'?r+1<e.length&&e[r+1]==='"'?(a+='"',r++):s=!1:a+=o:o==='"'?s=!0:o===","?(n.push(a),a=""):o===`
`||o==="\r"?(n.push(a),t.push(n),n=[],a="",o==="\r"&&r+1<e.length&&e[r+1]===`
...
${$.myTeam} vs ${$.oppTeam}`:""},label:function(I){const E=I.dataIndex,$=k[E],_=I.dataset.label||"";return _==="得点"?`得点 (${($==null?void 0:$.myTeam)||"自チーム"}): ${I.raw} 点`:_==="失点"?`失点 (${($==null?void 0:$.oppTeam)||"相手チーム"}): ${Math.abs(I.raw)} 点`:_==="累積勝率(%)"?`累積勝率: ${I.raw} %`:`${_}: ${I.raw}`}}}},scales:{x:{stacked:!0,ticks:{font:{size:10}}},y:{stacked:!0,position:"left"},y1:{position:"right",beginAtZero:!0,min:0,max:100}}}})}function It(e){let t=0,n=0,a=0,s=0,r=0,o=0,i=0,d=0,l=0,c=0,m=0,u=0,f=0,b=0,y=0;e.forEach(w=>{const I=w.plate_appearances||0,E=w.at_bats||0,$=w.hits||0,_=w.doubles||0,C=w.triples||0,P=w.home_runs||0,D=w.walks||0,O=w.hit_by_pitch||0,q=w.strike_outs||w.strikeouts||0,B=w.sacrifice_flies||w.sac_flies||0,A=w.sacrifice_hits||w.sac_bunts||0,N=w.runs_batted_in||0,T=w.runs||0,M=w.stolen_bases||0;t+=I,n+=E,a+=$,u+=_,f+=C,m+=P,d+=D,l+=O,c+=q,b+=B,y+=A,r+=N,o+=T,i+=M;const j=(w.singles!==void 0?w.singles:Math.max(0,$-_-C-P))*1+_*2+C*3+P*4,H=w.total_bases&&w.total_bases>j?w.total_bases:j;s+=H});const p=n>0?a/n:0,g=n+d+l+b,v=g>0?(a+d+l)/g:t>0?(a+d+l)/t:0,x=n>0?s/n:0,h=v+x,k=a+d+l,L=k>0?o/k:0;return{pa:t,ab:n,h:a,doubles:u,triples:f,hr:m,tb:s,sf:b,sh:y,bb:d,hbp:l,rbi:r,r:o,sb:i,so:c,ob:k,avg:p,obp:v,slg:x,ops:h,runRate:L,avgStr:p.toFixed(3).replace(/^0/,""),obpStr:v.toFixed(3).replace(/^0/,""),slgStr:x.toFixed(3).replace(/^0/,""),opsStr:h.toFixed(3),runRateStr:k>0?(L*100).toFixed(1)+"%":"0.0%",bbRate:t>0?(d+l)/t:0,soRate:t>0?c/t:0}}function $t(e){let t=0,n=0,a=0,s=0,r=0,o=0,i=0,d=0,l=0,c=0;e.forEach(p=>{t+=p.outs||0,n+=p.earned_runs||0,a+=p.hits_allowed||0,s+=(p.walks_allowed||0)+(p.hit_batters||0),r+=p.strike_outs||0,o+=p.batters_faced||0,i+=p.pitch_count||0,d+=p.strikes||0,l+=p.wins||0,c+=p.losses||0});const m=t/3,u=m>0?n*7/m:0,f=m>0?(a+s)/m:0,b=m>0?r*7/m:0,y=m>0?s*7/m:0;return{outs:t,er:n,h:a,bb:s,so:r,bf:o,pc:i,st:d,wins:l,losses:c,era:u,whip:f,k7:b,bb7:y,eraStr:u.toFixed(2),whipStr:f.toFixed(2),k7Str:b.toFixed(2),bb7Str:y.toFixed(2),kRate:o>0?r/o:0,bbRate:o>0?s/o:0,kbb:s>0?r/s:r>0?99.9:0,sRate:i>0?d/i:0}}async function es(){var d,l,c,m,u,f;await An();const e=((d=document.getElementById("ps-mode"))==null?void 0:d.value)||"single",t=(l=document.getElementById("ps-player"))==null?void 0:l.value,n=((c=document.getElementById("ps-role"))==null?void 0:c.value)||"batter",a=((m=document.getElementById("ps-limit-games"))==null?void 0:m.value)||"5",s=((u=document.getElementById("ps-ma-unit"))==null?void 0:u.value)||"ab",r=parseInt(((f=document.getElementById("ps-ma-window"))==null?void 0:f.value)||"10",10),o=document.getElementById("ps-single-container"),i=document.getElementById("ps-all-container");e==="single"?(o==null||o.classList.remove("hidden"),i==null||i.classList.add("hidden"),t&&ho(t,n,a,s,r)):(o==null||o.classList.add("hidden"),i==null||i.classList.remove("hidden"),_o(n,a,s,r))}function ho(e,t,n,a,s){var y;const{games:r,bStats:o,pStats:i}=yn,d=((y=rt.find(p=>p.id==e))==null?void 0:y.name)||"選手",c=(t==="batter"?o:i).filter(p=>p.player_id==e).map(p=>{const g=r.find(v=>v.id===p.game_id);return{date:(g==null?void 0:g.date)||"",game:g,stats:p}}).filter(p=>p.date).sort((p,g)=>p.date.localeCompare(g.date)),m=t==="batter"?It(c.map(p=>p.stats)):$t(c.map(p=>p.stats));let u=[...c];if(n!=="all"){const p=parseInt(n,10);u.length>p&&(u=u.slice(u.length-p))}const f=t==="batter"?It(u.map(p=>p.stats)):$t(u.map(p=>p.stats));xo(d,t,f,m,u);const b=ar(u,t,a,s);vo(d,t,u),wo(d,t,b,a,s),Eo(t,u)}function xo(e,t,n,a,s){const r=document.getElementById("ps-highlight-cards");if(r)if(t==="batter"){const o=n.avg-a.avg,i=(o>=0?"+":"")+o.toFixed(3).replace(/^0/,""),d=o>=.03,l=o<=-.05,c=d?"🔥 好調":l?"❄️ 不調":"⚖️ 安定",m=d?"bg-red-100 text-red-700 border-red-200":l?"bg-blue-100 text-blue-700 border-blue-200":"bg-gray-100 text-gray-700 border-gray-200";let u=0;for(let f=s.length-1;f>=0;f--){const b=s[f].stats.hits||0,y=s[f].stats.at_bats||0;if(b>0)u++;else if(y>0)break}r.innerHTML=`
        ...
        `,a.innerHTML=r.map(o=>{var q,B,A;const i=o.stats,d=ct((q=o.game)==null?void 0:q.team_first)?(B=o.game)==null?void 0:B.team_second:(A=o.game)==null?void 0:A.team_first,l=i.plate_appearances||0,c=i.at_bats||0,m=i.hits||0,u=i.doubles||0,f=i.triples||0,b=i.home_runs||0,y=i.singles!==void 0?i.singles:Math.max(0,m-u-f-b),p=i.runs_batted_in||0,g=i.runs||0,v=i.walks||0,x=i.hit_by_pitch||0,h=i.strikeouts||i.strike_outs||0,k=i.sacrifice_flies||i.sac_flies||0,L=(i.sacrifice_hits||i.sac_bunts||0)+k,w=i.stolen_bases||0,I=i.errors||0,E=c>0?(m/c).toFixed(3).replace(/^0/,""):".000",$=c+v+x+k,_=$>0?(m+v+x)/$:l>0?(m+v+x)/l:0;_.toFixed(3).replace(/^0/,"");const C=y+u*2+f*3+b*4,P=i.total_bases&&i.total_bases>C?i.total_bases:C,D=c>0?P/c:0;D.toFixed(3).replace(/^0/,"");const O=(_+D).toFixed(3);return`
        ...
        `,a.innerHTML=r.map(o=>{var w,I,E;const i=o.stats,d=ct((w=o.game)==null?void 0:w.team_first)?(I=o.game)==null?void 0:I.team_second:(E=o.game)==null?void 0:E.team_first,l=i.outs||0,c=`${Math.floor(l/3)}${l%3!==0?"."+l%3:""}`,m=i.pitch_count||0,u=i.strikes||0,f=i.hits_allowed||0,b=i.home_runs_allowed||0,y=i.walks_allowed||0,p=i.hit_batters||0,g=i.strike_outs||0,v=i.runs_allowed||0,x=i.earned_runs||0,h=l/3,k=h>0?(x*7/h).toFixed(2):"0.00",L=h>0?((f+y)/h).toFixed(2):"0.00";return`
                <tr class="hover:bg-gray-50">
                    <td class="p-2 border font-bold">${o.date}</td>
                    <td class="p-2 border">${d||"不明"}</td>
                    <td class="p-2 border text-center font-bold">${c}</td>
            ...
            `}).join(""))}function _o(e,t,n,a){const s=document.getElementById("ps-all-title"),r=document.getElementById("ps-all-subtitle"),o=document.getElementById("ps-all-thead"),i=document.getElementById("ps-all-tbody");if(!o||!i)return;const{games:d,bStats:l,pStats:c}=yn,m=t==="all"?"全試合":`直近 ${t} 試合`;s&&(s.textContent=`👥 全選手成績一覧 (${e==="batter"?"打撃":"投手"})`),r&&(r.textContent=`対象: ${m} / 移動平均: 直近${a}${n==="ab"?"打数/登板":"試合"} (各列クリックでソート)`);const u=new Set;l.forEach(g=>u.add(g.player_id)),c.forEach(g=>u.add(g.player_id));const b=rt.filter(g=>u.has(g.id)).sort((g,v)=>g.name.localeCompare(v.name)).map(g=>{let h=[...(e==="batter"?l:c).filter(I=>I.player_id==g.id).map(I=>{const E=d.find($=>$.id===I.game_id);return{date:(E==null?void 0:E.date)||"",game:E,stats:I}}).filter(I=>I.date).sort((I,E)=>I.date.localeCompare(E.date))];if(t!=="all"){const I=parseInt(t,10);h.length>I&&(h=h.slice(h.length-I))}const k=e==="batter"?It(h.map(I=>I.stats)):$t(h.map(I=>I.stats)),L=ar(h,e,n,a),w=L.length>0?L[L.length-1].calc:k;return{player:g,name:g.name,gameCount:h.length,calcPeriod:k,latestMa:w,ab:k.ab,h:k.h,hr:k.hr,rbi:k.rbi,r:k.r,bb:k.bb+k.hbp,avg:k.avg,obp:k.obp,slg:k.slg,ops:k.ops,runRate:k.runRate,maAvg:w.avg,maOps:w.ops,outs:k.outs,so:k.so,pBb:k.bb,wins:k.wins,losses:k.losses,era:k.era,whip:k.whip,maEra:w.era,maWhip:w.whip}}),y=nn[e];b.sort((g,v)=>{let x=g[y.key],h=v[y.key];return x===void 0&&(x=0),h===void 0&&(h=0),typeof x=="string"?y.order==="asc"?x.localeCompare(h):h.localeCompare(x):y.order==="asc"?x-h:h-x});const p=g=>y.key===g?y.order==="asc"?" ▲":" ▼":"";e==="batter"?(o.innerHTML=`
            <tr>
...
詳細: `+e.message)}}async function No(){Te=[{id:"team_default",name:"ありんこアントス (A軍)"},{id:"team_b",name:"ありんこアントス (B軍・ジュニア)"}],Se=Te[0].id,wt=[],ge=[{id:"p1",name:"とあ",number:"2",teamId:"team_default"},{id:"p2",name:"そうま",number:"10",teamId:"team_default"},{id:"p3",name:"あきと",number:"3",teamId:"team_default"},{id:"p4",name:"ゆうき",number:"4",teamId:"team_default"},{id:"p5",name:"あいのすけ",number:"1",teamId:"team_default"},{id:"p6",name:"けんせい",number:"6",teamId:"team_default"},{id:"p7",name:"りゅうと",number:"7",teamId:"team_default"},{id:"p8",name:"ながまさ",number:"8",teamId:"team_default"},{id:"p9",name:"そうすけ",number:"9",teamId:"team_default"},{id:"p10",name:"たいち",number:"5",teamId:"team_default"}],Ue=[]}async function ns(){return null}async function sr(){return null}async function la(e,t=!1){return null}async function Yn(e,t=!1){return null}async function Me(e){e&&(await Yn(),hn())}function Rt(){const e=document.getElementById("sim-team-select"),t=document.getElementById("new-sim-team"),n=document.getElementById("sim-current-team-label"),a=tn();n&&(n.textContent=`[${a.name}]`),e&&(e.innerHTML="",Te.forEach(s=>{const r=document.createElement("option");r.value=s.id,r.textContent=s.name,e.appendChild(r)}),e.value=Se),t&&(t.innerHTML="",Te.forEach(s=>{const r=document.createElement("option");r.value=s.id,r.textContent=s.name,t.appendChild(r)}),t.value=Se)}function ca(){const e=document.getElementById("sim-template-select-inline"),t=document.getElementById("new-sim-template");e&&(e.innerHTML='<option value="">(選択して適用)</option>',wt.forEach(n=>{const a=document.createElement("option");a.value=n.id,a.textContent=n.name,e.appendChild(a)})),t&&(t.innerHTML='<option value="">(未配置から開始)</option>',wt.forEach(n=>{const a=document.createElement("option");a.value=n.id,a.textContent=n.name,t.appendChild(a)}))}async function Po(e="",t=null,n=null,a=null){const s="pat_"+Date.now(),r=t||Se,o=Te.find(u=>u.id===r)||tn();let i={},d={},l=a!==null?a:Q;if(n){const u=wt.find(f=>f.id===n);u&&(i=JSON.parse(JSON.stringify(u.basePositions||{})),d=JSON.parse(JSON.stringify(u.battingOrder||{})),l=u.mode||l)}Q=l,Se=r;const c={id:s,name:e||"新規データ",teamId:r,mode:Q,basePositions:i,customSubstitutions:[],battingOrder:d,headerInfo:{date:"",tournament:"",teamHome:o.name||"ありんこアントス",teamVisitor:"",manager:"",captain:"",scorer:"",stadium:"",time:""}};Ue.push(c),Oe=s,await Me(c),Rt(),Ft(),hn();const m=document.getElementById("sim-pattern-select");m&&(m.value=s)}function hn(){const e=document.getElementById("sim-pattern-select");e&&(e.innerHTML='<option value="">選択してください...</option>',Ue.forEach(t=>{const n=document.createElement("option");n.value=t.id;const a=t.isSynced===!1?" (未同期)":"",s=Te.find(o=>o.id===t.teamId),r=s?`[${s.name}] `:"";n.textContent=r+t.name+a,e.appendChild(n)}),Oe&&(e.value=Oe))}function Ft(){const e=document.getElementById("btn-sim-mode-9"),t=document.getElementById("btn-sim-mode-10");Q===9?(e.className="px-3 py-1.5 text-xs font-bold bg-amber-600 text-white transition",t.className="px-3 py-1.5 text-xs font-bold bg-white text-gray-700 border-l hover:bg-gray-50 transition"):(e.className="px-3 py-1.5 text-xs font-bold bg-white text-gray-700 transition",t.className="px-3 py-1.5 text-xs font-bold bg-amber-600 text-white border-l hover:bg-gray-50 transition")}function Dn(e){const t={...e.basePositions||{}},n=Q===9?ot:it;return Object.keys(t).forEach(s=>{n.includes(s)||delete t[s]}),(e.customSubstitutions||[]).filter(s=>s.active).forEach(s=>{if(s.type==="swap"){const{pos1:r,pos2:o}=s.details;if(n.includes(r)&&n.includes(o)){const i=t[r];t[r]=t[o],t[o]=i}}else if(s.type==="sub"){const{outPlayerId:r,inPlayerId:o,pos:i}=s.details;if(r){const d=Object.keys(t).find(l=>t[l]===r);d&&(t[d]=o)}else i&&n.includes(i)&&(t[i]=o)}else if(s.type==="rotation"){const o=s.details.positions.filter(i=>n.includes(i));if(o.length>1){const i=o.map(d=>t[d]);for(let d=0;d<o.length;d++){const l=i[(d-1+o.length)%o.length];t[o[d]]=l}}}}),t}function Nn(e){const t=new Set;return(e.customSubstitutions||[]).filter(a=>a.active).forEach(a=>{a.type==="sub"&&a.details.outPlayerId&&t.add(a.details.outPlayerId)}),t}function mn(e){e.battingOrder||(e.battingOrder={});const t=Q===9?ot:it,n=e.basePositions||{},a=new Set;t.forEach(i=>{n[i]&&a.add(n[i])}),Object.keys(e.battingOrder).forEach(i=>{const d=e.battingOrder[i];a.has(d)||delete e.battingOrder[i]});const s=new Set(Object.values(e.battingOrder)),r=[];a.forEach(i=>{s.has(i)||r.push(i)});const o=t.length;for(let i=1;i<=o&&r.length!==0;i++)if(!e.battingOrder[i]){const d=r.shift();e.battingOrder[i]=d}}function de(){const e=xe();if(!e)return;const t=Dn(e),n=Nn(e),a=new Set(Object.values(t).filter(Boolean));mn(e),Oo(a,n),Mo(a,n),Ro(t,e),Fo(e),qo(e),jo(e)}function Oo(e,t){const n=document.getElementById("sim-players-list"),a=document.getElementById("sim-player-count");if(!n)return;n.innerHTML="";const s=Cn();if(a&&(a.textContent=`${s.length} 人`),s.length===0){n.innerHTML='<span class="text-xs text-gray-400 p-2">このチームに登録されている選手がいません。</span>';return}s.forEach(r=>{const o=e.has(r.id),i=t.has(r.id),d=we===r.id&&pe==="players-list",l=document.createElement("div");l.className=`sim-player-badge ${o?"assigned":""} ${i?"retired":""} ${d?"selected":""}`,l.setAttribute("data-player-id",r.id),!o&&!i&&(l.setAttribute("draggable","true"),l.addEventListener("dragstart",rr));const c=r.number?`#${r.number} `:"";l.innerHTML=`
        ...
        `,n.appendChild(l)})}function Fo(e){const t=document.getElementById("sim-batting-order-list");if(!t)return;t.innerHTML="";const n=Q===9?ot:it,a=n.length,s=e.basePositions||{},r={};n.forEach(o=>{const i=s[o];i&&(r[i]=o)});for(let o=1;o<=a;o++){const i=e.battingOrder[o],d=i?ge.find(p=>p.id===i):null,l=d?r[d.id]:null,c=l?ht[l]:"未配置",m=document.createElement("div");d?m.className="flex items-center justify-between bg-amber-50/50 border border-amber-100 rounded-lg p-2 text-xs transition-colors duration-150":m.className="flex items-center justify-between bg-gray-50/50 border border-dashed border-gray-200 rounded-lg p-2 text-xs text-gray-400 transition-colors duration-150";const u=document.createElement("div");u.className="flex items-center gap-1 shrink-0";const f=document.createElement("button");f.className="px-2 py-1 bg-amber-100 hover:bg-amber-200 disabled:opacity-30 disabled:cursor-not-allowed text-amber-900 font-bold rounded text-[10px] leading-none transition shadow-sm",f.textContent="▲",o===1&&(f.disabled=!0),f.addEventListener("click",()=>Bs(o,"up"));const b=document.createElement("button");b.className="px-2 py-1 bg-amber-100 hover:bg-amber-200 disabled:opacity-30 disabled:cursor-not-allowed text-amber-900 font-bold rounded text-[10px] leading-none transition shadow-sm",b.textContent="▼",o===a&&(b.disabled=!0),b.addEventListener("click",()=>Bs(o,"down")),u.appendChild(f),u.appendChild(b);let y="";if(d){const p=d.number?`#${d.number} `:"";y=`
                <div class="flex items-center gap-2">
                    ...
                    <span class="font-bold text-gray-800">${p}${me(d.name)}</span>
                </div>
                <div class="flex items-center gap-2">
            ...
            `;m.innerHTML=y,m.appendChild(u),t.appendChild(m)}}async function Bs(e,t){const n=xe();if(!n)return;n.battingOrder||(n.battingOrder={});const a=t==="up"?e-1:e+1,r=(Q===9?ot:it).length;if(a<1||a>r)return;const o=n.battingOrder[e],i=n.battingOrder[a];o&&i?(n.battingOrder[e]=i,n.battingOrder[a]=o):o?(n.battingOrder[a]=o,delete n.battingOrder[e]):i&&(n.battingOrder[e]=i,delete n.battingOrder[a]),await Me(n),de()}function as(e,t,n){if(!e)return{code:"",desc:"",fullDesc:""};if(e.type==="sub"){const{outPlayerId:a,inPlayerId:s}=e.details,r=ge.find(y=>y.id===a),o=ge.find(y=>y.id===s),i=r?r.name:"不明",d=o?o.name:"不明",l=Object.keys(t).find(y=>t[y]===a),c=l&&on[l]||"",m=l?ht[l]||"":"選手",u=c?`交代 (${c})`:"交代",f=n?`<span class="font-bold text-amber-900">${me(i)}</span>に代わって<span class="font-bold text-amber-900">${me(d)}</span>`:`${i}に代わって${d}`,b=`${m}の${i}に代わりまして、${d}が入ります。`;return{code:u,desc:f,fullDesc:b}}else if(e.type==="rotation"){const a=e.details.positions||[],r=a.map(l=>on[l]||l).join("-"),o=[];for(let l=0;l<a.length-1;l++){const c=a[l],m=a[l+1],u=t[c],f=ge.find(b=>b.id===u);if(f){const b=n?`<span class="font-bold text-amber-900">${me(f.name)}</span>`:f.name;o.push(`${b}が${ht[m]}`)}}const i=o.join("、"),d=i?`${i}へ。`:"ポジション交代";return{code:r,desc:i,fullDesc:d}}else if(e.type==="swap"){const{pos1:a,pos2:s}=e.details,r=t[a],o=t[s],i=ge.find(v=>v.id===r),d=ge.find(v=>v.id===o),l=on[a]||a,c=on[s]||s,m=ht[a]||"",u=ht[s]||"",f=i?i.name:"未配置",b=d?d.name:"未配置",y=`${l}⇔${c}`,p=n?`<span class="font-bold text-amber-900">${me(f)}</span>と<span class="font-bold text-amber-900">${me(b)}</span>の入れ替え`:`${f}と${b}の入れ替え`,g=`${m}の${f}と${u}の${b}が入れ替わります。`;return{code:y,desc:p,fullDesc:g}}return{code:"",desc:"",fullDesc:""}}function qo(e){const t=document.getElementById("sim-sub-rules-list");if(!t)return;t.innerHTML="";const n=e.customSubstitutions||[];if(n.length===0){t.innerHTML='<p class="text-xs text-gray-400 text-center py-4">登録された交代はありません。</p>';return}const a=Q===9?ot:it;let s={...e.basePositions||{}};n.forEach(r=>{const o=as(r,s,!0);if(r.active){if(r.type==="swap"){const{pos1:l,pos2:c}=r.details;if(a.includes(l)&&a.includes(c)){const m=s[l];s[l]=s[c],s[c]=m}}else if(r.type==="sub"){const{outPlayerId:l,inPlayerId:c,pos:m}=r.details;if(l){const u=Object.keys(s).find(f=>s[f]===l);u&&(s[u]=c)}else m&&a.includes(m)&&(s[m]=c)}else if(r.type==="rotation"){const c=r.details.positions.filter(m=>a.includes(m));if(c.length>1){const m=c.map(u=>s[u]);for(let u=0;u<c.length;u++){const f=m[(u-1+c.length)%c.length];s[c[u]]=f}}}}const i=document.createElement("div");i.className=`sub-rule-card ${r.active?"active":""}`;const d=r.name?`<span class="text-[10px] font-bold text-amber-800 bg-amber-50 px-1.5 py-0.5 rounded leading-none inline-block mb-1 border border-amber-200/50">${me(r.name)}</span>`:"";i.innerHTML=`
            <div class="flex items-center gap-2">
        ...
        `,t.appendChild(d)})}function Ae(){const e=xe();if(!e)return;const t=Q===9?ot:it,n=document.getElementById("sub-player-out"),a=document.getElementById("sub-player-in"),s=Dn(e),r=new Set(Object.values(s).filter(Boolean)),o=Nn(e);if(n&&(n.innerHTML="",t.forEach(d=>{var c;const l=s[d];if(l){const m=((c=ge.find(f=>f.id===l))==null?void 0:c.name)||"未配置",u=document.createElement("option");u.value=l,u.textContent=`${m} (${ht[d]})`,n.appendChild(u)}})),a){a.innerHTML="";const l=Cn().filter(c=>!r.has(c.id)&&!o.has(c.id));l.length===0?a.innerHTML='<option value="">控え選手なし</option>':l.forEach(c=>{const m=document.createElement("option");m.value=c.id,m.textContent=c.name,a.appendChild(m)})}const i=document.getElementById("rot-input-text");i&&(i.value="")}async function Ho(e,t){const n=xe();if(!n)return;const a=n.customSubstitutions.find(s=>s.id===e);a&&(a.active=t,await Me(n),Ae(),de())}async function Uo(){const e=xe();if(!e)return;const t=document.getElementById("rule-type-select"),n=t?t.value:"rotation";let a={};if(n==="sub"){const d=document.getElementById("sub-player-out").value,l=document.getElementById("sub-player-in").value;if(!d){alert("退く選手を選択してください。");return}if(!l){alert("入る控え選手を選択してください。");return}a={outPlayerId:d,inPlayerId:l}}else if(n==="rotation"){const d=document.getElementById("rot-input-text"),l=d?d.value.trim():"";if(!l){alert("交代ルートを入力してください（例: 1-3-1）。");return}if(!/^[0-9a-zA-Z]+(-[0-9a-zA-Z]+)*$/.test(l)){alert("入力形式が正しくありません。半角数字・英字とハイフンで入力してください（例: 1-3-1）。");return}const c=l.split("-").filter(Boolean);if(c.length<2){alert("交代には最低2つのポジションが必要です（例: 1-3）。");return}const m=Q===9?ot:it,u=[];for(const f of c){const b=Do[f];if(!b){alert(`無効なポジション番号「${f}」が含まれています。1〜9またはDHを指定してください。`);return}if(!m.includes(b)){alert(`ポジション「${f}」(${ht[b]||b})は、現在の守備モード（DH${Q===10?"あり":"なし"}）では使用できません。`);return}u.push(b)}for(let f=0;f<u.length-1;f++)if(u[f]===u[f+1]){alert("隣り合うポジションに同じものを指定することはできません（例: 1-1-3 は不可）。");return}a={positions:u}}const s=document.getElementById("rule-name-input"),r=s?s.value.trim():"",o={id:"rule_"+Date.now(),name:r,type:n,active:!1,details:a};e.customSubstitutions||(e.customSubstitutions=[]),e.customSubstitutions.push(o),await Me(e),s&&(s.value="");const i=document.getElementById("rot-input-text");i&&(i.value=""),Ae(),de(),Kn("subrules")}async function Vo(e){const t=xe();t&&(t.customSubstitutions=t.customSubstitutions.filter(n=>n.id!==e),await Me(t),Ae(),de())}function Kn(e){const t=document.getElementById("tab-btn-setup"),n=document.getElementById("tab-btn-subrules"),a=document.getElementById("panel-setup"),s=document.getElementById("panel-subrules");e==="setup"?(t.className="flex-1 py-2 px-3 text-center text-sm font-bold bg-amber-600 text-white transition",n.className="flex-1 py-2 px-3 text-center text-sm font-bold bg-white text-gray-700 border-l hover:bg-gray-50 transition",a.classList.remove("hidden"),s.classList.add("hidden")):(t.className="flex-1 py-2 px-3 text-center text-sm font-bold bg-white text-gray-700 transition",n.className="flex-1 py-2 px-3 text-center text-sm font-bold bg-amber-600 text-white border-l hover:bg-gray-50 transition font-bold",a.classList.add("hidden"),s.classList.remove("hidden"))}function rr(e){const t=e.currentTarget.getAttribute("data-player-id");e.dataTransfer.setData("text/plain",t);let n="bench";e.currentTarget.parentNode.id==="sim-players-list"&&(n="players-list"),e.dataTransfer.setData("source-pos",n)}function Go(e){e.preventDefault(),e.currentTarget.classList.add("drag-over")}function Wo(e){e.currentTarget.classList.remove("drag-over")}function zo(e){e.preventDefault(),e.currentTarget.classList.remove("drag-over");const t=e.dataTransfer.getData("text/plain"),n=e.currentTarget.getAttribute("data-position");!t||!n||or(t,n)}async function or(e,t){const n=xe();if(!n)return;n.basePositions||(n.basePositions={});let a=null;Object.keys(n.basePositions).forEach(r=>{n.basePositions[r]===e&&(a=r)});const s=n.basePositions[t];a&&(n.basePositions[a]=s),n.basePositions[t]=e,mn(n),await Me(n),we=null,pe=null,Ae(),de()}function ir(e,t){we===e&&pe===t?(we=null,pe=null):(we=e,pe=t),de()}async function Jo(e){const t=xe();if(!t)return;t.basePositions||(t.basePositions={});const n=t.basePositions[e];if(we)if(pe!=="players-list"&&pe!=="bench"){const a=t.basePositions[pe];t.basePositions[pe]=n,t.basePositions[e]=a,mn(t),await Me(t),we=null,pe=null,Ae(),de()}else or(we,e);else n&&(we=n,pe=e,de())}async function Yo(e){const t=xe();t&&(t.basePositions||(t.basePositions={}),t.basePositions[e]=null,mn(t),await Me(t),we=null,pe=null,Ae(),de())}async function Ko(){const e=prompt(`新しいチーム名を入力してください:
(例: ありんこアントス (B軍), ジュニア選抜など)`);if(!e||!e.trim())return;const t=e.trim();if(Te.some(a=>a.name===t)){alert("同じ名前のチームが既に存在します。");return}const n={id:"team_"+Date.now(),name:t};Te.push(n),Se=n.id,await ns(),Rt(),de()}async function Qo(){const e=tn(),t=prompt("チーム名を変更してください:",e.name);if(!t||!t.trim())return;const n=t.trim();if(n!==e.name){if(Te.some(a=>a.id!==e.id&&a.name===n)){alert("同じ名前のチームが既に存在します。");return}e.name=n,await ns(),Rt(),hn(),de()}}async function Zo(){if(Te.length<=1){alert("登録チームが1つのみのため、削除できません。");return}const e=tn(),t=Cn();let n=`チーム「${e.name}」を削除しますか？`;if(t.length>0&&(n+=`
※このチームに登録されている ${t.length} 名の選手データも削除されます。`),!confirm(n))return;const a=new Set(t.map(s=>s.id));ge=ge.filter(s=>!a.has(s.id));for(const s of t)await la(s,!0);Te=Te.filter(s=>s.id!==e.id),Se=Te[0].id,await ns(),Rt(),de()}function Xo(e){Se=e.target.value;const t=document.getElementById("sim-current-team-label"),n=tn();t&&(t.textContent=`[${n.name}]`),we=null,pe=null,Ae(),de()}async function ei(){const e=xe();if(!e)return;const t=e.name?`${e.name}の基本形`:"基本配置パターン",n=prompt(`初期パターン（テンプレート）の登録名を入力してください:
...
（現在のスタメン配置・打順が上書きされます）`)){const s=document.getElementById("sim-template-select-inline");s&&(s.value="");return}const n=xe();n&&(n.basePositions=JSON.parse(JSON.stringify(t.basePositions||{})),n.battingOrder=JSON.parse(JSON.stringify(t.battingOrder||{})),t.mode&&(n.mode=t.mode,Q=t.mode,Ft()),await Me(n));const a=document.getElementById("sim-template-select-inline");a&&(a.value=""),Ae(),de()}async function ni(){const e=document.getElementById("sim-template-select-inline"),t=e?e.value:"";if(!t){alert("削除したい初期パターンを選択してください。");return}const n=wt.find(a=>a.id===t);n&&confirm(`初期パターン「${n.name}」を削除しますか？`)&&(wt=wt.filter(a=>a.id!==t),await sr(),ca(),alert(`初期パターン「${n.name}」を削除しました。`))}function ai(){const e=document.getElementById("sim-new-modal");if(!e)return;Rt(),ca();const t=document.getElementById("new-sim-name");if(t){const r=new Date,o=`${r.getFullYear()}/${String(r.getMonth()+1).padStart(2,"0")}/${String(r.getDate()).padStart(2,"0")}`;t.value=`${o} 練習試合`}const n=document.getElementById("new-sim-team");n&&(n.value=Se);const a=document.getElementById("new-sim-template");a&&(a.value="");const s=document.querySelector(`input[name="new-sim-dh-mode"][value="${Q}"]`);s&&(s.checked=!0),e.classList.remove("hidden")}function Ra(){const e=document.getElementById("sim-new-modal");e&&e.classList.add("hidden")}async function si(){const e=document.getElementById("new-sim-name"),t=document.getElementById("new-sim-team"),n=document.getElementById("new-sim-template"),a=document.querySelector('input[name="new-sim-dh-mode"]:checked'),s=e?e.value.trim():"",r=t?t.value:Se,o=n?n.value:null,i=a?parseInt(a.value,10):9;if(!s){alert("シミュレーションデータ名を入力してください。");return}await Po(s,r,o,i),Ra(),Ae(),de()}async function Ia(){const e=document.getElementById("sim-new-player-input"),t=document.getElementById("sim-new-player-number");if(!e)return;const n=e.value.trim(),a=t?t.value.trim():"";if(!n)return;if(Cn().some(o=>o.name===n)){alert("このチームには同じフルネームの選手が既に登録されています。");return}const r={id:"p_"+Date.now()+"_"+Math.random().toString(36).substr(2,5),name:n,number:a,teamId:Se};ge.push(r),await la(),e.value="",t&&(t.value=""),Ae(),de()}async function ri(e){const t=ge.find(n=>n.id===e);if(t&&confirm(`選手「${t.name}」を削除しますか？
（データベースの全データ配置・交代設定からも削除されます）`)){ge=ge.filter(n=>n.id!==e),await la(t,!0);for(const n of Ue){let a=!1;if(n.basePositions||(n.basePositions={}),Object.keys(n.basePositions).forEach(s=>{n.basePositions[s]===e&&(n.basePositions[s]=null,a=!0)}),n.battingOrder&&Object.keys(n.battingOrder).forEach(s=>{n.battingOrder[s]===e&&(delete n.battingOrder[s],a=!0)}),n.customSubstitutions){const s=n.customSubstitutions.length;n.customSubstitutions=n.customSubstitutions.filter(r=>r.type==="sub"?r.details.outPlayerId!==e&&r.details.inPlayerId!==e:!0),n.customSubstitutions.length!==s&&(a=!0)}a&&await Me(n)}we===e&&(we=null,pe=null),Ae(),de()}}async function oi(){const e=document.getElementById("sim-pattern-select");if(!e)return;const t=e.value;let n=!1,a=null,s="";const r=xe();if(t){const o=e.options[e.selectedIndex];confirm(`現在「${o.text}」が選択されています。
このデータに上書き保存しますか？
（「キャンセル」を選ぶと新規保存になります）`)&&(n=!0,s=o.text,a=t)}if(!n){const o=r?r.name:"新規配置データ",i=prompt("保存名を入力してください：",o);if(!i)return;if(s=i.trim(),!s){alert("有効な保存名を入力してください。");return}}try{if(n&&a){if(r){r.name=s,r.teamId=Se,r.mode=Q;const i=await Yn(r);if(i)throw i;alert(`データ「${s}」を上書き保存しました。`)}}else{const i="pat_"+Date.now(),d=tn(),l={id:i,name:s,teamId:Se,mode:Q,basePositions:r?{...r.basePositions||{}}:{},customSubstitutions:r?JSON.parse(JSON.stringify(r.customSubstitutions||[])):[],battingOrder:r?{...r.battingOrder||{}}:{},headerInfo:r?JSON.parse(JSON.stringify(r.headerInfo||{})):{date:"",tournament:"",teamHome:d.name||"ありんこアントス",teamVisitor:"",manager:"",captain:"",scorer:"",stadium:"",time:""}};Ue.push(l),Oe=i;const c=await Yn(l);if(c)throw c;alert(`データ「${s}」を保存しました。`)}hn();const o=document.getElementById("sim-pattern-select");o&&(o.value=Oe),de()}catch(o){console.error("Save pattern error:",o),alert(`保存に失敗しました。
エラー詳細: ${o.message||o}`)}}async function ii(){const e=xe();if(!e||!e.id){alert("削除する配置データが選択されていません。");return}if(!confirm(`配置データ「${e.name}」を削除しますか？
（データベースから削除されます）`))return;Ue=Ue.filter(n=>n.id!==Oe);const t=await Yn(e,!0);Ue.length>0?Oe=Ue[0].id:Oe="",hn(),alert(t?`データ「${e.name}」のデータベースからの削除に失敗しました。
エラー詳細: ${t.message||t}`:`データ「${e.name}」を削除しました。`),we=null,pe=null,Ft(),Ae(),de()}async function di(e){const t=e.target.value;if(!t)Oe="",Un=null,we=null,pe=null,de();else{Oe=t;const n=xe();n&&(Q=n.mode||9,n.teamId&&Te.some(a=>a.id===n.teamId)&&(Se=n.teamId,Rt())),we=null,pe=null,Ft(),Ae(),de()}}function li(){const e=xe();if(!e)return;const t=Dn(e),n=Nn(e),a=e.headerInfo||{};document.getElementById("member-input-date").value=a.date||"",document.getElementById("member-input-tournament").value=a.tournament||"",document.getElementById("member-input-team-home").value=a.teamHome||"ありんこアントス",document.getElementById("member-input-team-visitor").value=a.teamVisitor||"",document.getElementById("member-input-manager").value=a.manager||"",document.getElementById("member-input-captain").value=a.captain||"",document.getElementById("member-input-scorer").value=a.scorer||"",document.getElementById("member-input-stadium").value=a.stadium||"",document.getElementById("member-input-time").value=a.time||"",dr(e,t,n);const s=document.getElementById("sim-member-modal");s&&s.classList.remove("hidden")}function ci(){const e=xe();if(!e)return;e.headerInfo||(e.headerInfo={}),e.headerInfo.date=document.getElementById("member-input-date").value.trim(),e.headerInfo.tournament=document.getElementById("member-input-tournament").value.trim(),e.headerInfo.teamHome=document.getElementById("member-input-team-home").value.trim(),e.headerInfo.teamVisitor=document.getElementById("member-input-team-visitor").value.trim(),e.headerInfo.manager=document.getElementById("member-input-manager").value.trim(),e.headerInfo.captain=document.getElementById("member-input-captain").value.trim(),e.headerInfo.scorer=document.getElementById("member-input-scorer").value.trim(),e.headerInfo.stadium=document.getElementById("member-input-stadium").value.trim(),e.headerInfo.time=document.getElementById("member-input-time").value.trim(),Me(e);const t=Dn(e),n=Nn(e);dr(e,t,n)}function dr(e,t,n){const a=Q===9?ot:it,s=e.headerInfo||{};let r=`【 メンバー表 (Ants) 】
...
`,r+=`◆ スターティングメンバー (打順順)
`;const o=a.length,i=e.basePositions||{},d={};a.forEach(f=>{const b=i[f];b&&(d[b]=f)});for(let f=1;f<=o;f++){const b=e.battingOrder[f];if(b){const y=ge.find(g=>g.id===b),p=d[b];if(y&&p){const g=on[p],v=y.number?` [#${y.number}]`:"";r+=`${f}. [${g}] ${ht[p]} : ${y.name}${v}
`}}}r+=`
◆ 控え選手 (ベンチ)
`;const l=new Set(Object.values(t).filter(Boolean)),c=ge.filter(f=>!l.has(f.id)&&!n.has(f.id));c.length===0?r+=`(なし)
...
`,b.type==="swap"){const{pos1:g,pos2:v}=b.details,x=f[g];f[g]=f[v],f[v]=x}else if(b.type==="sub"){const{outPlayerId:g,inPlayerId:v}=b.details,x=Object.keys(f).find(h=>f[h]===g);x&&(f[x]=v)}else if(b.type==="rotation"){const g=b.details.positions,v=g.map(x=>f[x]);for(let x=0;x<g.length;x++){const h=v[(x-1+g.length)%g.length];f[g[x]]=h}}})}const u=document.getElementById("sim-member-text");u&&(u.value=r)}function ui(){const e=document.getElementById("sim-member-text");e&&(e.select(),document.execCommand("copy"),alert("メンバー表をクリップボードにコピーしました！"))}function Ts(){const e=document.getElementById("sim-member-modal");e&&e.classList.add("hidden")}function mi(){const e=xe();if(!e)return;const t=Dn(e),n=Nn(e),a=Q===9?ot:it,s=e.headerInfo||{},r=e.basePositions||{},o={};a.forEach(b=>{const y=r[b];y&&(o[y]=b)});const i=a.length,d=[];for(let b=1;b<=i;b++){const y=e.battingOrder[b];if(y){const p=ge.find(v=>v.id===y),g=o[y];p&&g&&d.push({order:b,posNum:on[g],name:p.name,number:p.number||""})}}for(;d.length<9;)d.push({order:d.length+1,posNum:"",name:"",number:""});const l=new Set(Object.values(t).filter(Boolean)),c=ge.filter(b=>!l.has(b.id)&&!n.has(b.id)),m=[];for(let b=0;b<6;b++){const y=b*2,p=b*2+1,g=c[y],v=c[p];m.push({leftName:g?g.name:"",leftNumber:g&&g.number||"",rightName:v?v.name:"",rightNumber:v&&v.number||""})}let u="";for(let b=1;b<=4;b++){let y="";d.forEach(x=>{y+=`
                <tr>
                    <td class="cell-order">${x.order}</td>
                    <td class="cell-pos">${x.posNum}</td>
                    <td class="cell-name">${me(x.name)}</td>
                    <td class="cell-number">${x.number}</td>
                </tr>
            `}),Q===10&&d.length<10&&(y+='<tr><td class="cell-order">10</td><td class="cell-pos"></td><td class="cell-name"></td><td class="cell-number"></td></tr>');const p=Q===10?11:10,g=d.length;for(let x=g;x<p;x++)y+='<tr><td class="cell-order"></td><td class="cell-pos"></td><td class="cell-name"></td><td class="cell-number"></td></tr>';let v="";m.forEach(x=>{v+=`
                <tr>
                    <td class="cell-bench-name">${me(x.leftName)}</td>
                    <td class="cell-bench-num">${x.leftNumber}</td>
                    <td class="cell-bench-name">${me(x.rightName)}</td>
            ...
            <div class="sheet-card">
                ...
                <div class="header-table-wrapper">
                    ...
                    <table class="table-header table-header-top">
                        <tr>
                            <td class="cell-date font-variable">${me(s.date||"")}</td>
                            <td class="cell-tournament font-variable">${me(s.tournament||"")}</td>
                        </tr>
                    </table>
                    <!-- 2行目: 自チーム・相手チーム -->
                    <table class="table-header table-header-bottom">
                        <tr>
                            ...
                            <td class="cell-team-val font-variable">${me(s.teamHome||"")}</td>
                            <td class="cell-team-label-r cell-label-vertical"><span class="label-v-wrap">相手チーム</span></td>
                            <td class="cell-team-val font-variable">${me(s.teamVisitor||"")}</td>
                        </tr>
                    </table>
                </div>
                ...
                <table class="table-footer">
                    <tr>
                        ...
                        <td class="footer-val font-variable">${me(s.manager||"")}</td>
                        <td class="footer-label">主将</td>
                        <td class="footer-val font-variable">${me(s.captain||"")}</td>
                        ...
                        <td class="footer-val font-variable">${me(s.scorer||"")}</td>
                    </tr>
                    <tr>
                        <td class="footer-label">球場</td>
                        <td colspan="3" class="footer-val font-variable">${me(s.stadium||"")}</td>
                        <td class="footer-label">時間</td>
                        <td class="footer-val font-variable">${me(s.time||"")}</td>
                    </tr>
                </table>
            </div>
    ...
    `),f.document.close()}function pi(){const e=xe();if(!e)return;const t={version:"ants-sim-3.0",players:ge,pattern:e},n=new Blob([JSON.stringify(t,null,2)],{type:"application/json"}),a=URL.createObjectURL(n),s=document.createElement("a");s.href=a,s.download=`ants_positions_${e.name.replace(/[\s/\\?%*:|"<>\.]/g,"_")}.json`,document.body.appendChild(s),s.click(),document.body.removeChild(s),URL.revokeObjectURL(a)}function gi(e){const t=e.target.files[0];if(!t)return;const n=new FileReader;n.onload=async function(a){try{const s=JSON.parse(a.target.result);if(s.version!=="ants-sim-3.0"&&s.version!=="ants-sim-2.0"&&s.version!=="ants-sim-1.0"){alert("ファイル形式が正しくありません。");return}if(confirm(`インポートを実行しますか？
※既存の選手リストと配置データがマージ/追加されます。`)){for(const r of s.players)ge.some(o=>o.id===r.id||o.name===r.name)||(ge.push(r),await la(r));if(s.version==="ants-sim-3.0"){const r=s.pattern;r.id="pat_"+Date.now(),r.name=r.name+" (インポート)",Ue.push(r),Oe=r.id,Q=r.mode,await Me(r)}else if(s.version==="ants-sim-2.0"){const r=s.pattern;r.id="pat_"+Date.now(),r.name=r.name+" (インポート)",r.mode=r.mode||9,r.battingOrder={},r.headerInfo={},Ue.push(r),Oe=r.id,Q=r.mode,await Me(r)}else{const r=s.pattern,o=r.innings&&r.innings[0]?r.innings[0].positions:{},i={id:"pat_"+Date.now(),name:r.name+" (旧移行)",mode:r.mode||9,basePositions:o,customSubstitutions:[],battingOrder:{},headerInfo:{}};Ue.push(i),Oe=i.id,Q=i.mode,await Me(i)}Ft(),Ae(),de(),alert("インポートが完了しました。")}}catch(s){console.error(s),alert("ファイルの読み込みに失敗しました。")}e.target.value=""},n.readAsText(t)}function fi(){var s,r,o,i,d,l,c,m,u,f,b,y,p,g,v,x,h,k,L,w,I,E,$,_,C,P,D,O,q,B,A,N,T;const e=document.getElementById("btn-toggle-players-panel"),t=document.getElementById("players-panel-content"),n=document.getElementById("icon-toggle-players");e&&t&&n&&(e.addEventListener("click",()=>{const W=t.classList.toggle("hidden");n.textContent=W?"▼":"▲",localStorage.setItem("ants_sim_players_panel_collapsed",W?"true":"false")}),localStorage.getItem("ants_sim_players_panel_collapsed")==="true"&&(t.classList.add("hidden"),n.textContent="▼")),(s=document.getElementById("tab-btn-setup"))==null||s.addEventListener("click",()=>Kn("setup")),(r=document.getElementById("tab-btn-subrules"))==null||r.addEventListener("click",()=>Kn("subrules")),(o=document.getElementById("rule-type-select"))==null||o.addEventListener("change",M=>{const W=M.target.value;document.getElementById("form-sub").classList.add("hidden"),document.getElementById("form-rotation").classList.add("hidden"),W==="sub"?document.getElementById("form-sub").classList.remove("hidden"):W==="rotation"&&document.getElementById("form-rotation").classList.remove("hidden")}),(i=document.getElementById("btn-export-member-table"))==null||i.addEventListener("click",li),(d=document.getElementById("btn-copy-member-text"))==null||d.addEventListener("click",ui),(l=document.getElementById("btn-close-member-modal"))==null||l.addEventListener("click",Ts),(c=document.getElementById("btn-close-member-modal-footer"))==null||c.addEventListener("click",Ts),(m=document.getElementById("btn-print-member-table"))==null||m.addEventListener("click",mi),["member-input-date","member-input-tournament","member-input-team-home","member-input-team-visitor","member-input-manager","member-input-captain","member-input-scorer","member-input-stadium","member-input-time"].forEach(M=>{var W;(W=document.getElementById(M))==null||W.addEventListener("input",ci)}),(u=document.getElementById("btn-create-sub-rule"))==null||u.addEventListener("click",Uo),(f=document.getElementById("btn-add-sim-player"))==null||f.addEventListener("click",Ia),(b=document.getElementById("sim-new-player-input"))==null||b.addEventListener("keypress",M=>{M.key==="Enter"&&Ia()}),(y=document.getElementById("sim-new-player-number"))==null||y.addEventListener("keypress",M=>{M.key==="Enter"&&Ia()}),(p=document.getElementById("sim-team-select"))==null||p.addEventListener("change",Xo),(g=document.getElementById("btn-add-sim-team"))==null||g.addEventListener("click",Ko),(v=document.getElementById("btn-edit-sim-team"))==null||v.addEventListener("click",Qo),(x=document.getElementById("btn-delete-sim-team"))==null||x.addEventListener("click",Zo),(h=document.getElementById("sim-template-select-inline"))==null||h.addEventListener("change",M=>ti(M.target.value)),(k=document.getElementById("btn-save-as-template"))==null||k.addEventListener("click",ei),(L=document.getElementById("btn-delete-template"))==null||L.addEventListener("click",ni),(w=document.getElementById("btn-open-new-sim-modal"))==null||w.addEventListener("click",ai),(I=document.getElementById("btn-close-new-sim-modal"))==null||I.addEventListener("click",Ra),(E=document.getElementById("btn-close-new-sim-modal-footer"))==null||E.addEventListener("click",Ra),($=document.getElementById("btn-create-new-sim"))==null||$.addEventListener("click",si),(_=document.getElementById("sim-pattern-select"))==null||_.addEventListener("change",di),(C=document.getElementById("btn-save-sim-pattern"))==null||C.addEventListener("click",oi),(P=document.getElementById("btn-delete-sim-pattern"))==null||P.addEventListener("click",ii),(D=document.getElementById("btn-sim-mode-9"))==null||D.addEventListener("click",async()=>{if(Q!==9){Q=9,Ft();const M=xe();M&&(M.mode=9,mn(M),await Me(M)),Ae(),de()}}),(O=document.getElementById("btn-sim-mode-10"))==null||O.addEventListener("click",async()=>{if(Q!==10){Q=10,Ft();const M=xe();M&&(M.mode=10,mn(M),await Me(M)),Ae(),de()}}),(q=document.getElementById("btn-export-sim"))==null||q.addEventListener("click",pi),(B=document.getElementById("import-sim-input"))==null||B.addEventListener("change",gi),(A=document.getElementById("btn-back-to-menu-sim"))==null||A.addEventListener("click",()=>{we=null,pe=null,Y("app-menu-view")}),(N=document.getElementById("btn-logout-sim"))==null||N.addEventListener("click",()=>{var M;(M=document.getElementById("btn-logout"))==null||M.click()}),(T=document.getElementById("sim-bench-list"))==null||T.addEventListener("click",M=>{M.target.id==="sim-bench-list"&&we&&pe!=="players-list"&&pe!=="bench"&&Yo(pe)})}function me(e){return typeof e!="string"?"":e.replace(/[&<>'"]/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"})[t]||t)}let lr=null,Dt=null,wn=!1;const En="ants_surveys_v1",ln="ants_survey_responses_v1";let se=[],ue=[];const As=[{id:"survey_sample_1",title:"【夏季合宿】参加日程調整 & お弁当・合宿Tシャツ注文とりまとめ",description:`少年野球チーム Arinko Ants 夏季合宿（8月開催）の出欠日程および備品・お弁当の事前注文確認です。
**1回の送信で、保護者様ご本人と選手（兄弟含む）全員分の出欠・注文をまとめてご登録いただけます。**
ご不明な点がありましたら役員までお問い合わせください。`,status:"active",deadline:"2026-07-20 23:59",public_results:!0,enable_schedule:!0,schedule_options:["8/8(土) 午前 (練習・遠征)","8/8(土) 午後 (練習試合・BBQ)","8/8(土) 宿泊","8/9(日) 終日 (紅白戦・グラウンド納め)"],enable_family:!0,enable_orders:!0,order_items:[{id:"item_lunch_sat",name:"8/8(土) 選手・保護者弁当",price:650,max:10},{id:"item_lunch_sun",name:"8/9(日) 選手・保護者弁当",price:650,max:10},{id:"item_tshirt",name:"合宿記念チームTシャツ",price:2200,max:5},{id:"item_cap",name:" Ants オリジナル冷感タオル",price:800,max:5}],questions:[{id:"q_attendance",type:"single",title:"合宿全体の参加可否をお選びください",required:!0,options:[{label:"参加する（部分参加含む）",skip_to:null},{label:"不参加（全日程）",skip_to:"q_remarks"}],help:"不参加を選択された場合、日程詳細およびお弁当等の注文設問はスキップされます。"},{id:"q_transport",type:"single",title:"現地までの移動手段について",required:!1,options:[{label:"配車（チーム車）を利用希望",skip_to:null},{label:"自車で現地直行（他メンバー同乗可）",skip_to:null},{label:"自車で現地直行（家族のみ）",skip_to:null}],help:""},{id:"q_remarks",type:"text",title:"特記事項・アレルギー・連絡事項など",required:!1,options:[],help:"食物アレルギーや遅刻・早退のご予定等があればご記入ください。"}],created_by:"役員会",created_at:"2026-06-01",updated_at:"2026-06-01"}],Cs=[{id:"resp_sample_1",survey_id:"survey_sample_1",respondent_name:"菱沼 (保護者)",family_members:[{name:"菱沼 健一 (保護者・指導者)",role:"保護者"},{name:"菱沼 翔太 (6年・主将)",role:"選手"}],schedules:{"8/8(土) 午前 (練習・遠征)":"○","8/8(土) 午後 (練習試合・BBQ)":"○","8/8(土) 宿泊":"○","8/9(日) 終日 (紅白戦・グラウンド納め)":"○"},answers:{q_attendance:"参加する（部分参加含む）",q_transport:"自車で現地直行（他メンバー同乗可）",q_remarks:"車出し可能です。道具車としても利用できます。"},orders:{item_lunch_sat:2,item_lunch_sun:2,item_tshirt:1,item_cap:2},total_amount:6400,created_at:"2026-06-02 10:15"}];async function bi({supabaseClient:e,currentUser:t,currentUserRole:n}){lr=e,Dt=t,wn=n==="admin"||n==="leader",await Fa(),ur(),xn()}function Pn(){return lr||window.supabaseClient||null}async function Fa(){let e=!1,t=!1;const n=Pn();if(n){try{const{data:a,error:s}=await n.from("surveys").select("*").order("created_at",{ascending:!1});!s&&a&&a.length>0&&(se=a,e=!0)}catch(a){console.warn("Supabase surveys load failed, trying master_data fallback:",a)}if(!e)try{const{data:a,error:s}=await n.from("master_data").select("data").eq("key","ANTS_SURVEYS").single();!s&&a&&Array.isArray(a.data)&&a.data.length>0&&(se=a.data,e=!0)}catch(a){console.warn("Supabase master_data ANTS_SURVEYS load skipped:",a)}try{const{data:a,error:s}=await n.from("survey_responses").select("*").order("created_at",{ascending:!1});!s&&a&&a.length>0&&(ue=a,t=!0)}catch(a){console.warn("Supabase survey_responses load failed, trying master_data fallback:",a)}if(!t)try{const{data:a,error:s}=await n.from("master_data").select("data").eq("key","ANTS_SURVEY_RESPONSES").single();!s&&a&&Array.isArray(a.data)&&(ue=a.data,t=!0)}catch(a){console.warn("Supabase master_data ANTS_SURVEY_RESPONSES load skipped:",a)}}if(e)try{localStorage.setItem(En,JSON.stringify(se))}catch{}else try{const a=localStorage.getItem(En);a?se=JSON.parse(a):(se=[...As],localStorage.setItem(En,JSON.stringify(se)))}catch{se=[...As]}if(t)try{localStorage.setItem(ln,JSON.stringify(ue))}catch{}else try{const a=localStorage.getItem(ln);a?ue=JSON.parse(a):(ue=[...Cs],localStorage.setItem(ln,JSON.stringify(ue)))}catch{ue=[...Cs]}}async function yi(e,t=!1){const n=se.findIndex(s=>s.id===e.id);n>=0?se[n]=e:se.unshift(e);try{localStorage.setItem(En,JSON.stringify(se))}catch(s){console.error(s)}const a=Pn();if(a){try{t?await a.from("surveys").insert([e]):await a.from("surveys").update(e).eq("id",e.id)}catch(s){console.warn("Supabase survey table save skipped:",s)}try{await a.from("master_data").upsert({key:"ANTS_SURVEYS",data:se})}catch(s){console.warn("Supabase master_data ANTS_SURVEYS upsert failed:",s)}}}async function hi(e){se=se.filter(n=>n.id!==e),ue=ue.filter(n=>n.survey_id!==e);try{localStorage.setItem(En,JSON.stringify(se)),localStorage.setItem(ln,JSON.stringify(ue))}catch(n){console.error(n)}const t=Pn();if(t){try{await t.from("survey_responses").delete().eq("survey_id",e),await t.from("surveys").delete().eq("id",e)}catch{}try{await t.from("master_data").upsert({key:"ANTS_SURVEYS",data:se}),await t.from("master_data").upsert({key:"ANTS_SURVEY_RESPONSES",data:ue})}catch{}}}async function xi(e){const t=ue.findIndex(s=>s.id===e.id),n=t>=0;n?ue[t]=e:ue.unshift(e);try{localStorage.setItem(ln,JSON.stringify(ue))}catch(s){console.error(s)}const a=Pn();if(a){try{n?await a.from("survey_responses").update(e).eq("id",e.id):await a.from("survey_responses").insert([e])}catch(s){console.warn("Supabase survey_responses table save skipped:",s)}try{await a.from("master_data").upsert({key:"ANTS_SURVEY_RESPONSES",data:ue})}catch(s){console.warn("Supabase master_data ANTS_SURVEY_RESPONSES upsert failed:",s)}}}async function vi(e){ue=ue.filter(n=>n.id!==e);try{localStorage.setItem(ln,JSON.stringify(ue))}catch(n){console.error(n)}const t=Pn();if(t){try{await t.from("survey_responses").delete().eq("id",e)}catch{}try{await t.from("master_data").upsert({key:"ANTS_SURVEY_RESPONSES",data:ue})}catch{}}pn&&ua(pn.id),xn()}function xn(){const e=document.getElementById("info-surveys-container"),t=document.getElementById("info-surveys-empty"),n=document.getElementById("btn-survey-new");if(e){if(n&&(wn?n.classList.remove("hidden"):n.classList.add("hidden")),se.length===0){e.innerHTML="",t&&t.classList.remove("hidden");return}t&&t.classList.add("hidden"),e.innerHTML=se.map(a=>{const s=ue.filter(l=>l.survey_id===a.id),r=s.length,o=s.reduce((l,c)=>l+(Number(c.total_amount)||0),0);let i="";a.status==="active"?i='<span class="px-2.5 py-0.5 rounded-full text-xs font-bold bg-emerald-100 text-emerald-800 border border-emerald-200">🟢 受付中</span>':a.status==="closed"?i='<span class="px-2.5 py-0.5 rounded-full text-xs font-bold bg-gray-100 text-gray-700 border border-gray-200">⚪ 締切済み</span>':i='<span class="px-2.5 py-0.5 rounded-full text-xs font-bold bg-amber-100 text-amber-800 border border-amber-200">🟡 下書き</span>';const d=[];return a.enable_schedule&&d.push("📅 日程調整"),a.enable_family&&d.push("👨‍👩‍👧 家族一括"),a.enable_orders&&d.push("🛒 注文集計"),`
        ...
        `}).join(""),e.querySelectorAll(".btn-survey-respond").forEach(a=>{a.addEventListener("click",()=>{const s=a.getAttribute("data-id");cn(s)})}),e.querySelectorAll(".btn-survey-share").forEach(a=>{a.addEventListener("click",()=>{const s=a.getAttribute("data-id");wi(s)})}),e.querySelectorAll(".btn-survey-results").forEach(a=>{a.addEventListener("click",()=>{const s=a.getAttribute("data-id");ua(s)})}),wn&&(e.querySelectorAll(".btn-survey-edit").forEach(a=>{a.addEventListener("click",()=>{const s=a.getAttribute("data-id"),r=se.find(o=>o.id===s);r&&cr(r)})}),e.querySelectorAll(".btn-survey-delete").forEach(a=>{a.addEventListener("click",async()=>{const s=a.getAttribute("data-id"),r=se.find(o=>o.id===s);r&&confirm(`アンケート「${r.title}」と、そのすべての回答データを削除しますか？`)&&(await hi(s),xn())})}))}}function wi(e){const t=`${window.location.origin}${window.location.pathname}#survey-${e}`;navigator.clipboard.writeText(t).then(()=>{alert(`アンケート回答用URLをコピーしました！
LINEやメール等に貼り付けてご案内いただけます。

${t}`)}).catch(()=>{prompt("アンケート回答用URL:",t)})}let dn=null,Z=[],Yt=[],yt=[];function cr(e=null){const t=document.getElementById("modal-survey-editor");if(!t)return;dn=e?e.id:null,document.getElementById("survey-editor-modal-title").textContent=e?"アンケートの編集":"新規アンケート作成",document.getElementById("input-survey-title").value=e?e.title:"",document.getElementById("input-survey-desc").value=e&&e.description||"",document.getElementById("select-survey-status").value=e?e.status:"active",document.getElementById("input-survey-deadline").value=e&&e.deadline||"";const n=document.getElementById("chk-survey-enable-schedule"),a=document.getElementById("chk-survey-enable-family"),s=document.getElementById("chk-survey-enable-orders"),r=document.getElementById("chk-survey-public-results");n.checked=e?!!e.enable_schedule:!0,a.checked=e?!!e.enable_family:!0,s.checked=e?!!e.enable_orders:!1,r&&(r.checked=e?e.public_results!==!1:!0),Yt=e&&e.schedule_options?[...e.schedule_options]:["7/18(土) 午前","7/18(土) 午後","7/19(日) 終日"],yt=e&&e.order_items?JSON.parse(JSON.stringify(e.order_items)):[{id:`item_${Date.now()}_1`,name:"選手用お弁当",price:600,max:10},{id:`item_${Date.now()}_2`,name:"保護者用お弁当",price:700,max:10}],Z=e&&e.questions?JSON.parse(JSON.stringify(e.questions)):[],ss(),rs(),St(),ja(),t.classList.remove("hidden")}function qa(){const e=document.getElementById("modal-survey-editor");e&&e.classList.add("hidden"),dn=null}function ja(){var s,r;const e=(s=document.getElementById("chk-survey-enable-schedule"))==null?void 0:s.checked,t=(r=document.getElementById("chk-survey-enable-orders"))==null?void 0:r.checked,n=document.getElementById("editor-schedule-section"),a=document.getElementById("editor-orders-section");n&&(n.style.display=e?"block":"none"),a&&(a.style.display=t?"block":"none")}function ss(){const e=document.getElementById("editor-schedule-list");e&&(e.innerHTML=Yt.map((t,n)=>`
    ...
    `)}function $i(e,t){const n=document.getElementById("results-schedule-container");if(!n)return;if(!e.enable_schedule||!e.schedule_options||e.schedule_options.length===0){n.classList.add("hidden");return}n.classList.remove("hidden");const a={};e.schedule_options.forEach(r=>{a[r]={ok:[],maybe:[],ng:[]}}),t.forEach(r=>{const o=r.respondent_name||"無名";r.schedules&&Object.entries(r.schedules).forEach(([i,d])=>{a[i]&&(d==="○"||d==="ok"?a[i].ok.push(o):d==="△"||d==="maybe"?a[i].maybe.push(o):(d==="×"||d==="ng")&&a[i].ng.push(o))})});const s=document.getElementById("results-schedule-tbody");s&&(s.innerHTML=e.schedule_options.map(r=>{const o=a[r],i=o.ok.join("、 ");return`
        ...
        `}).join(""))}function Li(e,t){const n=document.getElementById("results-questions-list");if(n){if(!e.questions||e.questions.length===0){n.innerHTML='<p class="text-xs text-gray-400">設問はありません</p>';return}n.innerHTML=e.questions.map((a,s)=>{const r=t.map(o=>o.answers&&o.answers[a.id]).filter(o=>o!=null&&o!=="");if(a.type==="single"||a.type==="multiple"){const o={};(a.options||[]).forEach(d=>o[d.label]=0),o["(未回答)"]=0,r.forEach(d=>{Array.isArray(d)?d.forEach(l=>{o[l]=(o[l]||0)+1}):o[d]!==void 0?o[d]+=1:o[d]=(o[d]||0)+1});const i=t.length;return`
...
（集計や出欠、注文数から除外されます）`)&&await vi(i)})}))}function Bi(){if(!pn)return;const e=pn,t=ue.filter(d=>d.survey_id===e.id),n=["回答ID","回答日時","回答者(代表者名)"];e.enable_family&&n.push("家族メンバー一覧"),e.enable_schedule&&e.schedule_options&&e.schedule_options.forEach(d=>{n.push(`日程: ${d}`)}),e.questions&&e.questions.forEach((d,l)=>{n.push(`設問${l+1}: ${d.title}`)}),e.enable_orders&&e.order_items&&(e.order_items.forEach(d=>{n.push(`注文: ${d.name} (${d.price}円)`)}),n.push("注文合計金額(円)"));const a=[n];t.forEach(d=>{const l=[d.id,d.created_at||"",d.respondent_name||""];if(e.enable_family){const c=(d.family_members||[]).map(m=>`${m.name}${m.role?`(${m.role})`:""}`).join(", ");l.push(c)}e.enable_schedule&&e.schedule_options&&e.schedule_options.forEach(c=>{l.push(d.schedules&&d.schedules[c]||"")}),e.questions&&e.questions.forEach(c=>{const m=d.answers?d.answers[c.id]:"";Array.isArray(m)?l.push(m.join("; ")):l.push(m||"")}),e.enable_orders&&e.order_items&&(e.order_items.forEach(c=>{const m=d.orders&&d.orders[c.id]||0;l.push(m)}),l.push(d.total_amount||0)),a.push(l)});const s=a.map(d=>d.map(l=>`"${String(l||"").replace(/"/g,'""')}"`).join(",")).join(`
`),r=new Blob([new Uint8Array([239,187,191]),s],{type:"text/csv;charset=utf-8;"}),o=document.createElement("a");o.href=URL.createObjectURL(r);const i=new Date().toISOString().split("T")[0];o.download=`survey_${e.id}_${i}.csv`,o.click()}let $n=null,xt=[],Qn=null;async function cn(e,t=null){var c,m,u;(c=document.getElementById("loading-overlay"))==null||c.classList.add("hidden");const n=document.getElementById("loading-detail-text");n&&(n.style.opacity="0"),ur(),(!se||se.length===0)&&await Fa();let a=se.find(f=>f.id===e);if(a||(await Fa(),a=se.find(f=>f.id===e)),!a&&e)try{const f=decodeURIComponent(e).trim();a=se.find(b=>b.id===f)}catch{}if(!a){alert(`指定されたアンケートが見つかりませんでした。
（アンケートID: ${e}）

最新のアンケート一覧からURLを再取得してご確認ください。`);return}$n=a,xt=[];let s=null;t&&(s=ue.find(f=>f.id===t));const r=document.getElementById("respond-edit-mode-banner"),o=document.getElementById("btn-submit-survey-response");s?(Qn=s.id,r&&r.classList.remove("hidden"),o&&(o.textContent="回答を更新する")):(Qn=null,r&&r.classList.add("hidden"),o&&(o.textContent="回答を送信する")),["auth-view","signup-view","password-reset-view","password-update-view","app-menu-view","app-view","attendance-view","view-users","dashboard-view","dashboard-settings","position-simulator-view","info-view"].forEach(f=>{const b=document.getElementById(f);b&&b.classList.add("hidden")});const i=document.getElementById("survey-respond-view");i&&i.classList.remove("hidden"),document.getElementById("respond-survey-title").textContent=a.title,document.getElementById("respond-survey-desc").innerHTML=a.description?a.description.replace(/\n/g,"<br>"):"",document.getElementById("respond-survey-deadline").textContent=a.deadline?`回答期限: ${a.deadline}`:"";const d=document.getElementById("btn-view-public-results");d&&(a.public_results!==!1?(d.classList.remove("hidden"),d.onclick=()=>ua(a.id,!0)):d.classList.add("hidden"));const l=document.getElementById("input-respondent-name");l&&(s?l.value=s.respondent_name||"":l.value=Dt&&Dt.name?Dt.name:""),s&&s.family_members?xt=JSON.parse(JSON.stringify(s.family_members)):xt=[],os(a),Ti(a,s?s.schedules:null),Ai(a,s?s.answers:null),Di(a,s?s.orders:null),(m=document.getElementById("survey-respond-form-container"))==null||m.classList.remove("hidden"),(u=document.getElementById("survey-respond-success-container"))==null||u.classList.add("hidden"),window.scrollTo({top:0,behavior:"smooth"})}function os(e){const t=document.getElementById("respond-family-section");if(!t)return;if(!e.enable_family){t.classList.add("hidden");return}t.classList.remove("hidden");const n=document.getElementById("respond-family-list");n&&(n.innerHTML=xt.map((a,s)=>`
    ...
    `).join(""),n.querySelectorAll(".input-fam-name").forEach(a=>{a.addEventListener("input",s=>{const r=Number(s.target.getAttribute("data-index"));xt[r].name=s.target.value})}),n.querySelectorAll(".select-fam-role").forEach(a=>{a.addEventListener("change",s=>{const r=Number(s.target.getAttribute("data-index"));xt[r].role=s.target.value})}),n.querySelectorAll(".btn-del-fam").forEach(a=>{a.addEventListener("click",()=>{const s=Number(a.getAttribute("data-index"));xt.splice(s,1),os(e)})}))}function Ti(e,t=null){const n=document.getElementById("respond-schedule-section");if(!n)return;if(!e.enable_schedule||!e.schedule_options||e.schedule_options.length===0){n.classList.add("hidden");return}n.classList.remove("hidden");const a=document.getElementById("respond-schedule-list");a&&(a.innerHTML=e.schedule_options.map((s,r)=>{const o=t&&t[s]?t[s]:"○";return`
        ...
        `}).join(""),a.querySelectorAll('input[type="radio"].q-opt-radio').forEach(s=>{s.addEventListener("change",()=>{Ds(e)})}),Ds(e))}function Ci(e,t=void 0){if(e.type==="single")return(e.options||[]).map((n,a)=>{const s=t!==void 0?t===n.label:a===0;return`
        ...
        `}).join(""),a.querySelectorAll(".select-order-qty").forEach(s=>{s.addEventListener("change",Ns)}),Ns())}function Ns(){let e=0;document.querySelectorAll(".select-order-qty").forEach(n=>{const a=Number(n.value)||0,s=Number(n.getAttribute("data-price"))||0;e+=a*s});const t=document.getElementById("respond-orders-total-amount");t&&(t.textContent=`¥${e.toLocaleString()}`)}async function Ni(){var x,h,k,L;if(!$n)return;const e=$n,t=(x=document.getElementById("input-respondent-name"))==null?void 0:x.value.trim();if(!t){alert("回答者（代表者名）を入力してください"),(h=document.getElementById("input-respondent-name"))==null||h.focus();return}const n={};e.enable_schedule&&e.schedule_options&&e.schedule_options.forEach((w,I)=>{const E=document.querySelector(`input[name="respond_sched_${I}"]:checked`);E&&(n[w]=E.value)});const a={};let s=!1;if(e.questions&&e.questions.forEach(w=>{var $;const I=document.getElementById(`respond-q-block-${w.id}`);if(!(I&&I.classList.contains("opacity-40")))if(w.type==="single"){const _=document.querySelector(`input[name="respond_ans_${w.id}"]:checked`);_?a[w.id]=_.value:w.required&&(s=!0)}else if(w.type==="multiple"){const _=Array.from(document.querySelectorAll(`input[name="respond_ans_${w.id}"]:checked`)).map(C=>C.value);a[w.id]=_,w.required&&_.length===0&&(s=!0)}else{const _=($=document.querySelector(`textarea[name="respond_ans_${w.id}"]`))==null?void 0:$.value.trim();a[w.id]=_||"",w.required&&!_&&(s=!0)}}),s){alert("必須の設問に回答してください。");return}const r={};let o=0;e.enable_orders&&e.order_items&&document.querySelectorAll(".select-order-qty").forEach(w=>{const I=w.getAttribute("data-item-id"),E=Number(w.value)||0,$=Number(w.getAttribute("data-price"))||0;r[I]=E,o+=E*$});const i=!!Qn,d=Qn||`resp_${Date.now()}`,l=new Date,c=`${l.getFullYear()}-${String(l.getMonth()+1).padStart(2,"0")}-${String(l.getDate()).padStart(2,"0")} ${String(l.getHours()).padStart(2,"0")}:${String(l.getMinutes()).padStart(2,"0")}`,m={id:d,survey_id:e.id,respondent_name:t,family_members:xt.filter(w=>w.name.trim()),schedules:n,answers:a,orders:r,total_amount:o,created_at:c};await xi(m);const u=`${window.location.origin}${window.location.pathname}#survey-${e.id}&response=${m.id}`,f=document.getElementById("input-edit-response-url");f&&(f.value=u);const b=document.getElementById("btn-copy-edit-url");b&&(b.onclick=()=>{navigator.clipboard.writeText(u).then(()=>{alert(`回答修正用URLをコピーしました！
後から回答を変更する場合は、このURLから修正できます。`)}).catch(()=>{prompt("以下のURLをコピーしてください:",u)})});const y=document.getElementById("btn-re-edit-response");y&&(y.onclick=()=>{cn(e.id,m.id)});const p=document.getElementById("btn-view-results-from-success");p&&(e.public_results!==!1?(p.classList.remove("hidden"),p.onclick=()=>ua(e.id,!0)):p.classList.add("hidden"));const g=document.getElementById("survey-respond-success-title"),v=document.getElementById("survey-respond-success-desc");g&&(g.textContent=i?"回答を更新しました！":"回答を受け付けました！"),v&&(v.innerHTML=i?"回答内容の変更が正常に保存されました。<br>内容は役員・担当者へ最新状態で共有されます。":"ご回答いただきありがとうございました。<br>内容は役員・担当者へ共有されます。"),(k=document.getElementById("survey-respond-form-container"))==null||k.classList.add("hidden"),(L=document.getElementById("survey-respond-success-container"))==null||L.classList.remove("hidden"),window.scrollTo({top:0,behavior:"smooth"})}function X(e){return e?String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"):""}let Ps=!1;function ur(){var e,t,n,a,s,r,o,i,d,l,c,m,u,f;Ps||(Ps=!0,(e=document.getElementById("btn-survey-new"))==null||e.addEventListener("click",()=>cr()),(t=document.getElementById("chk-survey-enable-schedule"))==null||t.addEventListener("change",ja),(n=document.getElementById("chk-survey-enable-orders"))==null||n.addEventListener("change",ja),(a=document.getElementById("btn-add-sched-opt"))==null||a.addEventListener("click",()=>{Yt.push(`候補日 ${Yt.length+1}`),ss()}),(s=document.getElementById("btn-add-order-item"))==null||s.addEventListener("click",()=>{yt.push({id:`item_${Date.now()}`,name:`注文品目 ${yt.length+1}`,price:500,max:10}),rs()}),(r=document.getElementById("btn-add-question"))==null||r.addEventListener("click",()=>{Z.push({id:`q_${Date.now()}`,type:"single",title:`設問 ${Z.length+1}`,required:!1,options:[{label:"選択肢 1",skip_to:null},{label:"選択肢 2",skip_to:null}],help:""}),St()}),(o=document.getElementById("btn-save-survey"))==null||o.addEventListener("click",Ei),(i=document.getElementById("btn-close-survey-editor"))==null||i.addEventListener("click",qa),(d=document.getElementById("btn-cancel-survey-editor"))==null||d.addEventListener("click",qa),(l=document.getElementById("btn-close-survey-results"))==null||l.addEventListener("click",_i),(c=document.getElementById("btn-export-survey-csv"))==null||c.addEventListener("click",Bi),(m=document.getElementById("btn-respond-add-family"))==null||m.addEventListener("click",()=>{xt.push({name:"",role:"選手"}),$n&&os($n)}),(u=document.getElementById("btn-submit-survey-response"))==null||u.addEventListener("click",Ni),(f=document.getElementById("btn-back-from-survey-respond"))==null||f.addEventListener("click",()=>{if(Dt){const b=document.getElementById("survey-respond-view");b&&b.classList.add("hidden");const y=document.getElementById("info-view");y&&y.classList.remove("hidden"),xn()}else{const b=document.getElementById("survey-respond-view");b&&b.classList.add("hidden");const y=document.getElementById("auth-view");y&&y.classList.remove("hidden")}}))}let Le=null,st=null,Zn="user",Zt=!1,at="docs",Pe=[],Be=[],Xn="すべて",ea="すべて",kn="",Kt="";const ta="ants_info_documents_v1",na="ants_info_links_v1",mr="ants_info_top_md_v1",Pi=`## 🐜 Arinko Ants チーム情報 &amp; ドキュメントポータル
...
- [ ] **終了時**:
  ...
  - [ ] 倉庫施錠および学校鍵の返却`,files:[],created_by:"父母会",updated_at:"2026-04-01"}],Ms=[{id:"link-official-site",title:"Arinko Ants 公式ホームページ",url:"https://arinkoants.sakura.ne.jp/",category:"公式・連盟",description:"チームの公式Webサイト。チーム紹介、選手募集案内、活動予定などが掲載されています。",icon:"🐜",display_order:1},{id:"link-google-calendar",title:"チーム活動予定 Googleカレンダー",url:"https://calendar.google.com/",category:"スケジュール・連絡",description:"練習日、遠征試合、大会スケジュールが登録されているGoogleカレンダーです。",icon:"📅",display_order:2},{id:"link-nagareyama-league",title:"流山市少年野球連盟 公式サイト",url:"http://nagareyama-baseball.jp/",category:"公式・連盟",description:"市内大会の組み合わせトーナメント表、試合日程、グラウンド規程などが確認できます。",icon:"⚾",display_order:3},{id:"link-chiba-league",title:"千葉県少年野球連盟",url:"http://chiba-baseball.jp/",category:"公式・連盟",description:"千葉県大会の要項、大会結果、競技規則の最新情報が掲載されています。",icon:"🏆",display_order:4},{id:"link-ground-map",title:"主な活動グラウンド案内 (Google Maps)",url:"https://maps.google.com/",category:"グラウンド・施設",description:"ホームグラウンド（東小学校）および近隣の流山市内グラウンドへのアクセス地図一覧です。",icon:"📍",display_order:5},{id:"link-weather-forecast",title:"流山市のピンポイント天気予報 (tenki.jp)",url:"https://tenki.jp/forecast/3/15/4510/12220/",category:"便利ツール",description:"当日の雨雲レーダー、1時間ごとの降水確率、風速、WBGT（熱中症指数）を確認できます。",icon:"☀️",display_order:6},{id:"link-baseball-rules",title:"公認野球規則 & 少年野球特別規則",url:"https://japan-baseball.jp/",category:"便利ツール",description:"全日本軟式野球連盟（JSBB）による少年野球特別規程および公認野球規則の解説です。",icon:"📖",display_order:7}],pr=["すべて","配車マニュアル","チーム運営・規約","野球ルール・スコア","その他"],gr=["すべて","公式・連盟","グラウンド・施設","スケジュール・連絡","便利ツール","その他"];async function Gn({supabaseClient:e,currentUser:t,currentUserRole:n}){Le=e,st=t,Zn=n,Zt=n==="admin"||n==="leader",Oi(),await Mi(),await bi({supabaseClient:Le,currentUser:st,currentUserRole:Zn}),ed(),fr(),br()}function Oi(){document.querySelectorAll(".info-admin-only").forEach(n=>{Zt?n.classList.remove("hidden"):n.classList.add("hidden")});const t=document.getElementById("info-user-role-badge");t&&(Zn==="admin"?(t.className="px-2.5 py-0.5 text-xs font-bold rounded-full bg-purple-100 text-purple-700 border border-purple-200",t.textContent="管理者 (編集可)"):Zn==="leader"?(t.className="px-2.5 py-0.5 text-xs font-bold rounded-full bg-blue-100 text-blue-700 border border-blue-200",t.textContent="リーダー (編集可)"):(t.className="px-2.5 py-0.5 text-xs font-bold rounded-full bg-gray-100 text-gray-700 border border-gray-200",t.textContent="閲覧専用"))}async function Mi(){let e=!1,t=!1,n=!1;if(Le){try{const{data:a,error:s}=await Le.from("info_documents").select("*").order("updated_at",{ascending:!1});!s&&a&&a.length>0&&(Pe=a,e=!0)}catch(a){console.warn("Supabase info_documents select failed:",a)}try{const{data:a,error:s}=await Le.from("info_links").select("*").order("display_order",{ascending:!0});!s&&a&&a.length>0&&(Be=a,t=!0)}catch(a){console.warn("Supabase info_links select failed:",a)}try{const{data:a,error:s}=await Le.from("info_pages").select("*").eq("id","top_announcement").single();!s&&a&&a.content&&(Kt=a.content,n=!0)}catch(a){console.warn("Supabase info_pages select failed:",a)}}if(!e)try{const a=localStorage.getItem(ta);a?Pe=JSON.parse(a):(Pe=[...Os],localStorage.setItem(ta,JSON.stringify(Pe)))}catch{Pe=[...Os]}if(!t)try{const a=localStorage.getItem(na);a?Be=JSON.parse(a):(Be=[...Ms],localStorage.setItem(na,JSON.stringify(Be)))}catch{Be=[...Ms]}if(!n){const a=localStorage.getItem(mr);Kt=a!==null?a:Pi}}async function Ri(e,t=!1){const n=Pe.findIndex(a=>a.id===e.id);n>=0?Pe[n]=e:Pe.unshift(e);try{localStorage.setItem(ta,JSON.stringify(Pe))}catch(a){console.error(a)}if(Le)try{t?await Le.from("info_documents").insert([e]):await Le.from("info_documents").update(e).eq("id",e.id)}catch(a){console.warn("Supabase document save failed:",a)}}async function Fi(e){Pe=Pe.filter(t=>t.id!==e);try{localStorage.setItem(ta,JSON.stringify(Pe))}catch(t){console.error(t)}if(Le)try{await Le.from("info_documents").delete().eq("id",e)}catch(t){console.warn("Supabase document delete failed:",t)}}async function qi(e,t=!1){const n=Be.findIndex(a=>a.id===e.id);n>=0?Be[n]=e:Be.push(e);try{localStorage.setItem(na,JSON.stringify(Be))}catch(a){console.error(a)}if(Le)try{t?await Le.from("info_links").insert([e]):await Le.from("info_links").update(e).eq("id",e.id)}catch(a){console.warn("Supabase link save failed:",a)}}async function ji(e){Be=Be.filter(t=>t.id!==e);try{localStorage.setItem(na,JSON.stringify(Be))}catch(t){console.error(t)}if(Le)try{await Le.from("info_links").delete().eq("id",e)}catch(t){console.warn("Supabase link delete failed:",t)}}async function Hi(e){Kt=e;try{localStorage.setItem(mr,e)}catch(t){console.error(t)}if(Le)try{await Le.from("info_pages").upsert([{id:"top_announcement",content:e,updated_at:new Date().toISOString(),updated_by:(st==null?void 0:st.name)||"管理者"}])}catch(t){console.warn("Supabase top_announcement save failed:",t)}fr()}function fr(){const e=document.getElementById("info-top-announcement-rendered"),t=document.getElementById("info-top-announcement-wrap");!e||!t||(Kt&&Kt.trim()?(t.classList.remove("hidden"),e.innerHTML=ma(Kt)):t.classList.add("hidden"))}function br(){yr(),at==="docs"?On():at==="surveys"?xn():Mn()}function yr(){const e=document.getElementById("info-category-filters");if(!e)return;if(at==="surveys"){e.classList.add("hidden");return}e.classList.remove("hidden");const t=at==="docs"?pr:gr,n=at==="docs"?Xn:ea;e.innerHTML=t.map(a=>`<button class="info-cat-btn px-3.5 py-1.5 text-xs rounded-full transition-all cursor-pointer ${n===a?"bg-teal-600 text-white shadow-sm font-bold":"bg-white text-gray-600 hover:bg-gray-100 border border-gray-200"}" data-category="${a}">${a}</button>`).join(""),e.querySelectorAll(".info-cat-btn").forEach(a=>{a.addEventListener("click",()=>{const s=a.getAttribute("data-category");at==="docs"?(Xn=s,On()):(ea=s,Mn()),yr()})})}function On(){const e=document.getElementById("info-docs-container"),t=document.getElementById("info-docs-empty");if(!e)return;let n=Pe;if(Xn!=="すべて"&&(n=n.filter(a=>a.category===Xn)),kn.trim()){const a=kn.toLowerCase().trim();n=n.filter(s=>s.title&&s.title.toLowerCase().includes(a)||s.summary&&s.summary.toLowerCase().includes(a)||s.content&&s.content.toLowerCase().includes(a)||s.category&&s.category.toLowerCase().includes(a))}if(n.length===0){e.innerHTML="",t&&t.classList.remove("hidden");return}t&&t.classList.add("hidden"),e.innerHTML=n.map(a=>{const s=is(a.category),r=a.files?a.files.length:0;return`
        ...
        `}).join(""),e.querySelectorAll(".btn-view-doc").forEach(a=>{a.addEventListener("click",()=>{const s=a.getAttribute("data-id"),r=Pe.find(o=>o.id===s);r&&Ui(r)})}),Zt&&(e.querySelectorAll(".btn-edit-doc").forEach(a=>{a.addEventListener("click",()=>{const s=a.getAttribute("data-id"),r=Pe.find(o=>o.id===s);r&&hr(r)})}),e.querySelectorAll(".btn-delete-doc").forEach(a=>{a.addEventListener("click",async()=>{const s=a.getAttribute("data-id"),r=Pe.find(o=>o.id===s);r&&confirm(`ドキュメント「${r.title}」を削除しますか？`)&&(await Fi(s),On())})}))}function Mn(){const e=document.getElementById("info-links-container"),t=document.getElementById("info-links-empty");if(!e)return;let n=Be;if(ea!=="すべて"&&(n=n.filter(a=>a.category===ea)),kn.trim()){const a=kn.toLowerCase().trim();n=n.filter(s=>s.title&&s.title.toLowerCase().includes(a)||s.description&&s.description.toLowerCase().includes(a)||s.url&&s.url.toLowerCase().includes(a)||s.category&&s.category.toLowerCase().includes(a))}if(n.length===0){e.innerHTML="",t&&t.classList.remove("hidden");return}t&&t.classList.add("hidden"),e.innerHTML=n.map(a=>{const s=is(a.category),r=a.icon||"🔗",o=a.url?a.url.replace(/^https?:\/\//,"").replace(/\/$/,""):"";return`
            <div class="bg-white rounded-2xl border border-gray-200/80 shadow-sm hover:shadow-md transition-all p-5 flex flex-col justify-between group">
                <div>
                    <div class="flex items-center justify-between gap-2 mb-3">
        ...
        `}).join(""),Zt&&(e.querySelectorAll(".btn-edit-link").forEach(a=>{a.addEventListener("click",()=>{const s=a.getAttribute("data-id"),r=Be.find(o=>o.id===s);r&&xr(r)})}),e.querySelectorAll(".btn-delete-link").forEach(a=>{a.addEventListener("click",async()=>{const s=a.getAttribute("data-id"),r=Be.find(o=>o.id===s);r&&confirm(`リンク「${r.title}」を削除しますか？`)&&(await ji(s),Mn())})}))}function is(e){switch(e){case"配車マニュアル":return"bg-blue-100 text-blue-800 border border-blue-200";case"チーム運営・規約":return"bg-amber-100 text-amber-800 border border-amber-200";case"野球ルール・スコア":return"bg-green-100 text-green-800 border border-green-200";case"公式・連盟":return"bg-purple-100 text-purple-800 border border-purple-200";case"グラウンド・施設":return"bg-emerald-100 text-emerald-800 border border-emerald-200";case"スケジュール・連絡":return"bg-sky-100 text-sky-800 border border-sky-200";case"便利ツール":return"bg-indigo-100 text-indigo-800 border border-indigo-200";default:return"bg-gray-100 text-gray-700 border border-gray-200"}}let aa=null;function Ui(e){const t=document.getElementById("modal-info-doc-reader");if(!t)return;aa=e,document.getElementById("reader-doc-title").textContent=e.title,document.getElementById("reader-doc-category").textContent=e.category||"その他",document.getElementById("reader-doc-category").className=`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${is(e.category)}`,document.getElementById("reader-doc-updated").textContent=`最終更新: ${e.updated_at||"-"} (作成: ${e.created_by||"チーム"})`;const n=document.getElementById("reader-doc-files-container"),a=document.getElementById("reader-doc-files-list");n&&a&&(e.files&&e.files.length>0?(n.classList.remove("hidden"),a.innerHTML=e.files.map(r=>`
            ...
            `).join("")):(n.classList.add("hidden"),a.innerHTML=""));const s=document.getElementById("reader-doc-content");s&&(s.innerHTML=ma(e.content||"",!0),ls(s)),t.classList.remove("hidden")}function Rs(){const e=document.getElementById("modal-info-doc-reader");e&&e.classList.add("hidden"),aa=null}function Vi(){if(!aa)return;const e=aa,t=new Blob([e.content||""],{type:"text/markdown;charset=utf-8;"}),n=document.createElement("a");n.href=URL.createObjectURL(t),n.download=`${e.title||"document"}.md`,n.click()}let sa=null,Qt=[];function hr(e=null){const t=document.getElementById("modal-info-doc-editor");if(!t)return;sa=e?e.id:null,Qt=e&&e.files?JSON.parse(JSON.stringify(e.files)):[],document.getElementById("doc-editor-modal-title").textContent=e?"ドキュメントの編集":"新規ドキュメント作成",document.getElementById("input-doc-title").value=e?e.title:"",document.getElementById("input-doc-summary").value=e&&e.summary||"",document.getElementById("input-doc-content").value=e?e.content:"";const n=document.getElementById("select-doc-category");n.innerHTML=pr.filter(a=>a!=="すべて").map(a=>`<option value="${a}" ${e&&e.category===a?"selected":""}>${a}</option>`).join(""),ds(),Ua("write"),t.classList.remove("hidden")}function Ha(){const e=document.getElementById("modal-info-doc-editor");e&&e.classList.add("hidden"),sa=null,Qt=[]}function ds(){const e=document.getElementById("doc-editor-files-list");if(e){if(Qt.length===0){e.innerHTML='<span class="text-xs text-gray-400">添付ファイルはありません</span>';return}e.innerHTML=Qt.map((t,n)=>`
...
`;function Qi(){const e=document.getElementById("modal-info-sql");if(!e)return;const t=document.getElementById("info-sql-code");t&&(t.textContent=vr),e.classList.remove("hidden")}function Zi(){const e=document.getElementById("modal-info-sql");e&&e.classList.add("hidden")}function Xi(){navigator.clipboard.writeText(vr).then(()=>{alert(`SQL文をクリップボードにコピーしました！
Supabase Dashboardの「SQL Editor」で実行してください。`)}).catch(e=>{console.error("Copy failed:",e)})}let Fs=!1;function ed(){var u,f,b,y,p,g,v,x,h,k,L,w,I,E,$,_,C,P,D,O;if(Fs)return;Fs=!0;const e=document.getElementById("tab-info-docs"),t=document.getElementById("tab-info-surveys"),n=document.getElementById("tab-info-links"),a=document.getElementById("view-info-docs"),s=document.getElementById("view-info-surveys"),r=document.getElementById("view-info-links"),o=document.getElementById("btn-info-new"),i=document.getElementById("btn-survey-new");function d(q){at=q,[e,t,n].forEach(B=>{B&&(B.className="px-4 py-2 font-bold text-gray-500 hover:text-gray-700 border-b-2 border-transparent transition-colors flex items-center space-x-2 cursor-pointer")}),[a,s,r].forEach(B=>B==null?void 0:B.classList.add("hidden")),q==="docs"?(e.className="px-4 py-2 font-bold text-teal-600 border-b-2 border-teal-600 transition-colors flex items-center space-x-2 cursor-pointer",a==null||a.classList.remove("hidden"),o&&(o.classList.remove("hidden"),o.textContent="＋ 新規ドキュメント"),i&&i.classList.add("hidden")):q==="surveys"?(t.className="px-4 py-2 font-bold text-teal-600 border-b-2 border-teal-600 transition-colors flex items-center space-x-2 cursor-pointer",s==null||s.classList.remove("hidden"),o&&o.classList.add("hidden"),i&&Zt&&i.classList.remove("hidden")):(n.className="px-4 py-2 font-bold text-teal-600 border-b-2 border-teal-600 transition-colors flex items-center space-x-2 cursor-pointer",r==null||r.classList.remove("hidden"),o&&(o.classList.remove("hidden"),o.textContent="＋ 新規リンク"),i&&i.classList.add("hidden")),br()}e==null||e.addEventListener("click",()=>d("docs")),t==null||t.addEventListener("click",()=>d("surveys")),n==null||n.addEventListener("click",()=>d("links"));const l=document.getElementById("input-info-search");l==null||l.addEventListener("input",q=>{kn=q.target.value,at==="docs"?On():at==="links"&&Mn()}),o==null||o.addEventListener("click",()=>{at==="docs"?hr():at==="links"&&xr()}),(u=document.getElementById("btn-edit-top-announcement"))==null||u.addEventListener("click",Ji),(f=document.getElementById("btn-close-top-editor"))==null||f.addEventListener("click",Va),(b=document.getElementById("btn-cancel-top-editor"))==null||b.addEventListener("click",Va),(y=document.getElementById("btn-top-tab-write"))==null||y.addEventListener("click",()=>Ga("write")),(p=document.getElementById("btn-top-tab-preview"))==null||p.addEventListener("click",()=>Ga("preview")),(g=document.getElementById("btn-save-top-announcement"))==null||g.addEventListener("click",Yi),(v=document.getElementById("btn-close-doc-reader"))==null||v.addEventListener("click",Rs),(x=document.getElementById("btn-close-doc-reader-bg"))==null||x.addEventListener("click",Rs),(h=document.getElementById("btn-export-doc-md"))==null||h.addEventListener("click",Vi),(k=document.getElementById("btn-close-doc-editor"))==null||k.addEventListener("click",Ha),(L=document.getElementById("btn-cancel-doc-editor"))==null||L.addEventListener("click",Ha),(w=document.getElementById("btn-doc-tab-write"))==null||w.addEventListener("click",()=>Ua("write")),(I=document.getElementById("btn-doc-tab-preview"))==null||I.addEventListener("click",()=>Ua("preview")),(E=document.getElementById("btn-save-doc"))==null||E.addEventListener("click",zi);const c=document.getElementById("input-doc-attach-files");c==null||c.addEventListener("change",q=>{q.target.files&&Gi(q.target.files)});const m=document.getElementById("input-doc-import-md");m==null||m.addEventListener("change",q=>{q.target.files&&q.target.files[0]&&Wi(q.target.files[0])}),($=document.getElementById("btn-close-link-editor"))==null||$.addEventListener("click",Wa),(_=document.getElementById("btn-cancel-link-editor"))==null||_.addEventListener("click",Wa),(C=document.getElementById("btn-save-link"))==null||C.addEventListener("click",Ki),(P=document.getElementById("btn-info-sql-modal"))==null||P.addEventListener("click",Qi),(D=document.getElementById("btn-close-info-sql"))==null||D.addEventListener("click",Zi),(O=document.getElementById("btn-copy-info-sql"))==null||O.addEventListener("click",Xi)}function ma(e,t=!1){if(!e)return"";const n=e.split(`
`);let a="",s=!1,r=!1,o=!1,i="",d=!1,l=[],c=!1,m="",u="";const f=[];function b(){s&&(a+=`</ul>
`,s=!1),r&&(a+=`</ol>
`,r=!1),d&&(a+=qs(l),l=[],d=!1),c&&(a+=td(m,u),u="",c=!1)}for(let y=0;y<n.length;y++){let p=n[y];if(p.trim().startsWith("```")){o?(a+=`
...
`;continue}const v=p.trim().match(/^[-*]\s*\[([ xX])\]\s*(.*)/);if(v){b();const h=v[1].toLowerCase()==="x";a+=`
                <div class="flex items-center gap-2 my-1 text-xs text-gray-700">
                    ...
                    <span class="${h?"line-through text-gray-400":""}">${Ke(v[2])}</span>
                </div>
...
Vercel等の設定を確認してください。`),console.error("Missing SUPABASE_URL or SUPABASE_ANON_KEY");console.log("Checking Supabase URL:","Empty");!window.supabaseClient&&window.supabase&&window.supabase.createClient&&(window.supabaseClient=window.supabase.createClient(cs,us));const S=window.supabaseClient;let V=null,gn=!1,he="user";function pa(e){return e=e.trim(),e?e.includes("@")?e:`${e}@ants.local`:""}function Xt(e){return e?e.endsWith("@ants.local")?e.split("@")[0]:e:""}let un=0,tt=null;function nd(e){const t=document.getElementById("loading-overlay");if(t){const n=t.querySelector("span");n&&(n.textContent=e)}tt||(tt=document.createElement("div"),tt.id="loading-detail-text",tt.className="fixed bottom-2 right-2 text-xs md:text-sm font-bold text-gray-600 bg-white/90 border border-gray-300 px-3 py-1.5 rounded shadow-lg pointer-events-none z-[9999] transition-opacity duration-300",document.body.appendChild(tt)),tt.textContent=e,tt.style.opacity="1"}function U(e="通信中..."){un++;const t=document.getElementById("loading-overlay");t&&un===1&&t.classList.remove("hidden"),nd(e)}function F(){var e;un--,un<=0&&(un=0,(e=document.getElementById("loading-overlay"))==null||e.classList.add("hidden"),tt&&(tt.style.opacity="0"))}function qe(){var e;un=0,(e=document.getElementById("loading-overlay"))==null||e.classList.add("hidden"),tt&&(tt.style.opacity="0")}async function ye(e,t="通信中..."){{alert("環境変数 (VITE_SUPABASE_URL) が設定されていません。Vercelの設定を確認してください。");return}}async function ee(e,t){if(V&&e!=="NAVIGATE")try{await S.from("action_logs").insert({user_email:V.email,action_type:e,details:t})}catch(n){console.error("Log error:",n)}}function js(){var e,t,n,a,s,r,o,i,d,l,c,m,u,f,b,y,p,g,v,x,h,k,L,w,I,E,$,_,C,P,D,O,q,B,A,N,T,M,W,j,H,R,z,le,te,be;if(!window.isDomInitialized)try{if(window.isDomInitialized=!0,document.title="bb-sys for arinko ants.",document.body){const G=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT,null,!1);let ce;for(;ce=G.nextNode();)ce.nodeValue.includes("配車調整アプリ")&&(ce.nodeValue=ce.nodeValue.replace(/配車調整アプリ/g,"bb-sys for arinko ants.")),ce.nodeValue.includes("少年野球に役立つツール for arinko ants.")&&(ce.nodeValue=ce.nodeValue.replace(/少年野球に役立つツール for arinko ants./g,"bb-sys for arinko ants."))}document.documentElement.style.setProperty("--layout-columns",Ya),window.addEventListener("resize",Us),Us(),(e=document.getElementById("btn-login"))==null||e.addEventListener("click",Hs),(t=document.getElementById("auth-form"))==null||t.addEventListener("submit",Hs),(n=document.getElementById("btn-logout"))==null||n.addEventListener("click",jn),(a=document.getElementById("btn-logout-menu"))==null||a.addEventListener("click",jn),(s=document.getElementById("btn-clear-cache"))==null||s.addEventListener("click",ad),(r=document.getElementById("link-to-signup"))==null||r.addEventListener("click",G=>{G.preventDefault(),Y("signup-view")}),(o=document.getElementById("link-to-reset"))==null||o.addEventListener("click",G=>{G.preventDefault(),Y("password-reset-view")}),document.querySelectorAll(".link-back-to-login").forEach(G=>G.addEventListener("click",ce=>{ce.preventDefault(),Y("auth-view")})),(i=document.getElementById("btn-submit-signup"))==null||i.addEventListener("click",sd),(d=document.getElementById("btn-send-reset"))==null||d.addEventListener("click",rd),(l=document.getElementById("btn-update-password"))==null||l.addEventListener("click",od),(c=document.getElementById("btn-change-password"))==null||c.addEventListener("click",()=>{var G;(G=document.getElementById("change-password-modal"))==null||G.classList.remove("hidden")}),(m=document.getElementById("btn-close-change-password"))==null||m.addEventListener("click",()=>{var G;(G=document.getElementById("change-password-modal"))==null||G.classList.add("hidden")}),(u=document.getElementById("btn-submit-change-password"))==null||u.addEventListener("click",id);const Ee=document.getElementById("btn-change-password");Ee&&(Ee.textContent="パスワード変更"),(f=document.getElementById("nav-users"))==null||f.addEventListener("click",Ln),(b=document.getElementById("btn-admin-add-user"))==null||b.addEventListener("click",adminAddUser),(y=document.getElementById("btn-reload-users"))==null||y.addEventListener("click",ae),(p=document.getElementById("btn-save-all-users"))==null||p.addEventListener("click",dd),(g=document.getElementById("admin-users-sort"))==null||g.addEventListener("change",G=>{localStorage.setItem("admin_users_sort",G.target.value),ae()}),(v=document.getElementById("btn-export-users"))==null||v.addEventListener("click",Kd),(x=document.getElementById("btn-import-users"))==null||x.addEventListener("click",()=>document.getElementById("input-import-users-csv").click()),(h=document.getElementById("btn-download-users-sample"))==null||h.addEventListener("click",el),(k=document.getElementById("input-import-users-csv"))==null||k.addEventListener("change",Qd),(L=document.getElementById("btn-close-csv-modal"))==null||L.addEventListener("click",ia),(w=document.getElementById("btn-close-csv-modal-x"))==null||w.addEventListener("click",ia),(I=document.getElementById("btn-execute-csv-import"))==null||I.addEventListener("click",Zd);const Lt=document.getElementById("btn-admin-add-user");if(Lt&&!document.getElementById("btn-admin-add-dummy-user")){const G=document.createElement("button");G.id="btn-admin-add-dummy-user",G.className="ml-2 bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded shadow font-bold text-sm",G.textContent="代行専用メンバー追加",G.onclick=adminAddDummyUser,(E=Lt.parentNode)==null||E.appendChild(G)}const dt=document.querySelector("#view-master #btn-load-logs");dt&&dt.remove();const ne=document.querySelector("#view-master #log-list");if(ne){const G=ne.closest(".bg-white");G?G.remove():ne.remove()}document.querySelectorAll("#view-master h2").forEach(G=>{G.textContent.includes("操作ログ")&&G.remove()});const Je=document.getElementById("tab-master-admin");if(Je&&!document.getElementById("tab-logs-admin")){const G=Je.parentElement,ce=document.createElement("button");ce.id="tab-logs-admin",ce.className="px-4 py-2 font-bold text-gray-500 hover:text-gray-700 border-b-2 border-transparent transition-colors",ce.textContent="操作ログ",G==null||G.appendChild(ce);const Re=document.createElement("div");Re.id="tab-content-logs-admin",Re.className="hidden",Re.innerHTML=`
            ...
            `;const Ge=document.getElementById("tab-content-master-admin");Ge&&Ge.parentElement&&Ge.parentElement.appendChild(Re)}const Ce=G=>{[{btnId:"tab-users-admin",contentId:"tab-content-users-admin"},{btnId:"tab-master-admin",contentId:"tab-content-master-admin"},{btnId:"tab-logs-admin",contentId:"tab-content-logs-admin"}].forEach(Re=>{const Ge=document.getElementById(Re.btnId),Ut=document.getElementById(Re.contentId);!Ge||!Ut||(Re.btnId===G?(Ge.className="px-4 py-2 font-bold text-blue-600 border-b-2 border-blue-600 transition-colors",Ut.classList.remove("hidden"),G==="tab-logs-admin"&&Sn()):(Ge.className="px-4 py-2 font-bold text-gray-500 hover:text-gray-700 border-b-2 border-transparent transition-colors",Ut.classList.add("hidden")))})};($=document.getElementById("tab-users-admin"))==null||$.addEventListener("click",()=>Ce("tab-users-admin")),(_=document.getElementById("tab-master-admin"))==null||_.addEventListener("click",()=>Ce("tab-master-admin")),(C=document.getElementById("tab-logs-admin"))==null||C.addEventListener("click",()=>Ce("tab-logs-admin")),(P=document.getElementById("btn-load-logs"))==null||P.addEventListener("click",Sn),(D=document.getElementById("btn-app-dispatch"))==null||D.addEventListener("click",()=>{var G;Y("app-view","dispatch"),gn?(G=document.getElementById("nav-dispatch"))==null||G.click():Ja()}),(O=document.getElementById("btn-app-attendance"))==null||O.addEventListener("click",async()=>{Y("attendance-view"),await ye(Aa,"出欠管理画面を準備中...")}),(q=document.getElementById("btn-app-dashboard"))==null||q.addEventListener("click",async()=>{await ye(Na,"ダッシュボードを準備中..."),Y("dashboard-view")}),(B=document.getElementById("btn-app-info"))==null||B.addEventListener("click",async()=>{await ye(async()=>{await Gn({supabaseClient:S,currentUser:V,currentUserRole:he})},"Info画面を準備中..."),Y("info-view")}),(A=document.getElementById("btn-back-to-menu-info"))==null||A.addEventListener("click",()=>Y("app-menu-view")),(N=document.getElementById("btn-logout-info"))==null||N.addEventListener("click",jn),(T=document.getElementById("btn-back-to-menu"))==null||T.addEventListener("click",()=>{Y("app-menu-view")}),(M=document.getElementById("btn-back-to-menu-att"))==null||M.addEventListener("click",()=>Y("app-menu-view")),(W=document.getElementById("btn-logout-att"))==null||W.addEventListener("click",jn),(j=document.getElementById("btn-change-password-menu"))==null||j.addEventListener("click",ms);const ut=(H=document.getElementById("app-menu-view"))==null?void 0:H.querySelector(".space-y-4");ut&&(ut.className="w-full max-w-xs mx-auto space-y-3 mt-4");const mt=document.getElementById("btn-app-dispatch");mt&&(mt.className="flex items-center justify-between w-full px-5 py-3.5 rounded-xl shadow-md transition duration-200 font-bold bg-blue-600 hover:bg-blue-700 hover:shadow-lg text-white text-left",mt.innerHTML='<div class="flex items-center space-x-3 text-base sm:text-lg"><span class="text-2xl">🚗</span><span>配車調整</span></div><span class="text-white/60 text-sm font-normal">❯</span>');const lt=document.getElementById("btn-app-attendance");lt&&(lt.className="flex items-center justify-between w-full px-5 py-3.5 rounded-xl shadow-md transition duration-200 font-bold bg-green-600 hover:bg-green-700 hover:shadow-lg text-white text-left",lt.innerHTML='<div class="flex items-center space-x-3 text-base sm:text-lg"><span class="text-2xl">📅</span><span>出欠管理</span></div><span class="text-white/60 text-sm font-normal">❯</span>');const Ye=document.getElementById("btn-app-simulator");Ye&&(Ye.className="flex items-center justify-between w-full px-5 py-3.5 rounded-xl shadow-md transition duration-200 font-bold bg-amber-600 hover:bg-amber-700 hover:shadow-lg text-white text-left",Ye.innerHTML='<div class="flex items-center space-x-3 text-base sm:text-lg"><span class="text-2xl">⚾</span><span>ポジション設定</span></div><span class="text-white/60 text-sm font-normal">❯</span>',Ye.onclick=()=>{Y("position-simulator-view"),Vn()});const De=document.getElementById("btn-app-info");De&&(De.className="flex items-center justify-between w-full px-5 py-3.5 rounded-xl shadow-md transition duration-200 font-bold bg-teal-600 hover:bg-teal-700 hover:shadow-lg text-white text-left",De.innerHTML='<div class="flex items-center space-x-3 text-base sm:text-lg"><span class="text-2xl">ℹ️</span><span>Info</span></div><span class="text-white/60 text-sm font-normal">❯</span>',De.onclick=async()=>{await ye(async()=>{await Gn({supabaseClient:S,currentUser:V,currentUserRole:he})},"Info画面を準備中..."),Y("info-view")}),(R=document.getElementById("nav-info"))==null||R.addEventListener("click",async()=>{await ye(async()=>{await Gn({supabaseClient:S,currentUser:V,currentUserRole:he})},"Info画面を準備中..."),Y("info-view")}),(z=document.getElementById("nav-simulator"))==null||z.addEventListener("click",()=>{Y("position-simulator-view"),Vn()}),(le=document.getElementById("nav-simulator-att"))==null||le.addEventListener("click",()=>{Y("position-simulator-view"),Vn()}),(te=document.getElementById("nav-dispatch-sim"))==null||te.addEventListener("click",()=>{var G;Y("app-view","dispatch"),gn?(G=document.getElementById("nav-dispatch"))==null||G.click():Ja()}),(be=document.getElementById("nav-attendance-sim"))==null||be.addEventListener("click",async()=>{Y("attendance-view"),await ye(Aa,"出欠管理画面を準備中...")})}catch(Ee){console.error("DOM Initialization failed:",Ee),qe(),Y("auth-view")}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",js):setTimeout(js,0);let za=!1;function ra(e,t=null){if(za)return;const n="#"+e+(t?"-"+t:"");window.location.hash!==n&&history.pushState({screenId:e,subView:t},"",n)}function Y(e,t=null){["auth-view","signup-view","password-reset-view","password-update-view","app-menu-view","app-view","attendance-view","view-users","dashboard-view","dashboard-settings","position-simulator-view","info-view","survey-respond-view"].forEach(a=>{const s=document.getElementById(a);s&&s.classList.add("hidden")});const n=document.getElementById(e);n&&n.classList.remove("hidden"),ra(e,t),e!=="auth-view"&&e!=="survey-respond-view"&&V&&ee("NAVIGATE",`画面遷移: ${e}${t?" > "+t:""}`)}function Wn(){const e=new URLSearchParams(window.location.search);let t=e.get("survey"),n=e.get("response"),a=window.location.hash||"";if(a.startsWith("#/")&&(a="#"+a.substring(2)),a.startsWith("#survey-")||a.startsWith("#survey=")){const r=(a.startsWith("#survey-")?a.replace("#survey-",""):a.replace("#survey=","")).split("&");t=r[0];for(let o=1;o<r.length;o++){const[i,d]=r[o].split("=");i==="response"&&d&&(n=d)}}return t&&(t=decodeURIComponent(t).trim().replace(/^\/+|\/+$/g,"")),n&&(n=decodeURIComponent(n).trim()),{surveyId:t,responseId:n}}window.addEventListener("popstate",async e=>{var t,n;za=!0;try{const{surveyId:a,responseId:s}=Wn();if(a){await cn(a,s);return}e.state&&e.state.screenId?(Y(e.state.screenId,e.state.subView),e.state.screenId==="app-view"&&(e.state.subView==="users"?await Ln():e.state.subView==="master"?(t=document.getElementById("nav-master"))==null||t.click():(n=document.getElementById("nav-dispatch"))==null||n.click())):Y(V?"app-menu-view":"auth-view")}finally{za=!1}});S&&S.auth.onAuthStateChange(async(e,t)=>{if(e==="PASSWORD_RECOVERY"){Y("password-update-view");return}if(t&&gn&&V&&(e==="TOKEN_REFRESHED"||e==="USER_UPDATED")){console.log(`Bypassing auth state change handling for event: ${e}`);return}const n=async()=>{var r,o,i,d,l,c,m,u;const{surveyId:a,responseId:s}=Wn();if(a){if(qe(),t){V={...t.user};try{const{data:f}=await S.from("app_users").select("name, role").eq("email",V.email).single();f&&(V.name=f.name,he=f.role)}catch{}}await cn(a,s);return}if(t){U("ユーザー権限確認中...");try{const f=!V||V.id!==t.user.id;V={...t.user},f&&await ee("LOGIN","ログインしました");let b=!0,y=!0,p=!0,g=!0,v=!0;try{const{data:H}=await S.from("app_users").select("role, name, can_use_dispatch, can_use_dashboard, can_use_attendance, can_use_simulator, can_use_info").eq("email",V.email).single();H?(he=H.role,V.name=H.name,H.can_use_dispatch===!1&&(b=!1),H.can_use_dashboard===!1&&(y=!1),H.can_use_attendance===!1&&(p=!1),H.can_use_simulator===!1&&(g=!1),H.can_use_info===!1&&(v=!1)):he="user"}catch{he="user"}V.email==="hishinumak@gmail.com"&&(he="admin"),he==="admin"&&(b=!0,y=!0,p=!0,g=!0,v=!0);let x=document.getElementById("btn-app-dashboard");if(y)if(x)x.classList.remove("hidden");else{const H=((r=document.getElementById("app-menu-view"))==null?void 0:r.querySelector(".space-y-3"))||((o=document.getElementById("app-menu-view"))==null?void 0:o.querySelector(".space-y-4"))||((i=document.getElementById("app-menu-view"))==null?void 0:i.querySelector(".grid"));H&&(x=document.createElement("button"),x.id="btn-app-dashboard",x.className="flex items-center justify-between w-full px-5 py-3.5 rounded-xl shadow-md transition duration-200 font-bold bg-indigo-600 hover:bg-indigo-700 hover:shadow-lg text-white text-left",x.innerHTML='<div class="flex items-center space-x-3 text-base sm:text-lg"><span class="text-2xl">📊</span><span>分析</span></div><span class="text-white/60 text-sm font-normal">❯</span>',x.onclick=async()=>{await ye(Na,"ダッシュボードを準備中..."),Y("dashboard-view")},H.appendChild(x))}else x&&x.classList.add("hidden");const h=document.getElementById("nav-users"),k=document.getElementById("nav-master"),L=document.getElementById("nav-dispatch"),w=document.getElementById("btn-app-dispatch"),I=document.getElementById("btn-goto-master"),E=document.getElementById("clear-db-button");let $=document.getElementById("btn-app-users-admin");if(h==null||h.classList.add("hidden"),he==="admin"?(L==null||L.classList.remove("hidden"),k==null||k.classList.remove("hidden")):he==="leader"?(L==null||L.classList.remove("hidden"),k==null||k.classList.add("hidden")):(L==null||L.classList.add("hidden"),k==null||k.classList.add("hidden")),he==="admin")if(I==null||I.classList.remove("hidden"),E==null||E.classList.remove("hidden"),$)$.classList.remove("hidden");else{const H=((d=document.getElementById("app-menu-view"))==null?void 0:d.querySelector(".space-y-3"))||((l=document.getElementById("app-menu-view"))==null?void 0:l.querySelector(".space-y-4"))||((c=document.getElementById("app-menu-view"))==null?void 0:c.querySelector(".grid"));H&&($=document.createElement("button"),$.id="btn-app-users-admin",$.className="flex items-center justify-between w-full px-5 py-3.5 rounded-xl shadow-md transition duration-200 font-bold bg-purple-600 hover:bg-purple-700 hover:shadow-lg text-white text-left order-last",$.innerHTML='<div class="flex items-center space-x-3 text-base sm:text-lg"><span class="text-2xl">⚙️</span><span>管理者メニュー</span></div><span class="text-white/60 text-sm font-normal">❯</span>',$.onclick=()=>wr(),H.appendChild($))}else I==null||I.classList.add("hidden"),E==null||E.classList.add("hidden"),$&&$.classList.add("hidden");b?w==null||w.classList.remove("hidden"):w==null||w.classList.add("hidden");const _=document.getElementById("btn-app-attendance");p?_==null||_.classList.remove("hidden"):_==null||_.classList.add("hidden");const C=document.getElementById("btn-app-simulator"),P=document.getElementById("nav-simulator"),D=document.getElementById("nav-simulator-att"),O=document.getElementById("nav-dispatch-sim"),q=document.getElementById("nav-attendance-sim");g?(C==null||C.classList.remove("hidden"),P==null||P.classList.remove("hidden"),D==null||D.classList.remove("hidden")):(C==null||C.classList.add("hidden"),P==null||P.classList.add("hidden"),D==null||D.classList.add("hidden")),b?O==null||O.classList.remove("hidden"):O==null||O.classList.add("hidden"),p?q==null||q.classList.remove("hidden"):q==null||q.classList.add("hidden");const B=document.getElementById("btn-app-info"),A=document.getElementById("nav-info");v?(B==null||B.classList.remove("hidden"),A==null||A.classList.remove("hidden")):(B==null||B.classList.add("hidden"),A==null||A.classList.add("hidden"));const N=document.getElementById("user-email-display");N&&(N.textContent=V.name||Xt(V.email));const T=document.getElementById("sim-user-email-display");T&&(T.textContent=V.name||Xt(V.email));const{surveyId:M,responseId:W}=Wn();if(M){await cn(M,W);return}const j=window.location.hash;if(j&&j!=="#app-menu-view"&&j!=="#auth-view"){const H=["app-view","attendance-view","dashboard-view","position-simulator-view","info-view"];let R=!1;for(const z of H)if(j.startsWith("#"+z)){const le=j.length>z.length+1?j.substring(z.length+2):null;z==="app-view"?(Y("app-view",le),gn?le==="users"?Ln():le==="master"?(m=document.getElementById("nav-master"))==null||m.click():(u=document.getElementById("nav-dispatch"))==null||u.click():Ja().then(()=>{var te,be;le==="users"?Ln():le==="master"?(te=document.getElementById("nav-master"))==null||te.click():(be=document.getElementById("nav-dispatch"))==null||be.click()}).catch(te=>{console.error("App init error:",te),qe()})):z==="attendance-view"?(Y("attendance-view"),Aa().catch(te=>{console.error("Attendance init error:",te),qe()})):z==="dashboard-view"?(Y("dashboard-view"),Na().catch(te=>{console.error("Dashboard init error:",te),qe()})):z==="position-simulator-view"?(Y("position-simulator-view"),Vn()):z==="info-view"&&(Y("info-view"),Gn({supabaseClient:S,currentUser:V,currentUserRole:he}).catch(te=>{console.error("Info init error:",te),qe()})),R=!0;break}R||Y("app-menu-view")}else Y("app-menu-view")}catch(f){console.error("Auth state handling error:",f),qe()}finally{F()}}else{V=null,qe();const{surveyId:f,responseId:b}=Wn();if(f){await cn(f,b);return}const y=document.getElementById("password-update-view");(!y||y.classList.contains("hidden"))&&Y("auth-view")}};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>{n().catch(console.error)}):setTimeout(()=>{n().catch(console.error)},0)});async function ad(e){if(e&&e.preventDefault&&e.preventDefault(),!!confirm(`ブラウザに保存されているログイン情報（キャッシュ）をクリアして、ページを再読み込みしますか？
（動作がおかしい・ログインできない場合にお試しください）`)){U("キャッシュクリア中...");try{await S.auth.signOut().catch(()=>{});const t=[];for(let n=0;n<localStorage.length;n++){const a=localStorage.key(n);a&&a.startsWith("sb-")&&t.push(a)}t.forEach(n=>localStorage.removeItem(n)),alert("キャッシュをクリアしました。ページを再読み込みします。"),location.reload()}catch{qe()}}}let $a=!1;async function Hs(e){var r,o;if(e&&e.preventDefault&&e.preventDefault(),$a)return;const t=((r=document.getElementById("email-address"))==null?void 0:r.value)||"",n=pa(t),a=((o=document.getElementById("password"))==null?void 0:o.value)||"",s=document.getElementById("auth-message");if(s&&s.classList.add("hidden"),!(!n||!a)){$a=!0,U("ログイン認証中...");try{const{error:i}=await S.auth.signInWithPassword({email:n,password:a});i&&(s?(s.textContent="ログイン失敗: "+i.message,s.classList.remove("hidden"),s.classList.add("text-red-500")):alert("ログイン失敗: "+i.message))}catch(i){console.error("Login Error:",i);const d=i.message+`
...
`+d)}finally{F(),$a=!1}}}async function wr(){Y("app-view","users"),await Ln()}function ms(){const e=document.getElementById("change-password-modal");e&&(document.body.appendChild(e),e.classList.remove("hidden"),e.style.zIndex="9999")}let ka=!1;async function sd(e){var i,d,l,c;if(e&&e.preventDefault&&e.preventDefault(),ka)return;const t=((i=document.getElementById("signup-parent-name"))==null?void 0:i.value)||"",n=((d=document.getElementById("signup-player-name"))==null?void 0:d.value)||"",a=((l=document.getElementById("signup-email"))==null?void 0:l.value)||"";let s=((c=document.getElementById("signup-password"))==null?void 0:c.value)||"";const r=document.getElementById("signup-message");r&&r.classList.add("hidden");const o=document.getElementById("signup-password")!==null;if(o||(s=Math.random().toString(36).slice(-10)+"A1!"),!t||!n||!a||o&&s.length<6){r&&(r.textContent=o?"すべての項目を正しく入力してください(パスワードは6文字以上)":"すべての項目を正しく入力してください"),r&&r.classList.remove("hidden","text-green-600"),r&&r.classList.add("text-red-500");return}ka=!0,U("利用申請を送信中...");try{const{data:m,error:u}=await S.auth.signUp({email:a,password:s});if(u){r&&(r.textContent="登録エラー: "+u.message),r&&r.classList.remove("hidden","text-green-600"),r&&r.classList.add("text-red-500");return}const{error:f}=await S.from("signup_requests").insert([{parent_name:t,player_name:n,email:a,status:"pending"}]);await S.auth.signOut(),f?(r&&(r.textContent="申請失敗: "+f.message),r&&r.classList.remove("hidden","text-green-600"),r&&r.classList.add("text-red-500")):(r&&(r.textContent=o?"アカウントが作成され、利用申請が送信されました。管理者の承認をお待ちください。":"利用申請が送信されました。管理者の承認後、「パスワードを忘れた場合」からパスワードを再設定してログインしてください。"),r&&r.classList.remove("text-red-500"),r&&r.classList.add("text-green-600"),r&&r.classList.remove("hidden"),document.getElementById("signup-parent-name")&&(document.getElementById("signup-parent-name").value=""),document.getElementById("signup-player-name")&&(document.getElementById("signup-player-name").value=""),document.getElementById("signup-email")&&(document.getElementById("signup-email").value=""),document.getElementById("signup-password")&&(document.getElementById("signup-password").value=""))}catch(m){qe(),console.error("Signup Error:",m),r&&(r.textContent="登録処理中にエラーが発生しました。",r.classList.remove("hidden","text-green-600"),r.classList.add("text-red-500"))}finally{F(),ka=!1}}async function rd(e){var a;e&&e.preventDefault&&e.preventDefault();const t=((a=document.getElementById("reset-email"))==null?void 0:a.value)||"",n=document.getElementById("reset-message");if(n&&n.classList.add("hidden"),!!t){U("パスワード再設定メール送信中...");try{const{error:s}=await S.auth.resetPasswordForEmail(t,{redirectTo:window.location.origin});s?n?(n.textContent="送信失敗: "+s.message,n.classList.remove("hidden","text-green-600"),n.classList.add("text-red-500")):alert("送信失敗: "+s.message):n?(n.textContent="パスワード再設定メールを送信しました。",n.classList.remove("text-red-500"),n.classList.add("text-green-600"),n.classList.remove("hidden")):alert("パスワード再設定メールを送信しました。")}catch(s){console.error("Password reset error:",s)}finally{F()}}}async function od(e){var a;e&&e.preventDefault&&e.preventDefault();const t=((a=document.getElementById("new-password"))==null?void 0:a.value)||"",n=document.getElementById("update-password-message");if(n&&n.classList.add("hidden"),!t||t.length<6){n?(n.textContent="6文字以上のパスワードを入力してください",n.classList.remove("hidden","text-green-600"),n.classList.add("text-red-500")):alert("6文字以上のパスワードを入力してください");return}U("パスワード更新中...");try{const{error:s}=await S.auth.updateUser({password:t});s?n?(n.textContent="更新失敗: "+s.message,n.classList.remove("hidden","text-green-600"),n.classList.add("text-red-500")):alert("更新失敗: "+s.message):(alert("パスワードが更新されました。再度ログインしてください。"),Y("auth-view"))}catch(s){console.error("Password update error:",s)}finally{F()}}async function id(e){var n;e&&e.preventDefault&&e.preventDefault();const t=((n=document.getElementById("change-new-password"))==null?void 0:n.value)||"";if(!t||t.length<6)return alert("6文字以上のパスワードを入力してください");U("パスワード変更中...");try{const{error:a}=await S.auth.updateUser({password:t});if(a)alert("更新失敗: "+a.message);else{alert("パスワードが変更されました。");const s=document.getElementById("change-password-modal");s&&s.classList.add("hidden");const r=document.getElementById("change-new-password");r&&(r.value="")}}catch(a){console.error("Password change error:",a)}finally{F()}}async function jn(e){e&&e.preventDefault&&e.preventDefault(),V&&await ee("LOGOUT","ログアウトしました"),U("ログアウト処理中...");try{await S.auth.signOut().catch(()=>{})}finally{F(),gn=!1,history.pushState(null,"",window.location.pathname)}}function zn(e){const t=document.getElementById("nav-dispatch"),n=document.getElementById("nav-master"),a=document.getElementById("nav-users");e==="users"?(t==null||t.classList.add("hidden"),n==null||n.classList.add("hidden"),a==null||a.classList.add("hidden")):(a==null||a.classList.add("hidden"),he==="admin"?(t==null||t.classList.remove("hidden"),n==null||n.classList.remove("hidden")):he==="leader"?(t==null||t.classList.remove("hidden"),n==null||n.classList.add("hidden")):(t==null||t.classList.add("hidden"),n==null||n.classList.add("hidden")))}let re=[],oe=[],Fe=[],In=new Set;async function Ja(){var e,t,n;gn=!0,zn("dispatch"),Ie==null||Ie.addEventListener("click",async()=>{var a,s;an==null||an.classList.remove("hidden"),sn==null||sn.classList.add("hidden"),(a=document.getElementById("view-users"))==null||a.classList.add("hidden"),zn("dispatch"),Ie==null||Ie.classList.add("text-blue-300"),Ie==null||Ie.classList.remove("text-gray-400"),$e==null||$e.classList.remove("text-blue-300"),$e==null||$e.classList.add("text-gray-400"),(s=document.getElementById("nav-users"))==null||s.classList.remove("text-blue-300"),ra("app-view","dispatch"),await Vs()}),$e==null||$e.addEventListener("click",async()=>{var a,s;sn==null||sn.classList.remove("hidden"),an==null||an.classList.add("hidden"),(a=document.getElementById("view-users"))==null||a.classList.add("hidden"),zn("master"),$e==null||$e.classList.add("text-blue-300"),$e==null||$e.classList.remove("text-gray-400"),Ie==null||Ie.classList.remove("text-blue-300"),Ie==null||Ie.classList.add("text-gray-400"),(s=document.getElementById("nav-users"))==null||s.classList.remove("text-blue-300"),ra("app-view","master"),await md(),Sn()}),(e=document.getElementById("btn-goto-master"))==null||e.addEventListener("click",()=>{$e==null||$e.click()}),(t=document.getElementById("btn-back-to-dispatch"))==null||t.addEventListener("click",()=>{Ie==null||Ie.click()});try{await J.initMasterData(),re.length===0&&oe.length===0&&(await J.bulkAddFamilies(cd),await J.bulkAddCars(ud),await J.syncMaster(),await J.initMasterData()),await Vs(),gd(),Pd(),(n=document.getElementById("btn-load-logs"))==null||n.addEventListener("click",Sn)}catch(a){console.error(a),Ve("データの読み込みに失敗しました。","error")}}async function Ln(){he==="admin"&&(document.getElementById("view-users").classList.remove("hidden"),document.getElementById("view-master").classList.add("hidden"),document.getElementById("view-dispatch").classList.add("hidden"),zn("users"),document.getElementById("nav-users").classList.add("text-blue-300"),document.getElementById("nav-users").classList.remove("text-gray-400"),document.getElementById("nav-dispatch").classList.remove("text-blue-300"),document.getElementById("nav-master").classList.remove("text-blue-300"),ra("app-view","users"),await ae())}async function ae(){U("メンバー・マスタ情報読み込み中...");try{const{data:e}=await S.from("app_users").select("*").order("created_at",{ascending:!1});let t=[],n=[],a=[],s=[],r=[];try{const{data:g}=await S.from("groups").select("*").order("sort_order",{ascending:!0}).order("created_at",{ascending:!0});g&&(t=g);const{data:v}=await S.from("user_groups").select("*");v&&(n=v);const{data:x}=await S.from("event_categories").select("*").order("sort_order",{ascending:!0}).order("created_at",{ascending:!0});x&&(a=x);const{data:h}=await S.from("user_attributes").select("*").order("sort_order",{ascending:!0}).order("created_at",{ascending:!0});h&&(s=h);try{const{data:L}=await S.from("event_locations").select("*").order("sort_order",{ascending:!0}).order("created_at",{ascending:!0});L&&(r=L)}catch(L){console.warn("event_locations table not created yet:",L)}const{data:k}=await S.from("master_data").select("*").eq("key","ATTENDANCE_DELEGATIONS").single();k&&k.data?window.adminDelegations=k.data:window.adminDelegations={}}catch(g){console.error("Groups DB Error:",g)}const o=localStorage.getItem("admin_users_sort")||"created_desc",i=document.getElementById("admin-users-sort");if(i&&(i.value=o),e&&e.length>0){const g=new Map(t.map(h=>[h.id,h.name])),v=new Map(s.map(h=>[h.id,h.name])),x=new Map(n.map(h=>[h.user_email,h.group_id]));if(o==="manual"){let h=[];try{const k=localStorage.getItem("admin_users_manual_order");k&&(h=JSON.parse(k))}catch(k){console.error("Failed to parse manual user order:",k)}e.sort((k,L)=>{let w=h.indexOf(k.email),I=h.indexOf(L.email);return w===-1&&(w=999999),I===-1&&(I=999999),w!==I?w-I:new Date(L.created_at||0)-new Date(k.created_at||0)})}else if(o==="created_desc")e.sort((h,k)=>new Date(k.created_at||0)-new Date(h.created_at||0));else if(o==="created_asc")e.sort((h,k)=>new Date(h.created_at||0)-new Date(k.created_at||0));else if(o==="name_asc")e.sort((h,k)=>(h.name||"").localeCompare(k.name||"","ja"));else if(o==="name_desc")e.sort((h,k)=>(k.name||"").localeCompare(h.name||"","ja"));else if(o==="email_asc")e.sort((h,k)=>(h.email||"").localeCompare(k.email||"","en"));else if(o==="email_desc")e.sort((h,k)=>(k.email||"").localeCompare(h.email||"","en"));else if(o==="role_desc"){const h={admin:3,leader:2,user:1};e.sort((k,L)=>{const w=h[k.role]||0,I=h[L.role]||0;return w!==I?I-w:new Date(L.created_at||0)-new Date(k.created_at||0)})}else o==="group_asc"?e.sort((h,k)=>{const L=g.get(x.get(h.email))||"",w=g.get(x.get(k.email))||"";return!L&&w?1:L&&!w?-1:L.localeCompare(w,"ja")}):o==="attribute_asc"&&e.sort((h,k)=>{const L=v.get(h.attribute_id)||"",w=v.get(k.attribute_id)||"";return!L&&w?1:L&&!w?-1:L.localeCompare(w,"ja")})}const d=document.getElementById("allowed-users-list");let l=`
        ...
        `;const f=(e||[]).map((g,v)=>{var $;const x=g.email.endsWith("@local.dummy"),h=(($=n.find(_=>_.user_email===g.email))==null?void 0:$.group_id)||"",k=t.map(_=>`<option value="${_.id}" ${h===_.id?"selected":""}>${_.name}</option>`).join(""),L=`
                <select id="edit-group-${v}" class="border p-1 rounded text-sm w-36 font-semibold text-gray-700 bg-white">
                    <option value="">選択なし</option>
                    ${k}
                </select>
            `,w=(e||[]).filter(_=>_.email!==g.email).map(_=>`
                <label class="inline-flex items-center text-xs mr-3 mb-1 w-32 truncate" title="${_.email}">
                    <input type="checkbox" name="edit-delegation-${v}" value="${_.email}" ${window.adminDelegations[g.email]&&window.adminDelegations[g.email].includes(_.email)?"checked":""} class="mr-1 rounded text-blue-600">
            ...
            `:"";return`
            <div class="user-admin-card flex items-stretch p-3 bg-white border rounded shadow-sm mb-2 hover:bg-gray-50 transition" data-email="${g.email}" data-index="${v}" data-old-role="${g.role}">
                ${E}
                <div class="flex-grow flex flex-col">
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-2 border-b border-gray-100 pb-2 mb-2">
                        ...
                        <input type="text" id="edit-name-${v}" value="${g.name||""}" placeholder="氏名" class="border p-1 rounded text-sm w-32 font-bold">
                        <input type="text" id="edit-email-${v}" value="${Xt(g.email)}" class="border p-1 rounded text-sm w-48 font-bold" ${g.email===V.email||x?"disabled":""}>
                        ...
                        <select id="edit-role-${v}" class="border p-1 rounded text-sm" ${g.email===V.email?"disabled":""}>
                         ...
                         <div class="flex items-center space-x-3 ml-2 border-l pl-2 flex-wrap gap-y-1">
                            <label class="flex items-center space-x-1 text-xs font-bold text-gray-600"><input type="checkbox" id="edit-use-dispatch-${v}" class="rounded text-blue-600" ${g.can_use_dispatch!==!1?"checked":""}><span>配車可</span></label>
                            <label class="flex items-center space-x-1 text-xs font-bold text-gray-600"><input type="checkbox" id="edit-use-dashboard-${v}" class="rounded text-blue-600" ${g.can_use_dashboard!==!1?"checked":""}><span>成績可</span></label>
                            <label class="flex items-center space-x-1 text-xs font-bold text-gray-600"><input type="checkbox" id="edit-use-attendance-${v}" class="rounded text-blue-600" ${g.can_use_attendance!==!1?"checked":""}><span>出欠可</span></label>
                            <label class="flex items-center space-x-1 text-xs font-bold text-gray-600"><input type="checkbox" id="edit-use-simulator-${v}" class="rounded text-blue-600" ${g.can_use_simulator!==!1?"checked":""}><span>シミュレータ可</span></label>
                            <label class="flex items-center space-x-1 text-xs font-bold text-gray-600"><input type="checkbox" id="edit-use-info-${v}" class="rounded text-teal-600" ${g.can_use_info!==!1?"checked":""}><span>Info可</span></label>
                        </div>
                    </div>
                    <div class="flex items-center space-x-2 shrink-0">
                        ...
                        ${g.email!==V.email?`<button onclick="deleteAdminUser('${g.email}')" class="text-xs bg-red-500 hover:bg-red-600 text-white px-2 py-1 rounded shadow">削除</button>`:""}
                    </div>
                </div>
            ...
            `}).join("");d.innerHTML=f,ld();const b=document.getElementById("admin-master-list");b&&(b.innerHTML=l+c+m+u,Hn("admin-attribute-list","user_attributes",ae),Hn("admin-category-list","event_categories",ae),Hn("admin-group-list","groups",ae),Hn("admin-location-list","event_locations",ae));const{data:y}=await S.from("signup_requests").select("*").eq("status","pending").order("created_at",{ascending:!1}),p=document.getElementById("signup-requests-list");!y||y.length===0?p.innerHTML='<p class="text-gray-500 text-sm">現在、承認待ちの申請はありません。</p>':p.innerHTML=y.map(g=>`
            ...
            `).join("")}finally{F()}}async function dd(){const e=document.querySelectorAll(".user-admin-card");if(e.length!==0&&confirm("全メンバーの設定を一括保存しますか？")){U("全メンバー設定を保存中...");try{const t=[],n=[],a=[];e.forEach(d=>{const l=d.getAttribute("data-email"),c=d.getAttribute("data-index"),m=d.getAttribute("data-old-role"),u=document.getElementById(`edit-email-${c}`),f=document.getElementById(`edit-name-${c}`),b=document.getElementById(`edit-attribute-${c}`),y=document.getElementById(`edit-role-${c}`),p=document.getElementById(`edit-use-dispatch-${c}`),g=document.getElementById(`edit-use-dashboard-${c}`),v=document.getElementById(`edit-use-attendance-${c}`),x=document.getElementById(`edit-use-simulator-${c}`),h=document.getElementById(`edit-use-info-${c}`),k=u.disabled?l:pa(u.value.trim()),L=f.value.trim(),w=b&&b.value||null,I=y.disabled?m:y.value,E=p?p.checked:!0,$=g?g.checked:!0,_=v?v.checked:!0,C=x?x.checked:!0,P=h?h.checked:!0;if(!k)throw new Error("メールアドレスが空のレコードがあります。");a.push(l);const D={email:k,name:L,attribute_id:w,role:I,can_use_dispatch:E,can_use_dashboard:$,can_use_attendance:_,can_use_simulator:C,can_use_info:P};t.push({oldEmail:l,updatePayload:D});const O=document.getElementById(`edit-group-${c}`),q=O?O.value:"";q&&n.push({user_email:k,group_id:q});const B=document.querySelectorAll(`input[name="edit-delegation-${c}"]:checked`),A=Array.from(B).map(N=>N.value);l!==k&&window.adminDelegations[l]&&delete window.adminDelegations[l],window.adminDelegations[k]=A});const s=t.map(d=>S.from("app_users").update(d.updatePayload).eq("email",d.oldEmail)),r=await Promise.all(s);for(const d of r)if(d.error)throw d.error;const{error:o}=await S.from("user_groups").delete().in("user_email",a);if(o)throw o;if(n.length>0){const{error:d}=await S.from("user_groups").insert(n);if(d)throw d}const{error:i}=await S.from("master_data").upsert({key:"ATTENDANCE_DELEGATIONS",data:window.adminDelegations});if(i)throw i;await ee("UPDATE_USERS_ALL","全メンバーの設定を一括更新しました"),alert("全メンバーの設定を一括保存しました"),await ae()}catch(t){console.error(t),alert("保存中にエラーが発生しました: "+(t.message==="Load failed"||t.message==="Failed to fetch"?"通信に失敗しました。":t.message))}finally{F()}}}window.forceResetPassword=async function(e){if(confirm(`${e} 宛にパスワード再設定メールを送信し、強制的にパスワードをリセットさせますか？`)){U("パスワード再設定メール送信中...");try{const{error:t}=await S.auth.resetPasswordForEmail(e,{redirectTo:window.location.origin});alert(t?"送信失敗: "+t.message:"パスワード再設定メールを送信しました。")}catch(t){console.error(t)}finally{F()}}};window.adminChangeUserPassword=async function(e){const t=Xt(e),n=prompt(`「${t}」の新しいパスワードを入力してください（6文字以上）：`);if(n===null)return;const a=n.trim();if(a.length<6)return alert("パスワードは6文字以上で設定してください。");U("パスワード変更中...");try{const{error:s}=await S.rpc("admin_update_user_password",{user_email:e,new_password:a});if(s)throw new Error(s.message);await ee("ADMIN_CHANGE_PASSWORD",`ユーザー「${e}」のパスワードを変更しました`),alert(`「${t}」のパスワードを正常に変更しました。`)}catch(s){console.error(s),alert(`パスワード変更エラー:
`+s.message+`

※この機能を使用するには、あらかじめSupabaseのSQLエディタで専用のデータベース関数（admin_update_user_password）を設定する必要があります。詳細はマニュアルまたはエージェントのメッセージを参照してください。`)}finally{F()}};window.adminAddUser=async function(){var r,o,i,d;const e=((r=document.getElementById("admin-add-email"))==null?void 0:r.value.trim())||"",t=pa(e),n=((o=document.getElementById("admin-add-name"))==null?void 0:o.value.trim())||"",a=((i=document.getElementById("admin-add-password"))==null?void 0:i.value.trim())||"",s=((d=document.getElementById("admin-add-role"))==null?void 0:d.value)||"user";if(!e||!n||!a)return alert("ユーザーID（またはメールアドレス）、氏名、仮パスワードは必須入力項目です。");if(a.length<6)return alert("仮パスワードは6文字以上で設定してください。");U("アカウント払い出し中...");try{const l=window.supabase.createClient(cs,us,{auth:{persistSession:!1,autoRefreshToken:!1}}),{data:c,error:m}=await l.auth.signUp({email:t,password:a});if(m)throw new Error(`Authアカウント作成失敗: ${m.message}`);const{error:u}=await S.from("app_users").insert([{email:t,name:n,role:s}]);if(u)throw new Error(`データベース登録失敗: ${u.message}`);await ee("ADD_USER_COMPLETED",`アカウント「${t}」を仮パスワード付きで払い出しました`),document.getElementById("admin-add-email").value="",document.getElementById("admin-add-name").value="",document.getElementById("admin-add-password").value="",alert(`アカウントの払い出しが完了しました！
...
（※ログインはできず、他のメンバーからの代行入力専用アカウントとなります）`);if(!e||e.trim()==="")return;const t=`dummy_${Date.now()}@local.dummy`;U("代行専用メンバー追加処理中...");try{await S.from("app_users").insert([{email:t,name:e.trim(),role:"user"}]),await ee("ADD_DUMMY_USER",`代行専用メンバー「${e.trim()}」を追加しました`),await ae()}catch(n){console.error(n),alert("追加エラー: "+(n.message==="Load failed"||n.message==="Failed to fetch"?"通信に失敗しました。":n.message))}finally{F()}};window.deleteAdminUser=async function(e){if(confirm(`${e} のアクセス許可を取り消しますか？`)){U("ユーザー削除処理中...");try{await S.from("app_users").delete().eq("email",e),await ee("DELETE_USER",`ユーザー「${e}」を削除しました`),await ae()}catch(t){console.error(t),alert("削除エラー: "+(t.message==="Load failed"||t.message==="Failed to fetch"?"通信に失敗しました。":t.message))}finally{F()}}};window.saveNewCategoryAdmin=async function(){var n;const e=document.getElementById("admin-new-category-name").value.trim(),t=((n=document.getElementById("admin-new-category-color"))==null?void 0:n.value)||"#bfdbfe";if(!e)return alert("カテゴリ名を入力してください");U("カテゴリ追加中...");try{const{error:a}=await S.from("event_categories").insert([{name:e,color:t}]);if(a)throw a;await ae()}catch(a){alert("追加エラー: "+(a.message==="Load failed"||a.message==="Failed to fetch"?"通信に失敗しました。":a.message))}finally{F()}};window.deleteCategoryAdmin=async function(e){if(confirm(`このカテゴリを削除しますか？
※既存のイベントに設定されているカテゴリ名には影響しませんが、新規作成・編集時に選択できなくなります。`)){U("カテゴリ削除中...");try{const{error:t}=await S.from("event_categories").delete().eq("id",e);if(t)throw t;await ae()}catch(t){alert("削除エラー: "+(t.message==="Load failed"||t.message==="Failed to fetch"?"通信に失敗しました。":t.message))}finally{F()}}};window.renameCategoryAdmin=async function(e,t){const n=prompt("新しいカテゴリ名を入力してください:",t);if(!(!n||n.trim()===""||n===t)){U("カテゴリ名称変更中...");try{const{error:a}=await S.from("event_categories").update({name:n.trim()}).eq("id",e);if(a)throw a;await ae()}catch(a){alert("変更エラー: "+(a.message==="Load failed"||a.message==="Failed to fetch"?"通信に失敗しました。":a.message))}finally{F()}}};window.updateCategoryColorAdmin=async function(e,t){U("カテゴリ色変更中...");try{const{error:n}=await S.from("event_categories").update({color:t}).eq("id",e);if(n)throw n;await ae()}catch(n){alert("変更エラー: "+(n.message==="Load failed"||n.message==="Failed to fetch"?"通信に失敗しました。":n.message))}finally{F()}};window.updateGroupColorAdmin=async function(e,t){U("グループ色変更中...");try{const{error:n}=await S.from("groups").update({color:t}).eq("id",e);if(n)throw n;await ae()}catch(n){alert("色変更エラー: "+(n.message==="Load failed"||n.message==="Failed to fetch"?"通信に失敗しました。":n.message))}finally{F()}};window.saveNewGroupAdmin=async function(){var n;const e=document.getElementById("admin-new-group-name").value.trim(),t=((n=document.getElementById("admin-new-group-color"))==null?void 0:n.value)||"#d1fae5";if(!e)return alert("グループ名を入力してください");U("グループ追加中...");try{const{error:a}=await S.from("groups").insert([{name:e,color:t}]);if(a)throw a;await ae()}catch(a){alert("追加エラー: "+(a.message==="Load failed"||a.message==="Failed to fetch"?"通信に失敗しました。":a.message))}finally{F()}};window.deleteGroupAdmin=async function(e){if(confirm(`このグループを削除しますか？
※関連する出欠データやメンバー設定にも影響が出る可能性があります。`)){U("グループ削除中...");try{const{error:t}=await S.from("groups").delete().eq("id",e);if(t)throw t;await ae()}catch(t){alert("削除エラー: "+(t.message==="Load failed"||t.message==="Failed to fetch"?"通信に失敗しました。":t.message))}finally{F()}}};window.renameGroupAdmin=async function(e,t){const n=prompt("新しいグループ名を入力してください:",t);if(!(!n||n.trim()===""||n===t)){U("グループ名称変更中...");try{const{error:a}=await S.from("groups").update({name:n.trim()}).eq("id",e);if(a)throw a;await ae()}catch(a){alert("変更エラー: "+(a.message==="Load failed"||a.message==="Failed to fetch"?"通信に失敗しました。":a.message))}finally{F()}}};window.saveNewLocationAdmin=async function(){const e=document.getElementById("admin-new-location-name").value.trim(),t=document.getElementById("admin-new-location-url").value.trim();if(!e)return alert("場所名を入力してください");U("場所追加中...");try{const{error:n}=await S.from("event_locations").insert([{name:e,url:t||null}]);if(n)throw n;await ae()}catch(n){alert("追加エラー: "+(n.message==="Load failed"||n.message==="Failed to fetch"?"通信に失敗しました。":n.message))}finally{F()}};window.deleteLocationAdmin=async function(e){if(confirm(`この場所をマスタから削除しますか？
※既存の予定データ内の場所テキスト自体は削除されません。`)){U("場所削除中...");try{const{error:t}=await S.from("event_locations").delete().eq("id",e);if(t)throw t;await ae()}catch(t){alert("削除エラー: "+(t.message==="Load failed"||t.message==="Failed to fetch"?"通信に失敗しました。":t.message))}finally{F()}}};window.renameLocationAdmin=async function(e){U("場所データ読み込み中...");try{const{data:t,error:n}=await S.from("event_locations").select("*").eq("id",e).single();if(n)throw n;const a=document.getElementById("admin-edit-location-modal");a&&a.remove();const s=t.name||"",r=t.url||"",o=`
        ...
        `;document.body.insertAdjacentHTML("beforeend",o)}catch(t){alert("読み込みエラー: "+(t.message==="Load failed"||t.message==="Failed to fetch"?"通信に失敗しました。":t.message))}finally{F()}};window.saveLocationEditAdmin=async function(e){const t=document.getElementById("edit-location-name").value.trim(),n=document.getElementById("edit-location-url").value.trim();if(t==="")return alert("場所の名前は必須項目です。");U("場所マスタ更新中...");try{const{error:a}=await S.from("event_locations").update({name:t,url:n||null}).eq("id",e);if(a)throw a;const s=document.getElementById("admin-edit-location-modal");s&&s.remove(),await ae()}catch(a){alert("更新エラー: "+(a.message==="Load failed"||a.message==="Failed to fetch"?"通信に失敗しました。":a.message))}finally{F()}};function Hn(e,t,n){const a=document.getElementById(e);if(!a)return;let s=null;Array.from(a.children).forEach(r=>{r.setAttribute("draggable","true"),r.classList.add("cursor-move","select-none"),r.addEventListener("dragstart",o=>{s=r,o.dataTransfer.effectAllowed="move",r.classList.add("opacity-50")}),r.addEventListener("dragover",o=>{o.preventDefault(),o.dataTransfer.dropEffect="move";const i=o.target.closest('[draggable="true"]');if(i&&i!==s&&i.parentNode===a){const d=i.getBoundingClientRect(),l=(o.clientY-d.top)/(d.bottom-d.top)>.5;a.insertBefore(s,l?i.nextSibling:i)}}),r.addEventListener("dragend",async()=>{r.classList.remove("opacity-50"),s=null;const i=Array.from(a.children).map((d,l)=>{const c=d.dataset.id;return S.from(t).update({sort_order:l}).eq("id",c)});U("順序を保存中...");try{await Promise.all(i),n&&await n()}catch(d){console.error("Sort order save error:",d),alert("順序の保存に失敗しました。")}finally{F()}})})}function ld(){const e=document.getElementById("allowed-users-list");if(!e)return;if((localStorage.getItem("admin_users_sort")||"created_desc")!=="manual"){Array.from(e.children).forEach(a=>{a.removeAttribute("draggable"),a.classList.remove("cursor-move","select-none");const s=a.querySelector(".user-drag-handle");s&&(s.style.display="none")});return}let n=null;Array.from(e.children).forEach(a=>{const s=a.querySelector(".user-drag-handle");s?(s.style.display="flex",s.addEventListener("mousedown",()=>{a.setAttribute("draggable","true")}),s.addEventListener("mouseup",()=>{a.removeAttribute("draggable")})):a.setAttribute("draggable","true"),a.addEventListener("dragstart",r=>{if(a.getAttribute("draggable")!=="true"){r.preventDefault();return}n=a,r.dataTransfer.effectAllowed="move",a.classList.add("opacity-50")}),a.addEventListener("dragover",r=>{r.preventDefault(),r.dataTransfer.dropEffect="move";const o=r.target.closest(".user-admin-card");if(o&&o!==n&&o.parentNode===e){const i=o.getBoundingClientRect(),d=(r.clientY-i.top)/(i.bottom-i.top)>.5;e.insertBefore(n,d?o.nextSibling:o)}}),a.addEventListener("dragend",()=>{a.classList.remove("opacity-50"),a.removeAttribute("draggable"),n=null;const o=Array.from(e.children).map(i=>i.getAttribute("data-email")).filter(Boolean);localStorage.setItem("admin_users_manual_order",JSON.stringify(o))})})}window.saveNewUserAttributeAdmin=async function(){const e=document.getElementById("admin-new-attribute-name").value.trim();if(!e)return alert("属性名を入力してください");U("属性追加中...");try{const{error:t}=await S.from("user_attributes").insert([{name:e}]);if(t)throw t;await ae()}catch(t){alert("追加エラー: "+(t.message==="Load failed"||t.message==="Failed to fetch"?"通信に失敗しました。":t.message))}finally{F()}};window.deleteUserAttributeAdmin=async function(e){if(confirm(`この属性を削除しますか？
※ユーザーに設定されている属性は解除されます。`)){U("属性削除中...");try{const{error:t}=await S.from("user_attributes").delete().eq("id",e);if(t)throw t;await ae()}catch(t){alert("削除エラー: "+(t.message==="Load failed"||t.message==="Failed to fetch"?"通信に失敗しました。":t.message))}finally{F()}}};window.renameUserAttributeAdmin=async function(e,t){const n=prompt("新しい属性名を入力してください:",t);if(!(!n||n.trim()===""||n===t)){U("属性名称変更中...");try{const{error:a}=await S.from("user_attributes").update({name:n.trim()}).eq("id",e);if(a)throw a;await ae()}catch(a){alert("変更エラー: "+(a.message==="Load failed"||a.message==="Failed to fetch"?"通信に失敗しました。":a.message))}finally{F()}}};window.approveRequest=async function(e,t){U("申請承認中...");try{const{data:n}=await S.from("signup_requests").select("parent_name, player_name").eq("id",e).single(),a=n?`${n.parent_name} (${n.player_name})`:"";await S.from("app_users").insert([{email:t,role:"user",name:a}]),await S.from("signup_requests").update({status:"approved"}).eq("id",e);try{const{error:s}=await S.auth.resetPasswordForEmail(t,{redirectTo:window.location.origin});s&&(console.warn("Auto password reset email failed (rate limit):",s.message),alert(`メンバー承認は完了しました！

※ただし、Supabaseのメール送信制限（レートリミット等）により、パスワード設定案内メールの自動送信に失敗しました（エラー: `+s.message+`）。

お手数ですが、ログイン画面の『パスワードを忘れた場合』からユーザー自身で再設定を行っていただくよう案内するか、時間をおいてメンバーリストの『PWリセット送信』から再送信してください。`))}catch(s){console.error("Auto password reset email exception:",s)}await ae()}catch(n){console.error(n),alert("承認エラー: "+(n.message==="Load failed"||n.message==="Failed to fetch"?"通信に失敗しました。":n.message))}finally{F()}};window.rejectRequest=async function(e){if(confirm("この申請を拒否しますか？")){U("申請拒否中...");try{await S.from("signup_requests").update({status:"rejected"}).eq("id",e),await ae()}catch(t){console.error(t),alert("拒否エラー: "+(t.message==="Load failed"||t.message==="Failed to fetch"?"通信に失敗しました。":t.message))}finally{F()}}};const J={initMasterData:async()=>ye(async()=>{const{data:e,error:t}=await S.from("master_data").select("*");if(t)throw t;if(re=[],oe=[],e){const n=e.find(s=>s.key==="FAMILIES"),a=e.find(s=>s.key==="CARS");n&&(re=n.data||[]),a&&(oe=a.data||[])}},"マスターデータ初期化中..."),getAllFamilies:async()=>re,getFamily:async e=>re.find(t=>t.familyName===e),addFamily:e=>{re.push(e)},updateFamily:e=>{const t=re.findIndex(n=>n.familyName===e.familyName);t>-1&&(re[t]=e)},deleteFamily:e=>{re=re.filter(t=>t.familyName!==e)},bulkAddFamilies:e=>{re=e},getAllCars:async()=>oe,getCar:async e=>oe.find(t=>t.id===e),addCar:e=>{oe.push(e)},updateCar:e=>{const t=oe.findIndex(n=>n.id===e.id);t>-1&&(oe[t]=e)},deleteCar:e=>{oe=oe.filter(t=>t.id!==e)},bulkAddCars:e=>{oe=e},syncMaster:async()=>ye(async()=>{const{error:e}=await S.from("master_data").upsert([{key:"FAMILIES",data:re},{key:"CARS",data:oe}]);if(e)throw e;await ee("UPDATE_MASTER","初期データ(または強制)のマスター保存を実行しました")},"マスターデータ同期中..."),saveState:async(e,t)=>ye(async()=>{const n=Date.now().toString(),{error:a}=await S.from("states").insert({id:n,name:t,created_at:Date.now(),state_data:e});if(a)throw a;return await ee("SAVE_DISPATCH",`配車データ「${t}」を保存しました`),!0},"配車状態を保存中..."),getAllSavedStates:async()=>ye(async()=>{const{data:e,error:t}=await S.from("states").select("*").order("created_at",{ascending:!1});if(t)throw t;return e.map(n=>({id:n.id,name:n.name,timestamp:n.created_at,state:n.state_data}))},"配車状態一覧を取得中..."),getState:async e=>ye(async()=>{const{data:t,error:n}=await S.from("states").select("*").eq("id",e).single();if(n)throw n;return t?{id:t.id,name:t.name,timestamp:t.created_at,state:t.state_data}:null},"配車状態を取得中..."),deleteState:async e=>ye(async()=>{const{error:t}=await S.from("states").delete().eq("id",e);if(t)throw t;return await ee("DELETE_DISPATCH",`配車データ(ID:${e})を削除しました`),!0},"配車状態を削除中..."),saveParking:async(e,t)=>ye(async()=>{const n="p"+Date.now()+Math.floor(Math.random()*1e3),{error:a}=await S.from("parkings").insert({id:n,name:t,created_at:Date.now(),parking_data:e});if(a)throw a;return await ee("SAVE_PARKING",`駐車場データ「${t}」を保存しました`),!0},"駐車場データを保存中..."),getAllSavedParking:async()=>ye(async()=>{const{data:e,error:t}=await S.from("parkings").select("*").order("created_at",{ascending:!1});if(t)throw t;return e.map(n=>({id:n.id,name:n.name,timestamp:n.created_at,parking:n.parking_data}))},"駐車場データ一覧を取得中..."),getParking:async e=>ye(async()=>{const{data:t,error:n}=await S.from("parkings").select("*").eq("id",e).single();if(n)throw n;return t?{id:t.id,name:t.name,timestamp:t.created_at,parking:t.parking_data}:null},"駐車場データを取得中..."),updateParking:async e=>ye(async()=>{const{error:t}=await S.from("parkings").update({name:e.name,parking_data:e.parking}).eq("id",e.id);if(t)throw t;return!0},"駐車場データを更新中..."),deleteParking:async e=>ye(async()=>{const{error:t}=await S.from("parkings").delete().eq("id",e);if(t)throw t;return await ee("DELETE_PARKING",`駐車場データ(ID:${e})を削除しました`),!0},"駐車場データを削除中..."),addParkingMaster:(e,t)=>{Fe.push({id:"p"+Date.now()+Math.floor(Math.random()*1e3),name:t,timestamp:Date.now(),parking:e,isNew:!0})},updateParkingMaster:e=>{const t=Fe.findIndex(n=>n.id===e.id);t>-1&&(Fe[t]=e,Fe[t].isModified=!0)},deleteParkingMaster:e=>{const t=Fe.find(n=>n.id===e);t&&!t.isNew&&In.add(e),Fe=Fe.filter(n=>n.id!==e)},syncAllMaster:async()=>ye(async()=>{const{error:e}=await S.from("master_data").upsert([{key:"FAMILIES",data:re},{key:"CARS",data:oe}]);if(e)throw e;for(const t of In)await S.from("parkings").delete().eq("id",t);In.clear();for(const t of Fe)if(t.isNew||t.isModified){const{error:n}=await S.from("parkings").upsert({id:t.id,name:t.name,created_at:t.timestamp,parking_data:t.parking});if(n)throw n;t.isNew=!1,t.isModified=!1}await ee("UPDATE_MASTER","マスターデータ(家族・車・駐車場)を一括保存しました")},"マスターデータ一括同期中..."),clearDatabase:async()=>ye(async()=>{re=[],oe=[],await S.from("master_data").delete().neq("key",""),await S.from("states").delete().neq("id",""),await S.from("parkings").delete().neq("id",""),await ee("CLEAR_DB","データベースの全リセットを実行しました")},"データベース初期化中...")},cd=[{familyName:"山田家",order:1,members:[{id:"p1",name:"太郎",type:"選手",isFlagTarget:!0,data:{grade:"5年",school:"A小",other:"",memo:""}},{id:"p2",name:"山田父",type:"保護者",data:{memo:""}}]},{familyName:"佐藤家",order:2,members:[{id:"p3",name:"次郎",type:"選手",isFlagTarget:!0,data:{grade:"5年",school:"B小",other:"",memo:""}},{id:"p4",name:"佐藤母",type:"保護者",data:{memo:""}}]},{familyName:"スタッフ・個人",order:99,members:[{id:"p99",name:"監督",type:"その他",data:{memo:""}}]}],ud=[{id:"c1",name:"山田カー",familyName:"山田家",baseCapacity:6,order:1},{id:"c2",name:"佐藤カー",familyName:"佐藤家",baseCapacity:5,order:2}];let qt=[],oa=[],ga=[],jt=new Set,Ht=new Set,Et=new Map,Nt=new Set,Xe=new Set,et=new Map,fe={groundName:"",designated:{name:"",limit:0,memo:""},other:{name:"",memo:""}},ke={date:"",name:"",timeline:"",notes:""},K=[],gt={car:null,seat:null},La=null,Sa=null;const Ya=3,Ie=document.getElementById("nav-dispatch"),$e=document.getElementById("nav-master"),an=document.getElementById("view-dispatch"),sn=document.getElementById("view-master"),We=document.getElementById("participant-list"),Wt=document.getElementById("car-list"),zt=document.getElementById("exclusion-list"),Ba=document.getElementById("results");document.getElementById("text-output");const Qe=document.getElementById("family-list"),ft=document.getElementById("car-list-master"),Bt=document.getElementById("parking-list-master");function Us(){const e=document.getElementById("results-section"),t=document.getElementById("main-content");t&&(t.style.display="grid",t.style.gap="1.5rem",window.innerWidth>=768?(t.style.gridTemplateColumns=`repeat(${Ya}, minmax(0, 1fr))`,e&&(e.style.gridColumn=`span ${Ya}`)):(t.style.gridTemplateColumns="repeat(1, minmax(0, 1fr))",e&&(e.style.gridColumn="auto")))}async function Vs(){qt=await J.getAllFamilies(),oa=await J.getAllCars(),ga=qt.flatMap(e=>e.members),Er(),fa(),ba(),Rn(),await ps(),await gs()}async function md(){Fe=await J.getAllSavedParking()||[],In.clear(),Jt(),fn(),ha()}async function Sn(){const e=document.getElementById("btn-load-logs");if(e&&!document.getElementById("btn-download-logs-csv")){const s=document.createElement("button");s.id="btn-download-logs-csv",s.className="ml-2 bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded text-sm font-bold shadow",s.textContent="CSVダウンロード",s.onclick=pd,e.parentNode.insertBefore(s,e.nextSibling)}try{const s=new Date;s.setMonth(s.getMonth()-6),await S.from("action_logs").delete().lt("created_at",s.toISOString())}catch(s){console.error("Log rotation error:",s)}e&&(e.textContent="読込中...");const{data:t,error:n}=await S.from("action_logs").select("*").order("created_at",{ascending:!1}).limit(50);if(e&&(e.textContent="最新を読み込み"),n){console.error("Failed to load logs:",n);return}const a=document.getElementById("log-list");if(a){if(!t||t.length===0){a.innerHTML='<p class="text-gray-500 text-center py-4">ログはありません</p>';return}a.innerHTML=t.map(s=>`
        ...
        `).join("")}}async function pd(){U("ログ取得中...");try{const{data:e,error:t}=await S.from("action_logs").select("*").order("created_at",{ascending:!1}).limit(1e4);if(t)throw t;if(!e||e.length===0)return alert("ログデータがありません");const n=[["日時","アクション","ユーザー","詳細"]];e.forEach(i=>{const d=new Date(i.created_at).toLocaleString("ja-JP"),l=c=>`"${String(c||"").replace(/"/g,'""')}"`;n.push([l(d),l(i.action_type),l(i.user_email),l(i.details)])});const a=new Uint8Array([239,187,191]),s=new Blob([a,n.map(i=>i.join(",")).join(`
`)],{type:"text/csv;charset=utf-8;"}),r=URL.createObjectURL(s),o=document.createElement("a");o.href=r,o.download=`action_logs_${new Date().toISOString().split("T")[0]}.csv`,o.click(),URL.revokeObjectURL(r)}catch(e){console.error(e),alert("CSVの作成に失敗しました: "+e.message)}finally{F()}}function gd(){var e,t,n,a,s,r,o,i,d,l,c,m,u,f,b;We==null||We.addEventListener("change",bd),We==null||We.addEventListener("input",yd),We==null||We.addEventListener("click",fd),Wt==null||Wt.addEventListener("change",hd),zt==null||zt.addEventListener("change",xd),(e=document.getElementById("assign-button"))==null||e.addEventListener("click",vd),(t=document.getElementById("dispatch-message-close"))==null||t.addEventListener("click",fs),Ba==null||Ba.addEventListener("change",Ed),(n=document.getElementById("export-state-button"))==null||n.addEventListener("click",$d),(a=document.getElementById("import-state-input"))==null||a.addEventListener("change",kd),(s=document.getElementById("show-text-output-button"))==null||s.addEventListener("click",()=>{var y;(y=document.getElementById("text-output-container"))==null||y.classList.toggle("hidden"),Fn()}),(r=document.getElementById("copy-text-output-button"))==null||r.addEventListener("click",_d),(o=document.getElementById("toggle-details-button"))==null||o.addEventListener("click",Id),(i=document.getElementById("save-state-db-button"))==null||i.addEventListener("click",Ld),(d=document.getElementById("load-state-db-button"))==null||d.addEventListener("click",Sd),(l=document.getElementById("delete-state-db-button"))==null||l.addEventListener("click",Bd),(c=document.getElementById("save-parking-db-button"))==null||c.addEventListener("click",Td),(m=document.getElementById("load-parking-db-button"))==null||m.addEventListener("click",Ad),(u=document.getElementById("delete-parking-db-button"))==null||u.addEventListener("click",Cd),(f=document.getElementById("clear-db-button"))==null||f.addEventListener("click",Dd),(b=document.getElementById("btn-clear-inputs"))==null||b.addEventListener("click",window.clearCurrentInputs)}function Er(){et.clear(),qt.forEach(e=>{e.members.forEach(t=>{const n={grade:"",school:"",other:"",memo:"",...t.data||{}};t.isFlagTarget||(n.grade="",n.school="",n.other=""),et.set(t.id,n)})})}function fa(){if(We){if(We.innerHTML="",qt.length===0)return We.innerHTML='<p class="text-gray-500">データなし</p>';qt.forEach(e=>{const t=document.createElement("details");t.className="bg-gray-50 rounded border",t.open=!0;const n=document.createElement("summary");n.className="p-3 cursor-pointer select-none flex justify-between items-center",n.innerHTML=`<span class="font-semibold">${e.familyName}</span><div class="space-x-1"><button data-family-id="${e.familyName}" data-check-action="check" class="text-xs bg-blue-500 hover:bg-blue-600 text-white py-1 px-2 rounded">全員参加</button><button data-family-id="${e.familyName}" data-check-action="uncheck" class="text-xs bg-gray-400 hover:bg-gray-500 text-white py-1 px-2 rounded">全員不参加</button></div>`;const a=document.createElement("div");a.className="p-3 border-t border-gray-200 space-y-3",e.members.forEach(s=>{const r=jt.has(s.id),o=et.get(s.id)||{grade:"",school:"",other:"",memo:""};let i=s.isFlagTarget?`<input type="text" data-id="${s.id}" data-type="grade" value="${o.grade}" placeholder="学年" class="w-full text-sm p-1 border rounded" ${r?"":"disabled"}>
            ...
            </div></div>`})}}function Rn(){if(!zt)return;zt.innerHTML="";const e=ga.filter(t=>jt.has(t.id));if(e.length===0)return zt.innerHTML='<p class="text-gray-500 text-sm">参加者を選択してください</p>';e.forEach(t=>{zt.innerHTML+=`<div class="flex items-center"><input type="checkbox" id="ex-${t.id}" data-id="${t.id}" data-action="exclude-participant" class="mr-3" ${Xe.has(t.id)?"checked":""}><label for="ex-${t.id}">${t.name} (${t.type})</label></div>`})}function bd(e){const t=e.target;if(t.dataset.action==="select-participant"){const n=t.dataset.id,a=document.getElementById(`data-inputs-${n}`);t.checked?(jt.add(n),a&&(a.classList.remove("opacity-50"),a.querySelectorAll("input").forEach(s=>s.disabled=!1))):(jt.delete(n),Xe.delete(n),a&&(a.classList.add("opacity-50"),a.querySelectorAll("input").forEach(s=>s.disabled=!0))),Rn()}}function yd(e){const t=e.target;if(t.dataset.type){const n=et.get(t.dataset.id);n[t.dataset.type]=t.value,et.set(t.dataset.id,n)}}function hd(e){var a;const t=e.target,n=(a=t.closest("[data-car-id]"))==null?void 0:a.dataset.carId;n&&(t.dataset.action==="select-car"?t.checked?(Ht.add(n),document.getElementById(`car-options-${n}`).classList.remove("hidden")):(Ht.delete(n),Nt.delete(n),document.getElementById(`car-options-${n}`).classList.add("hidden")):t.dataset.action==="select-driver"?t.value?Et.set(n,t.value):Et.delete(n):t.dataset.action==="select-luggage"&&(t.checked?Nt.add(n):Nt.delete(n)))}function xd(e){const t=e.target;t.dataset.action==="exclude-participant"&&(t.checked?Xe.add(t.dataset.id):Xe.delete(t.dataset.id))}function vd(){var y,p,g,v,x,h,k,L,w,I;if(Ht.size===0)return Ve("車を選択してください","error");ke={date:((y=document.getElementById("event-date"))==null?void 0:y.value)||"",name:((p=document.getElementById("event-name"))==null?void 0:p.value)||"",timeline:((g=document.getElementById("event-timeline"))==null?void 0:g.value)||"",notes:((v=document.getElementById("event-notes"))==null?void 0:v.value)||""},fe={groundName:((x=document.getElementById("ground-name"))==null?void 0:x.value)||"",designated:{name:((h=document.getElementById("parking-designated-name"))==null?void 0:h.value)||"指定駐車場",limit:parseInt((k=document.getElementById("parking-designated-limit"))==null?void 0:k.value)||999,memo:((L=document.getElementById("parking-designated-memo"))==null?void 0:L.value)||""},other:{name:((w=document.getElementById("parking-other-name"))==null?void 0:w.value)||"指定以外",memo:((I=document.getElementById("parking-other-memo"))==null?void 0:I.value)||""}};let e=!1;if(K&&K.length>0&&K.some(E=>E.id!=="excluded-car")){if(confirm(`すでに配車結果が存在します。
現在の配車状態を【維持】して、追加・変更分のみを反映しますか？
（「キャンセル」を選ぶと、全てリセットして最初からやり直すか確認します）`))e=!0;else if(!confirm(`現在の状態を【全てリセット】して、最初から割り当てをやり直しますか？
（キャンセルを選ぶと処理を中断します）`))return}let t=[],n=new Map,a=[];const s=ga.filter(E=>jt.has(E.id)).map(E=>{const $=qt.find(C=>C.members.some(P=>P.id===E.id)),_=et.get(E.id)||{};return{...E,grade:_.grade,school:_.school,other:_.other,memo:_.memo,familyName:$?$.familyName:null}});if(Ht.forEach(E=>{const $=Et.get(E),_=oa.find(D=>D.id===E);if(!_)return;if(!$)return t.push(`${_.name}のドライバー未選択`);const C=s.find(D=>D.id===$);if(!C)return t.push(`${_.name}のドライバーが参加者にいません`);n.set(E,C);const P=Nt.has(E);a.push({id:E,name:_.name,familyName:_.familyName,baseCapacity:P?2:_.baseCapacity,driverId:$,capacity:P?1:_.baseCapacity-1,hasLuggage:P})}),t.length>0)return Ve(t.join("<br>"),"error");const r=new Set(Array.from(n.values()).map(E=>E.id));let o=s.filter(E=>Xe.has(E.id)&&!r.has(E.id)),i=[];e?a.forEach(E=>{let $=K.find(C=>C.id===E.id),_=[];$&&(_=$.members.filter(C=>{if(!C)return!1;let P=s.some(q=>q.id===C.id),D=Xe.has(C.id),O=r.has(C.id);return P&&!D&&!O}),_.length>E.capacity&&(_=_.slice(0,E.capacity))),i.push({...E,driver:n.get(E.id),members:_})}):i=a.map(E=>({...E,driver:n.get(E.id),members:[]}));let d=new Set(r);i.forEach(E=>E.members.forEach($=>{$&&d.add($.id)}));let l=s.filter(E=>!d.has(E.id)&&!Xe.has(E.id));i.reduce((E,$)=>E+$.capacity,0),i.reduce((E,$)=>E+$.members.length,0);let c=[...l];const m={保護者:1,兄弟:2,選手:3,その他:4};i.forEach(E=>{E.driver&&E.familyName&&c.filter(_=>_.familyName===E.familyName).sort((_,C)=>(m[_.type]||9)-(m[C.type]||9)).forEach(_=>{E.members.length<E.capacity&&(E.members.push(_),c=c.filter(C=>C.id!==_.id))})}),c.sort((E,$)=>E.isFlagTarget===$.isFlagTarget?Math.random()-.5:E.isFlagTarget?-1:1).forEach(E=>{let $=[],_=0;if(i.forEach(C=>{if(C.members.length>=C.capacity)return;let P=C.members.filter(D=>D).reduce((D,O)=>D+(O.isFlagTarget&&E.isFlagTarget?(E.grade===O.grade?1:0)+(E.school===O.school?1:0):0),0);P>_?(_=P,$=[C]):P===_&&$.push(C)}),$.length>0)$[Math.floor(Math.random()*$.length)].members.push(E);else{let C=i.filter(P=>P.members.length<P.capacity);C.length>0&&C[0].members.push(E)}}),i.forEach(E=>{for(E.members=E.members.filter($=>$);E.members.length<E.capacity;)E.members.push(null)}),e?i.sort((E,$)=>{let _=K.findIndex(P=>P.id===E.id),C=K.findIndex(P=>P.id===$.id);return _!==-1&&C!==-1?_-C:_!==-1?-1:C!==-1?1:$.members.filter(P=>P&&P.type==="選手").length-E.members.filter(P=>P&&P.type==="選手").length}):i.sort((E,$)=>$.members.filter(_=>_&&_.type==="選手").length-E.members.filter(_=>_&&_.type==="選手").length);let u=0;i.forEach(E=>{if(e){let $=K.find(_=>_.id===E.id);$?(E.assignedParking=$.assignedParking,E.assignedParking==="designated"&&u++):u<fe.designated.limit?(E.assignedParking="designated",u++):E.assignedParking="other"}else u<fe.designated.limit?(E.assignedParking="designated",u++):E.assignedParking="other"});const f=new Set(r);i.forEach(E=>{E.members.forEach($=>{$&&f.add($.id)})}),s.filter(E=>!f.has(E.id)&&!Xe.has(E.id)).forEach(E=>{o.some($=>$.id===E.id)||o.push(E)}),i.push({id:"excluded-car",name:"別便",capacity:999,baseCapacity:999,driver:null,members:o,hasLuggage:!1,assignedParking:"excluded"}),K=i,ya(),Fn()}function ya(){const e=document.getElementById("results");if(!e)return;if(e.innerHTML="",gt={car:null,seat:null},K.length===0){e.innerHTML='<p class="text-gray-500 bg-white p-4 rounded shadow">結果なし</p>',Gs();return}(ke.name||ke.date)&&(e.innerHTML+=`<h2 class="text-2xl font-bold mb-2">${ke.date} ${ke.name} ${fe.groundName?`@${fe.groundName}`:""}</h2>`);const t=K.filter(r=>r.assignedParking==="designated"),n=K.filter(r=>r.assignedParking==="other"),a=K.filter(r=>r.id==="excluded-car"),s=(r,o,i)=>`<div class="bg-white rounded shadow p-4"><h3 class="font-bold text-lg mb-2">◆${o.name||"別便"} ${r==="designated"&&o.limit<999?`(${o.limit}台)`:""}</h3><p class="text-sm text-gray-600 mb-4 whitespace-pre-line">${o.memo}</p><div class="grid grid-cols-1 md:grid-cols-3 gap-4">${i.map(wd).join("")}</div></div>`;e.innerHTML+=s("designated",fe.designated,t),e.innerHTML+=s("other",fe.other,n),a[0].members.length>0&&(e.innerHTML+=s("excluded",{name:"別便",memo:""},a)),Gs()}function wd(e){var r,o;`${e.id}`;let t="",n="",a="";const s=`swap-car-${e.id}`;if(e.id==="excluded-car"){t=`<div class="p-4 border-b bg-gray-100 flex-shrink-0"><h4 class="font-bold text-lg text-gray-700">合計: ${e.members.length}名</h4></div>`,a=e.members.map((d,l)=>{var f;if(!d)return"";const c=(((f=et.get(d.id))==null?void 0:f.memo)||"").trim(),m=d.isFlagTarget&&(d.grade||d.school||d.other)?[d.grade,d.school,d.other].filter(Boolean).join(" "):"",u=`seat-${e.id}-${d.id}`;return`<li class="p-2 bg-gray-100 rounded shadow-sm flex items-center justify-between">
            ...
            <li class="p-2 ${m?"bg-blue-100":"bg-red-50"} rounded shadow-sm flex items-center">
                 ...
                 <label for="${y}" class="flex flex-col min-w-0 ${m?"text-blue-800":"text-red-700"}">
                    ...
                    ${b?`<span class="text-xs ${m?"text-blue-600":"text-red-600"} ml-2 break-words">[${b}]</span>`:""}
        ...
        </div>`;for(let p=0;p<e.capacity;p++){const g=e.members[p];if(g){const v=(((o=et.get(g.id))==null?void 0:o.memo)||"").trim(),x=g.isFlagTarget&&(g.grade||g.school||g.other)?[g.grade,g.school,g.other].filter(Boolean).join(" "):"",h=`seat-${e.id}-${g.id}`;a+=`<li class="p-2 bg-gray-100 rounded shadow-sm flex items-center justify-between">
                                    <div class="flex items-center min-w-0">
                                ...
                                </li>`}else{const v=`seat-${e.id}-empty-${p}`;a+=`<li class="p-2 bg-gray-50 rounded shadow-sm flex items-center">
                                    <input type="checkbox" id="${v}" data-swap-type="seat" data-car-id="${e.id}" data-participant-id="empty" data-is-driver="false" data-slot-index="${p}" class="mr-3 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                    <label for="${v}" class="text-gray-400 italic">-- 空席 --</label>
                                </li>`}}}return`<div class="bg-white border rounded-lg shadow-md car-dropzone flex flex-col">${t}${n}<ul id="members-dropzone-${e.id}" class="p-4 space-y-2 min-h-[50px] members-dropzone flex-grow overflow-y-auto">${a}</ul></div>`}function Ed(e){const t=e.target;if(t.type!=="checkbox"||!t.dataset.swapType)return;const n=t.dataset.swapType;if(gt[n]&&gt[n].el===t){gt[n]=null,t.closest("div").classList.remove("swap-selected");return}const a={carId:t.dataset.carId,pid:t.dataset.participantId,isD:t.dataset.isDriver==="true",idx:parseInt(t.dataset.slotIndex),el:t};if(t.closest("div").classList.add("swap-selected"),n==="car"&&gt.seat||n==="seat"&&gt.car)return t.checked=!1,Ve("車と席の混在不可","error");if(!gt[n]){gt[n]=a;return}const s=gt[n],r=a;if(n==="car"){const o=K.find(d=>d.id===s.carId),i=K.find(d=>d.id===r.carId);if(o&&i)if(o.assignedParking===i.assignedParking){const d=K.indexOf(o),l=K.indexOf(i);d>-1&&l>-1&&([K[d],K[l]]=[K[l],K[d]])}else{const d=o.assignedParking;o.assignedParking=i.assignedParking,i.assignedParking=d}}else{const o=K.find(c=>c.id===s.carId),i=K.find(c=>c.id===r.carId),d=s.pid==="empty"?null:s.isD?o.driver:o.members[s.idx],l=r.pid==="empty"?null:r.isD?i.driver:i.members[r.idx];s.isD?o.driver=l:o.id!=="excluded-car"?o.members[s.idx]=l:(o.members=o.members.filter(c=>c&&c.id!==(d==null?void 0:d.id)),l&&o.members.push(l)),r.isD?i.driver=d:i.id!=="excluded-car"?i.members[r.idx]=d:(i.members=i.members.filter(c=>c&&c.id!==(l==null?void 0:l.id)),d&&i.members.push(d)),[o,i].forEach(c=>{if(c.id!=="excluded-car")for(c.members=c.members.filter(m=>m);c.members.length<c.capacity;)c.members.push(null);else c.members=c.members.filter(m=>m)})}ya(),Fn()}function Fn(){const e=document.getElementById("text-output");if(!e)return;if(K.length===0){e.value="";return}const t=fe.groundName?`@${fe.groundName}`:"";let n=[`${ke.date} ${ke.name}${t}
`];ke.timeline&&n.push(ke.timeline+`
`);const a=o=>`${o.name.replace(/の車|家の車/g,"カー")} (${o.driver?o.driver.name:"未定"}, ${o.members.filter(i=>i).map(i=>(i.type==="選手"?"★":"")+i.name).join(", ")}${o.hasLuggage?", 荷物":""})`;n.push(`◆${fe.designated.name}
${fe.designated.memo}`),K.filter(o=>o.assignedParking==="designated").forEach(o=>n.push("・"+a(o)));const s=K.filter(o=>o.assignedParking==="other");s.length>0&&(n.push(`
◆${fe.other.name}
${fe.other.memo}`),s.forEach(o=>n.push("・"+a(o))));const r=K.find(o=>o.id==="excluded-car");r&&r.members.length>0&&(n.push(`
◆別便`),r.members.filter(o=>o).forEach(o=>n.push("・"+(o.type==="選手"?"★":"")+o.name))),ke.notes&&n.push(`
◆その他
`+ke.notes),e.value=n.join(`
`)}function _d(){const e=document.getElementById("text-output");e&&(navigator.clipboard.writeText(e.value),Ve("コピーしました","success"))}function Id(){const e=document.getElementById("toggle-details-button"),t=e.textContent==="すべて開く";We.querySelectorAll("details").forEach(n=>n.open=t),e.textContent=t?"すべて閉じる":"すべて開く"}function _r(){var e,t,n,a,s,r,o,i,d,l;return ke={date:((e=document.getElementById("event-date"))==null?void 0:e.value)||"",name:((t=document.getElementById("event-name"))==null?void 0:t.value)||"",timeline:((n=document.getElementById("event-timeline"))==null?void 0:n.value)||"",notes:((a=document.getElementById("event-notes"))==null?void 0:a.value)||""},fe={groundName:((s=document.getElementById("ground-name"))==null?void 0:s.value)||"",designated:{name:((r=document.getElementById("parking-designated-name"))==null?void 0:r.value)||"指定駐車場",limit:parseInt((o=document.getElementById("parking-designated-limit"))==null?void 0:o.value)||999,memo:((i=document.getElementById("parking-designated-memo"))==null?void 0:i.value)||""},other:{name:((d=document.getElementById("parking-other-name"))==null?void 0:d.value)||"指定以外",memo:((l=document.getElementById("parking-other-memo"))==null?void 0:l.value)||""}},{selectedParticipantIds:Array.from(jt),selectedCarIds:Array.from(Ht),selectedDrivers:Array.from(Et.entries()),selectedLuggage:Array.from(Nt),excludedParticipantIds:Array.from(Xe),participantData:Array.from(et.entries()),currentAssignments:K,parkingInfo:fe,eventInfo:ke}}function Ir(e){var t,n,a,s,r;jt=new Set(e.selectedParticipantIds||[]),Ht=new Set(e.selectedCarIds||[]),Et=new Map(e.selectedDrivers||[]),Nt=new Set(e.selectedLuggage||[]),Xe=new Set(e.excludedParticipantIds||[]),et=new Map(e.participantData||[]),fe=e.parkingInfo||{groundName:"",designated:{name:"",limit:0,memo:""},other:{name:"",memo:""}},ke=e.eventInfo||{date:"",name:"",timeline:"",notes:""},document.getElementById("event-date")&&(document.getElementById("event-date").value=ke.date||""),document.getElementById("event-name")&&(document.getElementById("event-name").value=ke.name||""),document.getElementById("event-timeline")&&(document.getElementById("event-timeline").value=ke.timeline||""),document.getElementById("event-notes")&&(document.getElementById("event-notes").value=ke.notes||""),document.getElementById("ground-name")&&(document.getElementById("ground-name").value=fe.groundName||""),document.getElementById("parking-designated-name")&&(document.getElementById("parking-designated-name").value=((t=fe.designated)==null?void 0:t.name)||""),document.getElementById("parking-designated-limit")&&(document.getElementById("parking-designated-limit").value=((n=fe.designated)==null?void 0:n.limit)||""),document.getElementById("parking-designated-memo")&&(document.getElementById("parking-designated-memo").value=((a=fe.designated)==null?void 0:a.memo)||""),document.getElementById("parking-other-name")&&(document.getElementById("parking-other-name").value=((s=fe.other)==null?void 0:s.name)||""),document.getElementById("parking-other-memo")&&(document.getElementById("parking-other-memo").value=((r=fe.other)==null?void 0:r.memo)||""),K=e.currentAssignments||[],ya(),Fn()}function $d(){const e=new Blob([JSON.stringify(_r(),null,2)],{type:"application/json"}),t=document.createElement("a");t.href=URL.createObjectURL(e),t.download="state.json",t.click()}function kd(e){const t=e.target.files[0];if(!t)return;const n=new FileReader;n.onload=a=>{Ir(JSON.parse(a.target.result)),fa(),ba(),Rn()},n.readAsText(t)}async function Ld(){const e=document.getElementById("saved-state-select"),t=e.value;let n=`${document.getElementById("event-date").value}_${document.getElementById("event-name").value}`,a=!1,s=null;if(t){const r=e.options[e.selectedIndex];confirm(`現在「${r.text}」が選択されています。
このデータに上書き保存しますか？
（「キャンセル」を選ぶと新規保存になります）`)&&(a=!0,n=r.text,s=t)}if(!a){const r=prompt("保存名",n);if(!r)return;n=r}try{a&&s&&await J.deleteState(s),await J.saveState(_r(),n),await ps();const r=document.getElementById("saved-state-select");for(let o=0;o<r.options.length;o++)if(r.options[o].text===n){r.selectedIndex=o;break}Ve(a?"上書き保存しました":"保存しました","success")}catch{Ve("保存に失敗しました","error")}}async function Sd(){const e=document.getElementById("saved-state-select").value;if(!e)return;const t=await J.getState(e);t&&(Ir(t.state),fa(),ba(),Rn(),Ve("読込完了","success"))}async function Bd(){const e=document.getElementById("saved-state-select").value;e&&(await J.deleteState(e),await ps(),Ve("削除完了","success"))}async function ps(){const e=await J.getAllSavedStates(),t=document.getElementById("saved-state-select");t&&(t.innerHTML='<option value="">作業一覧...</option>'+e.map(n=>`<option value="${n.id}">${n.name}</option>`).join(""))}async function Td(){const e=document.getElementById("saved-parking-select"),t=e.value;let n=document.getElementById("ground-name").value||"新規駐車場",a=!1,s=null;if(t){const o=e.options[e.selectedIndex];confirm(`現在「${o.text}」が選択されています。
このデータに上書き保存しますか？
（「キャンセル」を選ぶと新規保存になります）`)&&(a=!0,n=o.text,s=t)}if(!a){const o=prompt("駐車場保存名",n);if(!o)return;n=o}const r={groundName:document.getElementById("ground-name").value,designated:{name:document.getElementById("parking-designated-name").value,limit:parseInt(document.getElementById("parking-designated-limit").value),memo:document.getElementById("parking-designated-memo").value},other:{name:document.getElementById("parking-other-name").value,memo:document.getElementById("parking-other-memo").value}};try{a&&s&&await J.deleteParking(s),await J.saveParking(r,n),await gs();const o=document.getElementById("saved-parking-select");for(let i=0;i<o.options.length;i++)if(o.options[i].text===n){o.selectedIndex=i;break}Ve(a?"駐車場を上書き保存しました":"駐車場を保存しました","success")}catch{Ve("保存に失敗しました","error")}}async function Ad(){const e=document.getElementById("saved-parking-select").value;if(!e)return;const t=await J.getParking(e);t&&(document.getElementById("ground-name").value=t.parking.groundName,document.getElementById("parking-designated-name").value=t.parking.designated.name,document.getElementById("parking-designated-limit").value=t.parking.designated.limit,document.getElementById("parking-designated-memo").value=t.parking.designated.memo,document.getElementById("parking-other-name").value=t.parking.other.name,document.getElementById("parking-other-memo").value=t.parking.other.memo,Ve("駐車場読込完了","success"))}async function Cd(){const e=document.getElementById("saved-parking-select").value;e&&(await J.deleteParking(e),await gs(),Ve("駐車場削除完了","success"))}async function gs(){const e=await J.getAllSavedParking(),t=document.getElementById("saved-parking-select");t&&(t.innerHTML='<option value="">駐車場一覧...</option>'+e.map(n=>`<option value="${n.id}">${n.name}</option>`).join(""))}async function Dd(){confirm("全データをリセットしますか？")&&(await J.clearDatabase(),location.reload())}window.clearCurrentInputs=function(){if(!confirm(`現在の入力内容（イベント情報、選択メンバー、車、駐車場、配車結果など）をすべてクリアして、初期状態に戻しますか？
（※データベースに保存されているマスタデータや過去の保存データは削除されません）`))return;const e=document.getElementById("event-date");e&&(e.value="");const t=document.getElementById("event-name");t&&(t.value="");const n=document.getElementById("event-timeline");n&&(n.value="");const a=document.getElementById("event-remarks");a&&(a.value="");const s=document.getElementById("ground-name");s&&(s.value="");const r=document.getElementById("parking-name");r&&(r.value="");const o=document.getElementById("parking-limit");o&&(o.value="99");const i=document.getElementById("parking-remarks");i&&(i.value="");const d=document.getElementById("other-parking-name");d&&(d.value="");const l=document.getElementById("other-parking-remarks");l&&(l.value=""),jt.clear(),Ht.clear(),Et.clear(),Nt.clear(),Xe.clear(),et.clear(),Er(),K=[],fa(),ba(),Rn(),ya(),Fn(),window.att_swapSelectedMember=null,window.att_swapSelectedCar=null,Ve("画面の入力をクリアしました。","success")};function Ve(e,t="info"){const n=document.getElementById("dispatch-message");let a="bg-blue-100 text-blue-700 border-blue-200";t==="error"?a="bg-red-100 text-red-700 border-red-200":t==="success"?a="bg-green-100 text-green-700 border-green-200":t==="warning"&&(a="bg-yellow-100 text-yellow-800 border-yellow-200"),n.className=`p-4 h-full border rounded-lg ${a}`,document.getElementById("dispatch-message-text").innerHTML=e,n.classList.remove("hidden"),La&&clearTimeout(La),t!=="warning"&&(La=setTimeout(fs,5e3))}function fs(){document.getElementById("dispatch-message").classList.add("hidden")}function Nd(){if(!K||K.length===0)return!1;for(const t of K){if(t.id==="excluded-car")continue;if((t.driver?1:0)+t.members.filter(a=>a!==null).length>t.baseCapacity)return!0}const e=K.find(t=>t.id==="excluded-car");return e?e.members.some(t=>t&&!Xe.has(t.id)):!1}function Gs(){if(Nd())Ve("定員オーバーです。車の台数が足りないか、定員を超過している車があります。","warning");else{const e=document.getElementById("dispatch-message"),t=document.getElementById("dispatch-message-text");e&&!e.classList.contains("hidden")&&t&&t.innerHTML.includes("定員オーバー")&&fs()}}function Pd(){var t,n,a,s,r,o,i;const e=document.querySelector("#view-master h2");if(e&&!document.getElementById("btn-back-to-dispatch-master")){const d=document.createElement("button");d.id="btn-back-to-dispatch-master",d.className="bg-blue-50 text-blue-600 border border-blue-200 hover:bg-blue-100 px-4 py-2 rounded shadow font-bold text-sm mb-4 flex items-center w-fit",d.innerHTML='<span class="mr-1">◀</span> 配車調整に戻る',d.addEventListener("click",()=>{var l;return(l=document.getElementById("nav-dispatch"))==null?void 0:l.click()}),e.parentElement.insertBefore(d,e)}(t=document.getElementById("add-family-button"))==null||t.addEventListener("click",Md),Qe==null||Qe.addEventListener("click",Rd),Qe==null||Qe.addEventListener("input",qd),Qe==null||Qe.addEventListener("change",Fd),(n=document.getElementById("add-car-button"))==null||n.addEventListener("click",jd),ft==null||ft.addEventListener("click",Hd),ft==null||ft.addEventListener("input",Ud),(a=document.getElementById("add-parking-button-master"))==null||a.addEventListener("click",Vd),Bt==null||Bt.addEventListener("click",Gd),Bt==null||Bt.addEventListener("input",Wd),(s=document.getElementById("export-master-button"))==null||s.addEventListener("click",zd),(r=document.getElementById("import-master-input"))==null||r.addEventListener("change",Jd),(o=document.getElementById("master-message-close"))==null||o.addEventListener("click",$r),(i=document.getElementById("save-master-db-button"))==null||i.addEventListener("click",Od)}async function Od(){const e=document.getElementById("save-master-db-button");try{e.textContent="保存中...",await J.syncAllMaster(),Ka("マスターデータをサーバーに保存しました","success"),Fe=await J.getAllSavedParking()||[],Sn()}catch(t){Ka("保存に失敗しました: "+t.message,"error")}finally{e.textContent="マスターデータを保存 (更新)"}}function Jt(){Qe&&(Qe.innerHTML="",re.sort((e,t)=>(e.order??99)-(t.order??99)).forEach((e,t)=>{const n=e.members.map(a=>`<div class="p-2 border rounded bg-gray-50 member-grid"><input data-id="${a.id}" data-f="name" value="${a.name}" class="col-span-3 md:col-span-1 p-1 border"><select data-id="${a.id}" data-f="type" class="col-span-2 md:col-span-1 p-1 border"><option ${a.type==="選手"?"selected":""}>選手</option><option ${a.type==="保護者"?"selected":""}>保護者</option><option ${a.type==="兄弟"?"selected":""}>兄弟</option><option ${a.type==="その他"?"selected":""}>その他</option></select><input data-id="${a.id}" data-f="data.grade" value="${a.data.grade||""}" placeholder="学年" class="col-span-1 border"><input data-id="${a.id}" data-f="data.school" value="${a.data.school||""}" placeholder="学校" class="col-span-2 md:col-span-1 border"><input data-id="${a.id}" data-f="data.other" value="${a.data.other||""}" placeholder="他" class="col-span-2 md:col-span-1 border"><input data-id="${a.id}" data-f="data.memo" value="${a.data.memo||""}" placeholder="備考" class="col-span-2 md:col-span-1 border"><div class="col-span-3 md:col-span-1 flex items-center justify-between"><label class="text-xs"><input type="checkbox" data-id="${a.id}" data-f="isFlagTarget" ${a.isFlagTarget?"checked":""}>同乗優先</label><button data-action="del-m" data-id="${a.id}" class="bg-red-500 text-white px-2 py-1 rounded text-xs">削</button></div></div>`).join("");Qe.innerHTML+=`<div class="bg-white border rounded shadow" data-fname="${e.familyName}"><div class="family-header"><input data-action="ren-f" value="${e.familyName}" class="font-bold border p-1"><div class="space-x-2"><button data-action="up" class="bg-gray-400 text-white px-2 rounded text-xs">▲</button><button data-action="down" class="bg-gray-400 text-white px-2 rounded text-xs">▼</button><button data-action="del-f" class="bg-red-500 text-white px-2 py-1 rounded text-xs">家族削除</button></div></div><div class="p-3 space-y-2">${n}</div><button data-action="add-m" class="ml-3 mb-3 bg-blue-500 text-white px-2 py-1 rounded text-xs">＋メンバー</button></div>`}))}function Md(){const e=prompt("家族名");e&&(J.addFamily({familyName:e,order:99,members:[{id:"p"+Date.now(),name:"新規",type:"選手",isFlagTarget:!0,data:{}}]}),Jt())}async function Rd(e){const t=e.target,n=t.dataset.action,a=t.closest("[data-fname]");if(!a)return;const s=a.dataset.fname;if(n==="del-f"&&confirm("削除?")&&(J.deleteFamily(s),Jt()),n==="add-m"){const r=await J.getFamily(s);r.members.push({id:"p"+Date.now(),name:"新規",type:"保護者",isFlagTarget:!1,data:{}}),J.updateFamily(r),Jt()}if(n==="del-m"){const r=await J.getFamily(s);r.members=r.members.filter(o=>o.id!==t.dataset.id),J.updateFamily(r),Jt()}if(n==="up"||n==="down"){re.sort((i,d)=>(i.order??99)-(d.order??99)),re.forEach((i,d)=>{i.order=d});const r=re.findIndex(i=>i.familyName===s),o=n==="up"?r-1:r+1;if(o>=0&&o<re.length){const i=re[r].order;re[r].order=re[o].order,re[o].order=i,Jt()}}}function Fd(e){const t=e.target,n=t.closest("[data-fname]");if(!n)return;const a=n.dataset.fname;if(t.dataset.action==="ren-f"){const s=t.value.trim();if(s&&s!==a){if(re.find(i=>i.familyName===s)){alert(`家族名「${s}」は既に存在します。別の名前を入力してください。`),t.value=a;return}const o=re.find(i=>i.familyName===a);if(o){o.familyName=s,n.dataset.fname=s;let i=!1;oe.forEach(d=>{d.familyName===a&&(d.familyName=s,i=!0)}),i&&fn()}}else s||(t.value=a)}}async function qd(e){const t=e.target;if(t.dataset.action==="ren-f")return;const n=t.dataset.id,a=t.dataset.f,s=t.closest("[data-fname]").dataset.fname;if(!n||!a)return;const r=await J.getFamily(s),o=r.members.find(d=>d.id===n),i=t.type==="checkbox"?t.checked:t.value;a.startsWith("data.")?o.data[a.split(".")[1]]=i:o[a]=i,J.updateFamily(r)}function fn(){ft&&(ft.innerHTML="",oe.sort((e,t)=>(e.order??99)-(t.order??99)).forEach(e=>{ft.innerHTML+=`<div class="p-3 border rounded bg-gray-50 flex flex-wrap gap-2 items-center" data-cid="${e.id}"><button data-act="up" class="bg-gray-400 text-white px-2 py-1 text-xs">▲</button><button data-act="down" class="bg-gray-400 text-white px-2 py-1 text-xs">▼</button><input data-f="name" value="${e.name}" class="p-1 border text-sm w-32"><input data-f="familyName" value="${e.familyName}" class="p-1 border text-sm w-32"><input type="number" data-f="baseCapacity" value="${e.baseCapacity}" class="p-1 border text-sm w-16"><button data-act="del" class="bg-red-500 text-white px-2 py-1 rounded text-xs">削</button></div>`}))}function jd(){J.addCar({id:"c"+Date.now(),name:"新規車",familyName:"",baseCapacity:5,order:99}),fn()}function Hd(e){const t=e.target,n=t.dataset.act,a=t.closest("[data-cid]");if(!a)return;const s=a.dataset.cid;if(n==="del"&&(J.deleteCar(s),fn()),n==="up"||n==="down"){oe.sort((i,d)=>(i.order??99)-(d.order??99)),oe.forEach((i,d)=>{i.order=d});const r=oe.findIndex(i=>i.id===s),o=n==="up"?r-1:r+1;if(o>=0&&o<oe.length){const i=oe[r].order;oe[r].order=oe[o].order,oe[o].order=i,fn()}}}async function Ud(e){const t=e.target,n=t.dataset.f,a=t.closest("[data-cid]").dataset.cid;if(!n)return;const s=await J.getCar(a);s[n]=t.type==="number"?parseInt(t.value):t.value,J.updateCar(s)}function ha(){Bt.innerHTML="",Fe.forEach(e=>{Bt.innerHTML+=`
        ...
        </div>`})}function Vd(){J.addParkingMaster({groundName:"",designated:{name:"",limit:0,memo:""},other:{name:"",memo:""}},"新規P"),ha()}function Gd(e){e.target.dataset.act==="del"&&(J.deleteParkingMaster(e.target.closest("[data-pid]").dataset.pid),ha())}function Wd(e){const t=e.target,n=t.dataset.f,a=t.closest("[data-pid]").dataset.pid;if(!n)return;const s=Fe.find(o=>o.id===a),r=t.type==="number"?parseInt(t.value):t.value;if(n==="name")s.name=r;else{const o=n.split(".");o.length===2?s[o[0]][o[1]]=r:s[o[0]][o[1]][o[2]]=r}J.updateParkingMaster(s)}async function zd(){const e={families:re,cars:oe,parking:await J.getAllSavedParking()},t=document.createElement("a");t.href=URL.createObjectURL(new Blob([JSON.stringify(e)],{type:"application/json"})),t.download="master.json",t.click()}function Jd(e){const t=e.target.files[0];if(!t)return;const n=new FileReader;n.onload=a=>{const s=JSON.parse(a.target.result);if(J.bulkAddFamilies(s.families),J.bulkAddCars(s.cars),s.parking){Fe.forEach(r=>In.add(r.id)),Fe=[];for(let r of s.parking)Fe.push({id:r.id||"p"+Date.now()+Math.floor(Math.random()*1e3),name:r.name,timestamp:r.timestamp||Date.now(),parking:r.parking,isNew:!0})}Jt(),fn(),ha(),Ka("読込完了 (※まだ保存されていません。保存ボタンを押してください)","info")},n.readAsText(t)}function Ka(e,t="info"){const n=document.getElementById("master-message");n.className=`p-4 mb-4 border rounded-lg ${t==="error"?"bg-red-100 text-red-700":t==="success"?"bg-green-100 text-green-700":"bg-blue-100 text-blue-700"}`,document.getElementById("master-message-text").innerHTML=e,n.classList.remove("hidden"),Sa&&clearTimeout(Sa),Sa=setTimeout($r,5e3)}function $r(){document.getElementById("master-message").classList.add("hidden")}let _t={type:"",add:[],update:[],delete:[]};function kr(e,t,n,a,s,r=""){_t={type:e,add:t,update:n,delete:a},document.getElementById("csv-confirm-title").textContent=s,document.getElementById("csv-add-count").textContent=t.length,document.getElementById("csv-update-count").textContent=n.length,document.getElementById("csv-delete-count").textContent=a.length;const o=document.getElementById("csv-confirm-warning");r?(o.innerHTML=r,o.classList.remove("hidden")):o.classList.add("hidden"),document.getElementById("tab-csv-add").onclick=()=>rn("add"),document.getElementById("tab-csv-update").onclick=()=>rn("update"),document.getElementById("tab-csv-delete").onclick=()=>rn("delete"),t.length>0?rn("add"):n.length>0?rn("update"):rn("delete"),document.getElementById("csv-confirm-modal").classList.remove("hidden")}function ia(){document.getElementById("csv-confirm-modal").classList.add("hidden");const e=document.getElementById("input-import-users-csv");e&&(e.value="");const t=document.getElementById("input-import-events-csv");t&&(t.value="")}function rn(e){["add","update","delete"].forEach(n=>{const a=document.getElementById(`tab-csv-${n}`);n===e?(a.classList.add("text-blue-600","border-blue-600"),a.classList.remove("text-gray-500","border-transparent")):(a.classList.remove("text-blue-600","border-blue-600"),a.classList.add("text-gray-500","border-transparent"))}),Yd(e)}function Yd(e){const t=document.getElementById("csv-confirm-content"),n=_t[e];if(!n||n.length===0){t.innerHTML='<p class="text-gray-500 p-4">対象のデータはありません。</p>';return}let a="";if(_t.type==="users"){const s=e==="add",r=e==="update";a+=`
...
`)||t.includes("\r")?'"'+t.replace(/"/g,'""')+'"':t}async function Kd(){U("メンバー情報をエクスポート中...");try{const{data:e}=await S.from("app_users").select("*").order("created_at",{ascending:!1}),{data:t}=await S.from("user_groups").select("*"),{data:n}=await S.from("groups").select("*"),{data:a}=await S.from("user_attributes").select("*"),s=new Map(n.map(f=>[f.id,f.name])),r=new Map(a.map(f=>[f.id,f.name])),o=new Map(t.map(f=>[f.user_email,f.group_id]));let i=window.adminDelegations;if(!i){const{data:f}=await S.from("master_data").select("*").eq("key","ATTENDANCE_DELEGATIONS").single();i=f&&f.data?f.data:{},window.adminDelegations=i}const l=[["メールアドレス","氏名","役割","所属グループ名","ユーザー属性名","配車利用可(1/0)","成績利用可(1/0)","出欠利用可(1/0)","シミュレータ利用可(1/0)","Info利用可(1/0)","代行入力先(カンマ区切りメールアドレス)","代行専用(1/0)","初期パスワード","削除(1/0)"]];e.forEach(f=>{const b=f.role==="admin"?"管理者":f.role==="leader"?"リーダー":"一般ユーザー",y=o.get(f.email),p=y&&s.get(y)||"",g=f.attribute_id&&r.get(f.attribute_id)||"",v=i[f.email]?i[f.email].join(","):"",x=f.email.endsWith("@local.dummy");l.push([f.email,f.name||"",b,p,g,f.can_use_dispatch!==!1?"1":"0",f.can_use_dashboard!==!1?"1":"0",f.can_use_attendance!==!1?"1":"0",f.can_use_simulator!==!1?"1":"0",f.can_use_info!==!1?"1":"0",v,x?"1":"0","","0"])});const c=l.map(f=>f.map(bs).join(",")).join(`
`),m=new Blob([new Uint8Array([239,187,191]),c],{type:"text/csv;charset=utf-8;"}),u=document.createElement("a");u.href=URL.createObjectURL(m),u.download=`members_${new Date().toISOString().split("T")[0]}.csv`,u.click()}catch(e){console.error(e),alert("エクスポートに失敗しました: "+e.message)}finally{F()}}async function Qd(e){const t=e.target.files[0];if(!t)return;U("CSVファイルを解析中...");const n=new FileReader;n.onload=async a=>{try{const s=a.target.result,r=Lr(s);if(r.length<2){alert("有効なデータがありません。"),F();return}const o=r[0].map(B=>B.trim()),i=o.indexOf("メールアドレス"),d=o.indexOf("氏名"),l=o.indexOf("役割"),c=o.indexOf("所属グループ名"),m=o.indexOf("ユーザー属性名"),u=o.findIndex(B=>B.includes("配車")),f=o.findIndex(B=>B.includes("成績")),b=o.findIndex(B=>B.includes("出欠")),y=o.findIndex(B=>B.includes("シミュレータ")),p=o.findIndex(B=>B.includes("Info")||B.includes("インフォ")),g=o.findIndex(B=>B.includes("代行入力先")),v=o.findIndex(B=>B.includes("代行専用")),x=o.findIndex(B=>B.includes("初期パスワード")),h=o.findIndex(B=>B.includes("削除"));if(i===-1||d===-1){alert("「メールアドレス」および「氏名」列は必須です。"),F();return}const{data:k}=await S.from("app_users").select("*"),{data:L}=await S.from("user_groups").select("*"),{data:w}=await S.from("groups").select("*"),{data:I}=await S.from("user_attributes").select("*");let E=window.adminDelegations;if(!E){const{data:B}=await S.from("master_data").select("*").eq("key","ATTENDANCE_DELEGATIONS").single();E=B&&B.data?B.data:{},window.adminDelegations=E}const $=new Map(w.map(B=>[B.name,B.id])),_=new Map(I.map(B=>[B.name,B.id])),C=new Map(k.map(B=>[B.email,B])),P=new Map(L.map(B=>[B.user_email,B.group_id])),D=[],O=[],q=[];for(let B=1;B<r.length;B++){const A=r[B];if(A.length<2)continue;const N=(A[d]||"").trim();if(!N)continue;let T=(A[i]||"").trim();const M=v!==-1?A[v]==="1"||A[v]==="true"||A[v]==="代行専用":!1,W=T.endsWith("@local.dummy"),j=M||W;if(T)j&&!W&&(T.includes("@")?T=T.split("@")[0]+"@local.dummy":T=T+"@local.dummy");else if(j)T=`dummy_${Date.now()}_${B}@local.dummy`;else continue;T=pa(T);const H=(A[l]||"").trim();let R="user";H==="管理者"||H==="admin"?R="admin":(H==="リーダー"||H==="leader")&&(R="leader");const z=c!==-1?(A[c]||"").trim():"",le=z&&$.get(z)||null,te=m!==-1?(A[m]||"").trim():"",be=te&&_.get(te)||null,Ee=u!==-1?!(A[u]==="0"||A[u]==="false"):!0,Lt=f!==-1?!(A[f]==="0"||A[f]==="false"):!0,dt=b!==-1?!(A[b]==="0"||A[b]==="false"):!0,ne=y!==-1?!(A[y]==="0"||A[y]==="false"):!0,Je=p!==-1?!(A[p]==="0"||A[p]==="false"):!0,Ce=h!==-1?A[h]==="1"||A[h]==="削除":!1;let ut=null;g!==-1&&(ut=(A[g]||"").split(",").map(ce=>ce.trim()).filter(Boolean));let mt="",lt=null;if(!j){const G=x!==-1?(A[x]||"").trim():"";if(C.has(T))G.length>=6&&(lt=G);else if(G.length>=6)mt=G;else{const ce="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";let Re="pw-";for(let Ge=0;Ge<6;Ge++)Re+=ce.charAt(Math.floor(Math.random()*ce.length));mt=Re}}const Ye={email:T,name:N,role:R,group_id:le,group_name:z,attribute_id:be,attribute_name:te,can_use_dispatch:Ee,can_use_dashboard:Lt,can_use_attendance:dt,can_use_simulator:ne,can_use_info:Je,delegations:ut,is_dummy:j,initial_password:lt!==null?lt:mt},De=C.get(T);if(De)if(Ce)q.push(Ye);else{const G=P.get(T)!==le,ce=De.role!==R,Re=De.name!==N,Ge=De.attribute_id!==be,Ut=De.can_use_dispatch!==!1!==Ee,Sr=De.can_use_dashboard!==!1!==Lt,Br=De.can_use_attendance!==!1!==dt,Tr=De.can_use_simulator!==!1!==ne,Ar=De.can_use_info!==!1!==Je;let ys=!1;if(ut!==null){const Cr=E[T]||[],Dr=[...ut].sort().join(","),Nr=[...Cr].sort().join(",");Dr!==Nr&&(ys=!0)}let hs=!1;lt!==null&&(hs=!0),(G||ce||Re||Ge||Ut||Sr||Br||Tr||Ar||ys||hs)&&O.push(Ye)}else Ce||(Ye.delegations===null&&(Ye.delegations=[]),D.push(Ye))}F(),kr("users",D,O,q,"メンバーCSVインポート確認")}catch(s){console.error(s),alert("CSVの解析に失敗しました: "+s.message),F()}},n.readAsText(t)}async function Zd(){_t.type==="users"?await Xd():_t.type==="events"&&typeof window.executeEventsImport=="function"&&await window.executeEventsImport()}async function Xd(){U("インポートデータを保存中...");try{const{add:e,update:t,delete:n}=_t;let a=window.adminDelegations;if(!a){const{data:d}=await S.from("master_data").select("*").eq("key","ATTENDANCE_DELEGATIONS").single();a=d&&d.data?d.data:{},window.adminDelegations=a}if(n.length>0){const d=n.map(c=>c.email),{error:l}=await S.from("app_users").delete().in("email",d);if(l)throw l;d.forEach(c=>{delete window.adminDelegations[c]}),Object.keys(window.adminDelegations).forEach(c=>{window.adminDelegations[c]=(window.adminDelegations[c]||[]).filter(m=>!d.includes(m))}),await ee("IMPORT_USERS_DELETE",`${n.length}件のユーザーをインポートで削除しました`)}if(e.length>0){const d=[];for(let u of e)if(!u.is_dummy)try{const f=window.supabase.createClient(cs,us,{auth:{persistSession:!1,autoRefreshToken:!1}}),{error:b}=await f.auth.signUp({email:u.email,password:u.initial_password});if(b)throw new Error(`Auth作成失敗: ${b.message}`)}catch(f){console.error(`Sign up error for ${u.email}:`,f),d.push(`${u.email}: ${f.message}`)}if(d.length>0&&!confirm(`一部のアカウント払い出し（Auth）に失敗しました。データベース登録を続行しますか？

エラー内容:
${d.join(`
`)}`))throw new Error("インポート処理を中断しました。");const l=e.map(u=>({email:u.email,name:u.name,role:u.role,attribute_id:u.attribute_id,can_use_dispatch:u.can_use_dispatch,can_use_dashboard:u.can_use_dashboard,can_use_attendance:u.can_use_attendance,can_use_simulator:u.can_use_simulator,can_use_info:u.can_use_info})),{error:c}=await S.from("app_users").insert(l);if(c)throw c;const m=e.filter(u=>u.group_id).map(u=>({user_email:u.email,group_id:u.group_id}));if(m.length>0){const{error:u}=await S.from("user_groups").insert(m);if(u)throw u}e.forEach(u=>{u.delegations&&(window.adminDelegations[u.email]=u.delegations)}),await ee("IMPORT_USERS_ADD",`${e.length}件のユーザーをインポートで追加しました`)}if(t.length>0){const d=[];for(let l of t){if(l.initial_password)try{const{error:u}=await S.rpc("admin_update_user_password",{user_email:l.email,new_password:l.initial_password});if(u)throw u;await ee("ADMIN_CHANGE_PASSWORD",`ユーザー「${l.email}」のパスワードをインポートで変更しました`)}catch(u){console.error(`Failed to update password for ${l.email}:`,u),d.push(`${l.email}: ${u.message}`)}const{error:c}=await S.from("app_users").update({name:l.name,role:l.role,attribute_id:l.attribute_id,can_use_dispatch:l.can_use_dispatch,can_use_dashboard:l.can_use_dashboard,can_use_attendance:l.can_use_attendance,can_use_simulator:l.can_use_simulator,can_use_info:l.can_use_info}).eq("email",l.email);if(c)throw c;const{data:m}=await S.from("user_groups").select("*").eq("user_email",l.email);if(l.group_id)if(m&&m.length>0){const{error:u}=await S.from("user_groups").update({group_id:l.group_id}).eq("user_email",l.email);if(u)throw u}else{const{error:u}=await S.from("user_groups").insert([{user_email:l.email,group_id:l.group_id}]);if(u)throw u}else if(m&&m.length>0){const{error:u}=await S.from("user_groups").delete().eq("user_email",l.email);if(u)throw u}l.delegations!==null&&(window.adminDelegations[l.email]=l.delegations)}d.length>0&&alert(`一部のパスワード更新に失敗しました。詳細は開発者コンソールを確認してください。

失敗したユーザー:
${d.join(`
`)}`),await ee("IMPORT_USERS_UPDATE",`${t.length}件のユーザーをインポートで更新しました`)}const{error:s}=await S.from("master_data").upsert({key:"ATTENDANCE_DELEGATIONS",data:window.adminDelegations});if(s)throw s;let r="インポートが完了しました。";const o=e.filter(d=>!d.is_dummy),i=t.filter(d=>d.initial_password);(o.length>0||i.length>0)&&(r+=`
...
※上記情報をコピーし、対象のユーザーへお伝えください。`),alert(r),ia(),await ae()}catch(e){console.error(e),alert("保存に失敗しました: "+e.message)}finally{F()}}window.showCSVConfirmModal=kr;window.closeCSVConfirmModal=ia;window.parseCSV=Lr;window.escapeCSV=bs;function el(){const n=[["メールアドレス","氏名","役割","所属グループ名","ユーザー属性名","配車利用可(1/0)","成績利用可(1/0)","出欠利用可(1/0)","代行入力先(カンマ区切りメールアドレス)","代行専用(1/0)","初期パスワード","削除(1/0)"],["sample_parent@example.com","山田 太郎","一般ユーザー","選手・保護者","A軍","1","1","1","sample_child1@example.com,sample_child2@example.com","0","tempPw123","0"],["sample_child1@example.com","山田 一郎","一般ユーザー","選手・保護者","A軍","1","0","1","","0","tempPw123","0"],["sample_child2@example.com","山田 二郎","一般ユーザー","選手・保護者","A軍","1","0","1","","0","tempPw123","0"],["","代行専用の子ども","一般ユーザー","選手・保護者","A軍","0","0","1","","1","","0"]].map(r=>r.map(bs).join(",")).join(`
`),a=new Blob([new Uint8Array([239,187,191]),n],{type:"text/csv;charset=utf-8;"}),s=document.createElement("a");s.href=URL.createObjectURL(a),s.download="members_sample.csv",s.click()}const tl=Object.freeze(Object.defineProperty({__proto__:null,get csvImportState(){return _t},get currentUser(){return V},get currentUserRole(){return he},forceHideLoading:qe,goToUsersAdmin:wr,hideLoading:F,logAction:ee,openChangePasswordModal:ms,showLoading:U,supabaseClient:S,switchAuthScreen:Y,withLoading:ye},Symbol.toStringTag,{value:"Module"}));
