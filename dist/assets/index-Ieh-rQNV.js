(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))a(s);new MutationObserver(s=>{for(const r of s)if(r.type==="childList")for(const o of r.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&a(o)}).observe(document,{childList:!0,subtree:!0});function n(s){const r={};return s.integrity&&(r.integrity=s.integrity),s.referrerPolicy&&(r.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?r.credentials="include":s.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function a(s){if(s.ep)return;s.ep=!0;const r=n(s);fetch(s.href,r)}})();let Wt=new Date,Xe=[],He=[],Me=[],Za=[],da=[],st=[],Ie=[],Ot=[],Ct=[],vs=!1,Ta=[],Mt=[],ce=[];window.att_multiSelectMode=!1;window.att_selectedDates=new Set;window.att_selectedCategories=new Set;window.att_selectedGroups=new Set;function xn(e){let t=e.target_group_ids||[];if(t.length===0&&e.target_group_id&&(t=[e.target_group_id]),t.length===0)return{ids:[],name:"全体",color:"#e5e7eb"};const n=He.filter(a=>t.includes(a.id));return n.length===0?{ids:[],name:"全体",color:"#e5e7eb"}:{ids:t,name:n.map(a=>a.name).join(", "),color:n[0].color||"#e5e7eb",groups:n}}function zs(e){return e.ids.length===0?'<span class="text-xs border border-gray-300 text-gray-800 px-2 py-0.5 rounded shadow-sm" style="background-color: #e5e7eb">全体</span>':e.groups.map(t=>`<span class="text-xs border border-gray-300 text-gray-800 px-2 py-0.5 rounded shadow-sm" style="background-color: ${t.color||"#e5e7eb"}">${t.name}</span>`).join(" ")}function Js(e){if(!e)return"-";try{const t=new Date(e);if(isNaN(t.getTime()))return"-";const n=t.getMonth()+1,a=t.getDate(),s=String(t.getHours()).padStart(2,"0"),r=String(t.getMinutes()).padStart(2,"0");return`${n}/${a} ${s}:${r}`}catch{return"-"}}async function Aa(){try{vs||(jr(),vs=!0),await nn(),Ue(),Hr(),Ur()}catch(e){console.error("Attendance App Init Error:",e),typeof je=="function"&&je()}}function jr(){var n,a,s,r,o,i,d,l,c,m,u,f,b,y,g,p,v,x,h,k,L,w,I,E,$;const e=document.getElementById("btn-logout-att");if(e&&!document.getElementById("btn-change-password-att")){const _=document.createElement("button");_.id="btn-change-password-att",_.className="text-xs md:text-sm bg-gray-500 hover:bg-gray-600 text-white px-2 py-1 md:px-3 md:py-1 rounded shadow mr-2 font-bold",_.textContent="パスワード変更",_.onclick=()=>ps(),e.parentNode.insertBefore(_,e)}(n=document.getElementById("tab-calendar"))==null||n.addEventListener("click",()=>ws("calendar")),(a=document.getElementById("tab-list"))==null||a.addEventListener("click",()=>{ws("list"),Lt()}),(s=document.getElementById("cal-prev-month"))==null||s.addEventListener("click",()=>{Wt.setMonth(Wt.getMonth()-1),Ue()}),(r=document.getElementById("cal-next-month"))==null||r.addEventListener("click",()=>{Wt.setMonth(Wt.getMonth()+1),Ue()}),(o=document.getElementById("cal-today"))==null||o.addEventListener("click",()=>{Wt=new Date,Ue()}),(i=document.getElementById("btn-add-event"))==null||i.addEventListener("click",()=>Cn()),(d=document.getElementById("btn-export-events"))==null||d.addEventListener("click",io),(l=document.getElementById("btn-import-events"))==null||l.addEventListener("click",()=>document.getElementById("input-import-events-csv").click()),(c=document.getElementById("btn-import-ics-events"))==null||c.addEventListener("click",()=>document.getElementById("input-import-events-ics").click()),(m=document.getElementById("btn-download-events-sample"))==null||m.addEventListener("click",uo),(u=document.getElementById("input-import-events-csv"))==null||u.addEventListener("change",lo),(f=document.getElementById("input-import-events-ics"))==null||f.addEventListener("change",po),(b=document.getElementById("btn-close-ics-modal-x"))==null||b.addEventListener("click",Ca),(y=document.getElementById("btn-close-ics-modal"))==null||y.addEventListener("click",Ca),(g=document.getElementById("btn-execute-ics-import"))==null||g.addEventListener("click",fo),(p=document.getElementById("ics-import-list"))==null||p.addEventListener("change",_=>{if(_.target&&_.target.classList.contains("ics-row-location-select")){const C=_.target,O=C.closest(".ics-row").querySelector(".ics-row-location");C.value&&C.value!=="custom"?O.value=C.value:C.value==="custom"&&(O.value="",O.focus())}});const t=document.getElementById("btn-group-manage");t&&(t.style.display="none"),(v=document.getElementById("btn-toggle-multiselect"))==null||v.addEventListener("click",function(){window.att_multiSelectMode=!window.att_multiSelectMode,window.att_multiSelectMode||window.att_selectedDates.clear(),this.classList.toggle("bg-blue-600",window.att_multiSelectMode),this.classList.toggle("text-white",window.att_multiSelectMode),this.classList.toggle("bg-blue-50",!window.att_multiSelectMode),this.classList.toggle("text-blue-600",!window.att_multiSelectMode),Ue(),window.att_updateMultiselectBar()}),(x=document.getElementById("btn-cal-filter"))==null||x.addEventListener("click",Es),(h=document.getElementById("btn-cal-filter-clear"))==null||h.addEventListener("click",xa),(k=document.getElementById("btn-list-filter"))==null||k.addEventListener("click",Es),(L=document.getElementById("btn-list-filter-clear"))==null||L.addEventListener("click",xa),(w=document.getElementById("btn-close-filter-modal-x"))==null||w.addEventListener("click",Yn),(I=document.getElementById("btn-close-filter-modal"))==null||I.addEventListener("click",Yn),(E=document.getElementById("btn-apply-filter"))==null||E.addEventListener("click",Vr),($=document.getElementById("btn-filter-clear"))==null||$.addEventListener("click",xa)}function ws(e){const t=document.getElementById("tab-calendar"),n=document.getElementById("tab-list");e==="calendar"?(document.getElementById("calendar-container").classList.remove("hidden"),document.getElementById("list-container").classList.add("hidden"),t.classList.replace("bg-white","bg-green-600"),t.classList.replace("text-green-600","text-white"),n.classList.replace("bg-green-600","bg-white"),n.classList.replace("text-white","text-green-600")):(document.getElementById("calendar-container").classList.add("hidden"),document.getElementById("list-container").classList.remove("hidden"),n.classList.replace("bg-white","bg-green-600"),n.classList.replace("text-green-600","text-white"),t.classList.replace("bg-green-600","bg-white"),t.classList.replace("text-white","text-green-600"))}async function nn(){H("イベント・出欠データ読み込み中...");try{const{data:e}=await S.from("groups").select("*").order("sort_order",{ascending:!0}).order("created_at",{ascending:!0});e&&(He=e);const{data:t}=await S.from("event_categories").select("*").order("sort_order",{ascending:!0}).order("created_at",{ascending:!0});t&&t.length>0?Me=t:Me=[{id:"1",name:"練習"},{id:"2",name:"試合"},{id:"3",name:"イベント"}];const n=new Date,a=new Date(n.getFullYear(),n.getMonth()-6,1),s=new Date(n.getFullYear(),n.getMonth()+7,1),{data:r}=await S.from("events").select("*").gte("start_time",a.toISOString()).lt("start_time",s.toISOString()).order("start_time");r&&(Xe=r);const{data:o}=await S.from("app_users").select("email, name, attribute_id, can_use_attendance");o&&(Ot=o);const{data:i}=await S.from("user_attributes").select("*").order("sort_order",{ascending:!0}).order("created_at",{ascending:!0});i&&(Ct=i);try{const{data:c}=await S.from("event_locations").select("*").order("sort_order",{ascending:!0}).order("created_at",{ascending:!0});c&&(Mt=c)}catch(c){console.warn("event_locations table might not exist yet:",c),Mt=[]}const{data:d}=await S.from("user_groups").select("*");d&&(st=d,j&&(Za=d.filter(c=>c.user_email===j.email)));let l=[];if(Xe&&Xe.length>0){const c=Xe.map(u=>u.id),{data:m}=await S.from("attendances").select("*").in("event_id",c);m&&(l=m)}Ie=l,j&&(da=l.filter(c=>c.user_email===j.email));try{const{data:c}=await S.from("master_data").select("data").eq("key","ATTENDANCE_DELEGATIONS").single();c&&c.data&&j&&(Ta=c.data[j.email]||[])}catch{Ta=[]}}catch(e){console.error("Attendance DB Error:",e)}finally{typeof je=="function"?je():q()}}function Hr(){const e=document.getElementById("filter-group");e&&(e.innerHTML='<option value="">すべてのグループ</option>'+He.map(t=>`<option value="${t.id}">${t.name}</option>`).join(""))}function Ur(){const e=document.getElementById("filter-category");if(!e)return;const t=e.value;e.innerHTML='<option value="">すべてのカテゴリ</option>'+Me.map(n=>`<option value="${n.name}">${n.name}</option>`).join(""),Me.some(n=>n.name===t)&&(e.value=t)}function Es(){const e=document.getElementById("filter-modal-content");if(!e)return;let t="";t+='<div class="mb-5">',t+='<h4 class="text-xs font-bold text-gray-400 tracking-wider uppercase mb-2">カテゴリ（複数選択）</h4>',Me.length===0?t+='<p class="text-xs text-gray-400">カテゴリが登録されていません</p>':(t+='<div class="grid grid-cols-2 gap-2">',Me.forEach(a=>{const s=window.att_selectedCategories.has(a.name)?"checked":"";t+=`
    ...
    `,He.length>0&&He.forEach(a=>{const s=window.att_selectedGroups.has(a.id)?"checked":"";t+=`
                <label class="flex items-center space-x-2 text-sm text-gray-700 cursor-pointer hover:bg-gray-50 p-1.5 rounded transition">
                ...
                <div class="flex space-x-2">
                    <div class="w-1/2">
                        ...
                        <select id="ev-category" class="w-full border p-1.5 rounded text-xs bg-white">
                            ${Me.map(T=>`<option value="${T.name}" ${T.name===l?"selected":""}>${T.name}</option>`).join("")}
                        </select>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                        ...
                        <div>
                            ...
                            <select class="ics-row-category w-full border p-2 rounded-lg font-semibold text-sm bg-white focus:ring-2 focus:ring-blue-500">
                                ${Me.map(p=>`<option value="${p.name}" ${p.name===l?"selected":""}>${p.name}</option>`).join("")}
                            </select>
                        </div>
                        <div>
...
${$.myTeam} vs ${$.oppTeam}`:""},label:function(I){const E=I.dataIndex,$=k[E],_=I.dataset.label||"";return _==="得点"?`得点 (${($==null?void 0:$.myTeam)||"自チーム"}): ${I.raw} 点`:_==="失点"?`失点 (${($==null?void 0:$.oppTeam)||"相手チーム"}): ${Math.abs(I.raw)} 点`:_==="累積勝率(%)"?`累積勝率: ${I.raw} %`:`${_}: ${I.raw}`}}}},scales:{x:{stacked:!0,ticks:{font:{size:10}}},y:{stacked:!0,position:"left"},y1:{position:"right",beginAtZero:!0,min:0,max:100}}}})}function It(e){let t=0,n=0,a=0,s=0,r=0,o=0,i=0,d=0,l=0,c=0,m=0,u=0,f=0,b=0,y=0;e.forEach(w=>{const I=w.plate_appearances||0,E=w.at_bats||0,$=w.hits||0,_=w.doubles||0,C=w.triples||0,O=w.home_runs||0,P=w.walks||0,M=w.hit_by_pitch||0,F=w.strike_outs||w.strikeouts||0,D=w.sacrifice_flies||w.sac_flies||0,B=w.sacrifice_hits||w.sac_bunts||0,A=w.runs_batted_in||0,T=w.runs||0,N=w.stolen_bases||0;t+=I,n+=E,a+=$,u+=_,f+=C,m+=O,d+=P,l+=M,c+=F,b+=D,y+=B,r+=A,o+=T,i+=N;const U=(w.singles!==void 0?w.singles:Math.max(0,$-_-C-O))*1+_*2+C*3+O*4,W=w.total_bases&&w.total_bases>U?w.total_bases:U;s+=W});const g=n>0?a/n:0,p=n+d+l+b,v=p>0?(a+d+l)/p:t>0?(a+d+l)/t:0,x=n>0?s/n:0,h=v+x,k=a+d+l,L=k>0?o/k:0;return{pa:t,ab:n,h:a,doubles:u,triples:f,hr:m,tb:s,sf:b,sh:y,bb:d,hbp:l,rbi:r,r:o,sb:i,so:c,ob:k,avg:g,obp:v,slg:x,ops:h,runRate:L,avgStr:g.toFixed(3).replace(/^0/,""),obpStr:v.toFixed(3).replace(/^0/,""),slgStr:x.toFixed(3).replace(/^0/,""),opsStr:h.toFixed(3),runRateStr:k>0?(L*100).toFixed(1)+"%":"0.0%",bbRate:t>0?(d+l)/t:0,soRate:t>0?c/t:0}}function $t(e){let t=0,n=0,a=0,s=0,r=0,o=0,i=0,d=0,l=0,c=0;e.forEach(g=>{t+=g.outs||0,n+=g.earned_runs||0,a+=g.hits_allowed||0,s+=(g.walks_allowed||0)+(g.hit_batters||0),r+=g.strike_outs||0,o+=g.batters_faced||0,i+=g.pitch_count||0,d+=g.strikes||0,l+=g.wins||0,c+=g.losses||0});const m=t/3,u=m>0?n*7/m:0,f=m>0?(a+s)/m:0,b=m>0?r*7/m:0,y=m>0?s*7/m:0;return{outs:t,er:n,h:a,bb:s,so:r,bf:o,pc:i,st:d,wins:l,losses:c,era:u,whip:f,k7:b,bb7:y,eraStr:u.toFixed(2),whipStr:f.toFixed(2),k7Str:b.toFixed(2),bb7Str:y.toFixed(2),kRate:o>0?r/o:0,bbRate:o>0?s/o:0,kbb:s>0?r/s:r>0?99.9:0,sRate:i>0?d/i:0}}async function ts(){var d,l,c,m,u,f;await Nn();const e=((d=document.getElementById("ps-mode"))==null?void 0:d.value)||"single",t=(l=document.getElementById("ps-player"))==null?void 0:l.value,n=((c=document.getElementById("ps-role"))==null?void 0:c.value)||"batter",a=((m=document.getElementById("ps-limit-games"))==null?void 0:m.value)||"5",s=((u=document.getElementById("ps-ma-unit"))==null?void 0:u.value)||"ab",r=parseInt(((f=document.getElementById("ps-ma-window"))==null?void 0:f.value)||"10",10),o=document.getElementById("ps-single-container"),i=document.getElementById("ps-all-container");e==="single"?(o==null||o.classList.remove("hidden"),i==null||i.classList.add("hidden"),t&&Io(t,n,a,s,r)):(o==null||o.classList.add("hidden"),i==null||i.classList.remove("hidden"),Bo(n,a,s,r))}function Io(e,t,n,a,s){var y;const{games:r,bStats:o,pStats:i}=vn,d=((y=ot.find(g=>g.id==e))==null?void 0:y.name)||"選手",c=(t==="batter"?o:i).filter(g=>g.player_id==e).map(g=>{const p=r.find(v=>v.id===g.game_id);return{date:(p==null?void 0:p.date)||"",game:p,stats:g}}).filter(g=>g.date).sort((g,p)=>g.date.localeCompare(p.date)),m=t==="batter"?It(c.map(g=>g.stats)):$t(c.map(g=>g.stats));let u=[...c];if(n!=="all"){const g=parseInt(n,10);u.length>g&&(u=u.slice(u.length-g))}const f=t==="batter"?It(u.map(g=>g.stats)):$t(u.map(g=>g.stats));$o(d,t,f,m,u);const b=sr(u,t,a,s);ko(d,t,u),Lo(d,t,b,a,s),So(t,u)}function $o(e,t,n,a,s){const r=document.getElementById("ps-highlight-cards");if(r)if(t==="batter"){const o=n.avg-a.avg,i=(o>=0?"+":"")+o.toFixed(3).replace(/^0/,""),d=o>=.03,l=o<=-.05,c=d?"🔥 好調":l?"❄️ 不調":"⚖️ 安定",m=d?"bg-red-100 text-red-700 border-red-200":l?"bg-blue-100 text-blue-700 border-blue-200":"bg-gray-100 text-gray-700 border-gray-200";let u=0;for(let f=s.length-1;f>=0;f--){const b=s[f].stats.hits||0,y=s[f].stats.at_bats||0;if(b>0)u++;else if(y>0)break}r.innerHTML=`
        ...
        `,a.innerHTML=r.map(o=>{var F,D,B;const i=o.stats,d=mt((F=o.game)==null?void 0:F.team_first)?(D=o.game)==null?void 0:D.team_second:(B=o.game)==null?void 0:B.team_first,l=i.plate_appearances||0,c=i.at_bats||0,m=i.hits||0,u=i.doubles||0,f=i.triples||0,b=i.home_runs||0,y=i.singles!==void 0?i.singles:Math.max(0,m-u-f-b),g=i.runs_batted_in||0,p=i.runs||0,v=i.walks||0,x=i.hit_by_pitch||0,h=i.strikeouts||i.strike_outs||0,k=i.sacrifice_flies||i.sac_flies||0,L=(i.sacrifice_hits||i.sac_bunts||0)+k,w=i.stolen_bases||0,I=i.errors||0,E=c>0?(m/c).toFixed(3).replace(/^0/,""):".000",$=c+v+x+k,_=$>0?(m+v+x)/$:l>0?(m+v+x)/l:0;_.toFixed(3).replace(/^0/,"");const C=y+u*2+f*3+b*4,O=i.total_bases&&i.total_bases>C?i.total_bases:C,P=c>0?O/c:0;P.toFixed(3).replace(/^0/,"");const M=(_+P).toFixed(3);return`
...
詳細: `+e.message)}}async function Fo(){De=[{id:"team_default",name:"ありんこアントス (A軍)"},{id:"team_b",name:"ありんこアントス (B軍・ジュニア)"}],Ae=De[0].id,wt=[],fe=[{id:"p1",name:"とあ",number:"2",teamId:"team_default"},{id:"p2",name:"そうま",number:"10",teamId:"team_default"},{id:"p3",name:"あきと",number:"3",teamId:"team_default"},{id:"p4",name:"ゆうき",number:"4",teamId:"team_default"},{id:"p5",name:"あいのすけ",number:"1",teamId:"team_default"},{id:"p6",name:"けんせい",number:"6",teamId:"team_default"},{id:"p7",name:"りゅうと",number:"7",teamId:"team_default"},{id:"p8",name:"ながまさ",number:"8",teamId:"team_default"},{id:"p9",name:"そうすけ",number:"9",teamId:"team_default"},{id:"p10",name:"たいち",number:"5",teamId:"team_default"}],Ve=[]}async function as(){return null}async function rr(){return null}async function la(e,t=!1){return null}async function Kn(e,t=!1){return null}async function qe(e){e&&(await Kn(),wn())}function qt(){const e=document.getElementById("sim-team-select"),t=document.getElementById("new-sim-team"),n=document.getElementById("sim-current-team-label"),a=an();n&&(n.textContent=`[${a.name}]`),e&&(e.innerHTML="",De.forEach(s=>{const r=document.createElement("option");r.value=s.id,r.textContent=s.name,e.appendChild(r)}),e.value=Ae),t&&(t.innerHTML="",De.forEach(s=>{const r=document.createElement("option");r.value=s.id,r.textContent=s.name,t.appendChild(r)}),t.value=Ae)}function ca(){const e=document.getElementById("sim-template-select-inline"),t=document.getElementById("new-sim-template");e&&(e.innerHTML='<option value="">(選択して適用)</option>',wt.forEach(n=>{const a=document.createElement("option");a.value=n.id,a.textContent=n.name,e.appendChild(a)})),t&&(t.innerHTML='<option value="">(未配置から開始)</option>',wt.forEach(n=>{const a=document.createElement("option");a.value=n.id,a.textContent=n.name,t.appendChild(a)}))}async function jo(e="",t=null,n=null,a=null){const s="pat_"+Date.now(),r=t||Ae,o=De.find(u=>u.id===r)||an();let i={},d={},l=a!==null?a:Z;if(n){const u=wt.find(f=>f.id===n);u&&(i=JSON.parse(JSON.stringify(u.basePositions||{})),d=JSON.parse(JSON.stringify(u.battingOrder||{})),l=u.mode||l)}Z=l,Ae=r;const c={id:s,name:e||"新規データ",teamId:r,mode:Z,basePositions:i,customSubstitutions:[],battingOrder:d,headerInfo:{date:"",tournament:"",teamHome:o.name||"ありんこアントス",teamVisitor:"",manager:"",captain:"",scorer:"",stadium:"",time:""}};Ve.push(c),Re=s,await qe(c),qt(),Ft(),wn();const m=document.getElementById("sim-pattern-select");m&&(m.value=s)}function wn(){const e=document.getElementById("sim-pattern-select");e&&(e.innerHTML='<option value="">選択してください...</option>',Ve.forEach(t=>{const n=document.createElement("option");n.value=t.id;const a=t.isSynced===!1?" (未同期)":"",s=De.find(o=>o.id===t.teamId),r=s?`[${s.name}] `:"";n.textContent=r+t.name+a,e.appendChild(n)}),Re&&(e.value=Re))}function Ft(){const e=document.getElementById("btn-sim-mode-9"),t=document.getElementById("btn-sim-mode-10");Z===9?(e.className="px-3 py-1.5 text-xs font-bold bg-amber-600 text-white transition",t.className="px-3 py-1.5 text-xs font-bold bg-white text-gray-700 border-l hover:bg-gray-50 transition"):(e.className="px-3 py-1.5 text-xs font-bold bg-white text-gray-700 transition",t.className="px-3 py-1.5 text-xs font-bold bg-amber-600 text-white border-l hover:bg-gray-50 transition")}function On(e){const t={...e.basePositions||{}},n=Z===9?it:dt;return Object.keys(t).forEach(s=>{n.includes(s)||delete t[s]}),(e.customSubstitutions||[]).filter(s=>s.active).forEach(s=>{if(s.type==="swap"){const{pos1:r,pos2:o}=s.details;if(n.includes(r)&&n.includes(o)){const i=t[r];t[r]=t[o],t[o]=i}}else if(s.type==="sub"){const{outPlayerId:r,inPlayerId:o,pos:i}=s.details;if(r){const d=Object.keys(t).find(l=>t[l]===r);d&&(t[d]=o)}else i&&n.includes(i)&&(t[i]=o)}else if(s.type==="rotation"){const o=s.details.positions.filter(i=>n.includes(i));if(o.length>1){const i=o.map(d=>t[d]);for(let d=0;d<o.length;d++){const l=i[(d-1+o.length)%o.length];t[o[d]]=l}}}}),t}function Mn(e){const t=new Set;return(e.customSubstitutions||[]).filter(a=>a.active).forEach(a=>{a.type==="sub"&&a.details.outPlayerId&&t.add(a.details.outPlayerId)}),t}function fn(e){e.battingOrder||(e.battingOrder={});const t=Z===9?it:dt,n=e.basePositions||{},a=new Set;t.forEach(i=>{n[i]&&a.add(n[i])}),Object.keys(e.battingOrder).forEach(i=>{const d=e.battingOrder[i];a.has(d)||delete e.battingOrder[i]});const s=new Set(Object.values(e.battingOrder)),r=[];a.forEach(i=>{s.has(i)||r.push(i)});const o=t.length;for(let i=1;i<=o&&r.length!==0;i++)if(!e.battingOrder[i]){const d=r.shift();e.battingOrder[i]=d}}function ue(){const e=ve();if(!e)return;const t=On(e),n=Mn(e),a=new Set(Object.values(t).filter(Boolean));fn(e),Ho(a,n),Uo(a,n),Vo(t,e),Go(e),Wo(e),zo(e)}function Ho(e,t){const n=document.getElementById("sim-players-list"),a=document.getElementById("sim-player-count");if(!n)return;n.innerHTML="";const s=Pn();if(a&&(a.textContent=`${s.length} 人`),s.length===0){n.innerHTML='<span class="text-xs text-gray-400 p-2">このチームに登録されている選手がいません。</span>';return}s.forEach(r=>{const o=e.has(r.id),i=t.has(r.id),d=Ee===r.id&&ge==="players-list",l=document.createElement("div");l.className=`sim-player-badge ${o?"assigned":""} ${i?"retired":""} ${d?"selected":""}`,l.setAttribute("data-player-id",r.id),!o&&!i&&(l.setAttribute("draggable","true"),l.addEventListener("dragstart",or));const c=r.number?`#${r.number} `:"";l.innerHTML=`
        ...
        `,n.appendChild(l)})}function Go(e){const t=document.getElementById("sim-batting-order-list");if(!t)return;t.innerHTML="";const n=Z===9?it:dt,a=n.length,s=e.basePositions||{},r={};n.forEach(o=>{const i=s[o];i&&(r[i]=o)});for(let o=1;o<=a;o++){const i=e.battingOrder[o],d=i?fe.find(g=>g.id===i):null,l=d?r[d.id]:null,c=l?ht[l]:"未配置",m=document.createElement("div");d?m.className="flex items-center justify-between bg-amber-50/50 border border-amber-100 rounded-lg p-2 text-xs transition-colors duration-150":m.className="flex items-center justify-between bg-gray-50/50 border border-dashed border-gray-200 rounded-lg p-2 text-xs text-gray-400 transition-colors duration-150";const u=document.createElement("div");u.className="flex items-center gap-1 shrink-0";const f=document.createElement("button");f.className="px-2 py-1 bg-amber-100 hover:bg-amber-200 disabled:opacity-30 disabled:cursor-not-allowed text-amber-900 font-bold rounded text-[10px] leading-none transition shadow-sm",f.textContent="▲",o===1&&(f.disabled=!0),f.addEventListener("click",()=>Ts(o,"up"));const b=document.createElement("button");b.className="px-2 py-1 bg-amber-100 hover:bg-amber-200 disabled:opacity-30 disabled:cursor-not-allowed text-amber-900 font-bold rounded text-[10px] leading-none transition shadow-sm",b.textContent="▼",o===a&&(b.disabled=!0),b.addEventListener("click",()=>Ts(o,"down")),u.appendChild(f),u.appendChild(b);let y="";if(d){const g=d.number?`#${d.number} `:"";y=`
            ...
            `;m.innerHTML=y,m.appendChild(u),t.appendChild(m)}}async function Ts(e,t){const n=ve();if(!n)return;n.battingOrder||(n.battingOrder={});const a=t==="up"?e-1:e+1,r=(Z===9?it:dt).length;if(a<1||a>r)return;const o=n.battingOrder[e],i=n.battingOrder[a];o&&i?(n.battingOrder[e]=i,n.battingOrder[a]=o):o?(n.battingOrder[a]=o,delete n.battingOrder[e]):i&&(n.battingOrder[e]=i,delete n.battingOrder[a]),await qe(n),ue()}function ss(e,t,n){if(!e)return{code:"",desc:"",fullDesc:""};if(e.type==="sub"){const{outPlayerId:a,inPlayerId:s}=e.details,r=fe.find(y=>y.id===a),o=fe.find(y=>y.id===s),i=r?r.name:"不明",d=o?o.name:"不明",l=Object.keys(t).find(y=>t[y]===a),c=l&&cn[l]||"",m=l?ht[l]||"":"選手",u=c?`交代 (${c})`:"交代",f=n?`<span class="font-bold text-amber-900">${pe(i)}</span>に代わって<span class="font-bold text-amber-900">${pe(d)}</span>`:`${i}に代わって${d}`,b=`${m}の${i}に代わりまして、${d}が入ります。`;return{code:u,desc:f,fullDesc:b}}else if(e.type==="rotation"){const a=e.details.positions||[],r=a.map(l=>cn[l]||l).join("-"),o=[];for(let l=0;l<a.length-1;l++){const c=a[l],m=a[l+1],u=t[c],f=fe.find(b=>b.id===u);if(f){const b=n?`<span class="font-bold text-amber-900">${pe(f.name)}</span>`:f.name;o.push(`${b}が${ht[m]}`)}}const i=o.join("、"),d=i?`${i}へ。`:"ポジション交代";return{code:r,desc:i,fullDesc:d}}else if(e.type==="swap"){const{pos1:a,pos2:s}=e.details,r=t[a],o=t[s],i=fe.find(v=>v.id===r),d=fe.find(v=>v.id===o),l=cn[a]||a,c=cn[s]||s,m=ht[a]||"",u=ht[s]||"",f=i?i.name:"未配置",b=d?d.name:"未配置",y=`${l}⇔${c}`,g=n?`<span class="font-bold text-amber-900">${pe(f)}</span>と<span class="font-bold text-amber-900">${pe(b)}</span>の入れ替え`:`${f}と${b}の入れ替え`,p=`${m}の${f}と${u}の${b}が入れ替わります。`;return{code:y,desc:g,fullDesc:p}}return{code:"",desc:"",fullDesc:""}}function Wo(e){const t=document.getElementById("sim-sub-rules-list");if(!t)return;t.innerHTML="";const n=e.customSubstitutions||[];if(n.length===0){t.innerHTML='<p class="text-xs text-gray-400 text-center py-4">登録された交代はありません。</p>';return}const a=Z===9?it:dt;let s={...e.basePositions||{}};n.forEach(r=>{const o=ss(r,s,!0);if(r.active){if(r.type==="swap"){const{pos1:l,pos2:c}=r.details;if(a.includes(l)&&a.includes(c)){const m=s[l];s[l]=s[c],s[c]=m}}else if(r.type==="sub"){const{outPlayerId:l,inPlayerId:c,pos:m}=r.details;if(l){const u=Object.keys(s).find(f=>s[f]===l);u&&(s[u]=c)}else m&&a.includes(m)&&(s[m]=c)}else if(r.type==="rotation"){const c=r.details.positions.filter(m=>a.includes(m));if(c.length>1){const m=c.map(u=>s[u]);for(let u=0;u<c.length;u++){const f=m[(u-1+c.length)%c.length];s[c[u]]=f}}}}const i=document.createElement("div");i.className=`sub-rule-card ${r.active?"active":""}`;const d=r.name?`<span class="text-[10px] font-bold text-amber-800 bg-amber-50 px-1.5 py-0.5 rounded leading-none inline-block mb-1 border border-amber-200/50">${pe(r.name)}</span>`:"";i.innerHTML=`
            <div class="flex items-center gap-2">
        ...
        `,t.appendChild(d)})}function Ne(){const e=ve();if(!e)return;const t=Z===9?it:dt,n=document.getElementById("sub-player-out"),a=document.getElementById("sub-player-in"),s=On(e),r=new Set(Object.values(s).filter(Boolean)),o=Mn(e);if(n&&(n.innerHTML="",t.forEach(d=>{var c;const l=s[d];if(l){const m=((c=fe.find(f=>f.id===l))==null?void 0:c.name)||"未配置",u=document.createElement("option");u.value=l,u.textContent=`${m} (${ht[d]})`,n.appendChild(u)}})),a){a.innerHTML="";const l=Pn().filter(c=>!r.has(c.id)&&!o.has(c.id));l.length===0?a.innerHTML='<option value="">控え選手なし</option>':l.forEach(c=>{const m=document.createElement("option");m.value=c.id,m.textContent=c.name,a.appendChild(m)})}const i=document.getElementById("rot-input-text");i&&(i.value="")}async function Jo(e,t){const n=ve();if(!n)return;const a=n.customSubstitutions.find(s=>s.id===e);a&&(a.active=t,await qe(n),Ne(),ue())}async function Yo(){const e=ve();if(!e)return;const t=document.getElementById("rule-type-select"),n=t?t.value:"rotation";let a={};if(n==="sub"){const d=document.getElementById("sub-player-out").value,l=document.getElementById("sub-player-in").value;if(!d){alert("退く選手を選択してください。");return}if(!l){alert("入る控え選手を選択してください。");return}a={outPlayerId:d,inPlayerId:l}}else if(n==="rotation"){const d=document.getElementById("rot-input-text"),l=d?d.value.trim():"";if(!l){alert("交代ルートを入力してください（例: 1-3-1）。");return}if(!/^[0-9a-zA-Z]+(-[0-9a-zA-Z]+)*$/.test(l)){alert("入力形式が正しくありません。半角数字・英字とハイフンで入力してください（例: 1-3-1）。");return}const c=l.split("-").filter(Boolean);if(c.length<2){alert("交代には最低2つのポジションが必要です（例: 1-3）。");return}const m=Z===9?it:dt,u=[];for(const f of c){const b=qo[f];if(!b){alert(`無効なポジション番号「${f}」が含まれています。1〜9またはDHを指定してください。`);return}if(!m.includes(b)){alert(`ポジション「${f}」(${ht[b]||b})は、現在の守備モード（DH${Z===10?"あり":"なし"}）では使用できません。`);return}u.push(b)}for(let f=0;f<u.length-1;f++)if(u[f]===u[f+1]){alert("隣り合うポジションに同じものを指定することはできません（例: 1-1-3 は不可）。");return}a={positions:u}}const s=document.getElementById("rule-name-input"),r=s?s.value.trim():"",o={id:"rule_"+Date.now(),name:r,type:n,active:!1,details:a};e.customSubstitutions||(e.customSubstitutions=[]),e.customSubstitutions.push(o),await qe(e),s&&(s.value="");const i=document.getElementById("rot-input-text");i&&(i.value=""),Ne(),ue(),Qn("subrules")}async function Ko(e){const t=ve();t&&(t.customSubstitutions=t.customSubstitutions.filter(n=>n.id!==e),await qe(t),Ne(),ue())}function Qn(e){const t=document.getElementById("tab-btn-setup"),n=document.getElementById("tab-btn-subrules"),a=document.getElementById("panel-setup"),s=document.getElementById("panel-subrules");e==="setup"?(t.className="flex-1 py-2 px-3 text-center text-sm font-bold bg-amber-600 text-white transition",n.className="flex-1 py-2 px-3 text-center text-sm font-bold bg-white text-gray-700 border-l hover:bg-gray-50 transition",a.classList.remove("hidden"),s.classList.add("hidden")):(t.className="flex-1 py-2 px-3 text-center text-sm font-bold bg-white text-gray-700 transition",n.className="flex-1 py-2 px-3 text-center text-sm font-bold bg-amber-600 text-white border-l hover:bg-gray-50 transition font-bold",a.classList.add("hidden"),s.classList.remove("hidden"))}function or(e){const t=e.currentTarget.getAttribute("data-player-id");e.dataTransfer.setData("text/plain",t);let n="bench";e.currentTarget.parentNode.id==="sim-players-list"&&(n="players-list"),e.dataTransfer.setData("source-pos",n)}function Qo(e){e.preventDefault(),e.currentTarget.classList.add("drag-over")}function Zo(e){e.currentTarget.classList.remove("drag-over")}function Xo(e){e.preventDefault(),e.currentTarget.classList.remove("drag-over");const t=e.dataTransfer.getData("text/plain"),n=e.currentTarget.getAttribute("data-position");!t||!n||ir(t,n)}async function ir(e,t){const n=ve();if(!n)return;n.basePositions||(n.basePositions={});let a=null;Object.keys(n.basePositions).forEach(r=>{n.basePositions[r]===e&&(a=r)});const s=n.basePositions[t];a&&(n.basePositions[a]=s),n.basePositions[t]=e,fn(n),await qe(n),Ee=null,ge=null,Ne(),ue()}function dr(e,t){Ee===e&&ge===t?(Ee=null,ge=null):(Ee=e,ge=t),ue()}async function ei(e){const t=ve();if(!t)return;t.basePositions||(t.basePositions={});const n=t.basePositions[e];if(Ee)if(ge!=="players-list"&&ge!=="bench"){const a=t.basePositions[ge];t.basePositions[ge]=n,t.basePositions[e]=a,fn(t),await qe(t),Ee=null,ge=null,Ne(),ue()}else ir(Ee,e);else n&&(Ee=n,ge=e,ue())}async function ti(e){const t=ve();t&&(t.basePositions||(t.basePositions={}),t.basePositions[e]=null,fn(t),await qe(t),Ee=null,ge=null,Ne(),ue())}async function ni(){const e=prompt(`新しいチーム名を入力してください:
(例: ありんこアントス (B軍), ジュニア選抜など)`);if(!e||!e.trim())return;const t=e.trim();if(De.some(a=>a.name===t)){alert("同じ名前のチームが既に存在します。");return}const n={id:"team_"+Date.now(),name:t};De.push(n),Ae=n.id,await as(),qt(),ue()}async function ai(){const e=an(),t=prompt("チーム名を変更してください:",e.name);if(!t||!t.trim())return;const n=t.trim();if(n!==e.name){if(De.some(a=>a.id!==e.id&&a.name===n)){alert("同じ名前のチームが既に存在します。");return}e.name=n,await as(),qt(),wn(),ue()}}async function si(){if(De.length<=1){alert("登録チームが1つのみのため、削除できません。");return}const e=an(),t=Pn();let n=`チーム「${e.name}」を削除しますか？`;if(t.length>0&&(n+=`
※このチームに登録されている ${t.length} 名の選手データも削除されます。`),!confirm(n))return;const a=new Set(t.map(s=>s.id));fe=fe.filter(s=>!a.has(s.id));for(const s of t)await la(s,!0);De=De.filter(s=>s.id!==e.id),Ae=De[0].id,await as(),qt(),ue()}function ri(e){Ae=e.target.value;const t=document.getElementById("sim-current-team-label"),n=an();t&&(t.textContent=`[${n.name}]`),Ee=null,ge=null,Ne(),ue()}async function oi(){const e=ve();if(!e)return;const t=e.name?`${e.name}の基本形`:"基本配置パターン",n=prompt(`初期パターン（テンプレート）の登録名を入力してください:
...
（現在のスタメン配置・打順が上書きされます）`)){const s=document.getElementById("sim-template-select-inline");s&&(s.value="");return}const n=ve();n&&(n.basePositions=JSON.parse(JSON.stringify(t.basePositions||{})),n.battingOrder=JSON.parse(JSON.stringify(t.battingOrder||{})),t.mode&&(n.mode=t.mode,Z=t.mode,Ft()),await qe(n));const a=document.getElementById("sim-template-select-inline");a&&(a.value=""),Ne(),ue()}async function di(){const e=document.getElementById("sim-template-select-inline"),t=e?e.value:"";if(!t){alert("削除したい初期パターンを選択してください。");return}const n=wt.find(a=>a.id===t);n&&confirm(`初期パターン「${n.name}」を削除しますか？`)&&(wt=wt.filter(a=>a.id!==t),await rr(),ca(),alert(`初期パターン「${n.name}」を削除しました。`))}function li(){const e=document.getElementById("sim-new-modal");if(!e)return;qt(),ca();const t=document.getElementById("new-sim-name");if(t){const r=new Date,o=`${r.getFullYear()}/${String(r.getMonth()+1).padStart(2,"0")}/${String(r.getDate()).padStart(2,"0")}`;t.value=`${o} 練習試合`}const n=document.getElementById("new-sim-team");n&&(n.value=Ae);const a=document.getElementById("new-sim-template");a&&(a.value="");const s=document.querySelector(`input[name="new-sim-dh-mode"][value="${Z}"]`);s&&(s.checked=!0),e.classList.remove("hidden")}function Ra(){const e=document.getElementById("sim-new-modal");e&&e.classList.add("hidden")}async function ci(){const e=document.getElementById("new-sim-name"),t=document.getElementById("new-sim-team"),n=document.getElementById("new-sim-template"),a=document.querySelector('input[name="new-sim-dh-mode"]:checked'),s=e?e.value.trim():"",r=t?t.value:Ae,o=n?n.value:null,i=a?parseInt(a.value,10):9;if(!s){alert("シミュレーションデータ名を入力してください。");return}await jo(s,r,o,i),Ra(),Ne(),ue()}async function Ia(){const e=document.getElementById("sim-new-player-input"),t=document.getElementById("sim-new-player-number");if(!e)return;const n=e.value.trim(),a=t?t.value.trim():"";if(!n)return;if(Pn().some(o=>o.name===n)){alert("このチームには同じフルネームの選手が既に登録されています。");return}const r={id:"p_"+Date.now()+"_"+Math.random().toString(36).substr(2,5),name:n,number:a,teamId:Ae};fe.push(r),await la(),e.value="",t&&(t.value=""),Ne(),ue()}async function ui(e){const t=fe.find(n=>n.id===e);if(t&&confirm(`選手「${t.name}」を削除しますか？
（データベースの全データ配置・交代設定からも削除されます）`)){fe=fe.filter(n=>n.id!==e),await la(t,!0);for(const n of Ve){let a=!1;if(n.basePositions||(n.basePositions={}),Object.keys(n.basePositions).forEach(s=>{n.basePositions[s]===e&&(n.basePositions[s]=null,a=!0)}),n.battingOrder&&Object.keys(n.battingOrder).forEach(s=>{n.battingOrder[s]===e&&(delete n.battingOrder[s],a=!0)}),n.customSubstitutions){const s=n.customSubstitutions.length;n.customSubstitutions=n.customSubstitutions.filter(r=>r.type==="sub"?r.details.outPlayerId!==e&&r.details.inPlayerId!==e:!0),n.customSubstitutions.length!==s&&(a=!0)}a&&await qe(n)}Ee===e&&(Ee=null,ge=null),Ne(),ue()}}async function mi(){const e=document.getElementById("sim-pattern-select");if(!e)return;const t=e.value;let n=!1,a=null,s="";const r=ve();if(t){const o=e.options[e.selectedIndex];confirm(`現在「${o.text}」が選択されています。
このデータに上書き保存しますか？
（「キャンセル」を選ぶと新規保存になります）`)&&(n=!0,s=o.text,a=t)}if(!n){const o=r?r.name:"新規配置データ",i=prompt("保存名を入力してください：",o);if(!i)return;if(s=i.trim(),!s){alert("有効な保存名を入力してください。");return}}try{if(n&&a){if(r){r.name=s,r.teamId=Ae,r.mode=Z;const i=await Kn(r);if(i)throw i;alert(`データ「${s}」を上書き保存しました。`)}}else{const i="pat_"+Date.now(),d=an(),l={id:i,name:s,teamId:Ae,mode:Z,basePositions:r?{...r.basePositions||{}}:{},customSubstitutions:r?JSON.parse(JSON.stringify(r.customSubstitutions||[])):[],battingOrder:r?{...r.battingOrder||{}}:{},headerInfo:r?JSON.parse(JSON.stringify(r.headerInfo||{})):{date:"",tournament:"",teamHome:d.name||"ありんこアントス",teamVisitor:"",manager:"",captain:"",scorer:"",stadium:"",time:""}};Ve.push(l),Re=i;const c=await Kn(l);if(c)throw c;alert(`データ「${s}」を保存しました。`)}wn();const o=document.getElementById("sim-pattern-select");o&&(o.value=Re),ue()}catch(o){console.error("Save pattern error:",o),alert(`保存に失敗しました。
...
`;else{let f={...e.basePositions||{}};m.forEach((b,y)=>{const g=ss(b,f,!1);if(r+=`${y+1}. [${g.code}] ${g.fullDesc}
`,b.type==="swap"){const{pos1:p,pos2:v}=b.details,x=f[p];f[p]=f[v],f[v]=x}else if(b.type==="sub"){const{outPlayerId:p,inPlayerId:v}=b.details,x=Object.keys(f).find(h=>f[h]===p);x&&(f[x]=v)}else if(b.type==="rotation"){const p=b.details.positions,v=p.map(x=>f[x]);for(let x=0;x<p.length;x++){const h=v[(x-1+p.length)%p.length];f[p[x]]=h}}})}const u=document.getElementById("sim-member-text");u&&(u.value=r)}function yi(){const e=document.getElementById("sim-member-text");e&&(e.select(),document.execCommand("copy"),alert("メンバー表をクリップボードにコピーしました！"))}function As(){const e=document.getElementById("sim-member-modal");e&&e.classList.add("hidden")}function hi(){const e=ve();if(!e)return;const t=On(e),n=Mn(e),a=Z===9?it:dt,s=e.headerInfo||{},r=e.basePositions||{},o={};a.forEach(b=>{const y=r[b];y&&(o[y]=b)});const i=a.length,d=[];for(let b=1;b<=i;b++){const y=e.battingOrder[b];if(y){const g=fe.find(v=>v.id===y),p=o[y];g&&p&&d.push({order:b,posNum:cn[p],name:g.name,number:g.number||""})}}for(;d.length<9;)d.push({order:d.length+1,posNum:"",name:"",number:""});const l=new Set(Object.values(t).filter(Boolean)),c=fe.filter(b=>!l.has(b.id)&&!n.has(b.id)),m=[];for(let b=0;b<6;b++){const y=b*2,g=b*2+1,p=c[y],v=c[g];m.push({leftName:p?p.name:"",leftNumber:p&&p.number||"",rightName:v?v.name:"",rightNumber:v&&v.number||""})}let u="";for(let b=1;b<=4;b++){let y="";d.forEach(x=>{y+=`
                <tr>
            ...
            `}),Z===10&&d.length<10&&(y+='<tr><td class="cell-order">10</td><td class="cell-pos"></td><td class="cell-name"></td><td class="cell-number"></td></tr>');const g=Z===10?11:10,p=d.length;for(let x=p;x<g;x++)y+='<tr><td class="cell-order"></td><td class="cell-pos"></td><td class="cell-name"></td><td class="cell-number"></td></tr>';let v="";m.forEach(x=>{v+=`
                <tr>
...
- [ ] **終了時**:
  ...
  - [ ] 倉庫施錠および学校鍵の返却`,files:[],created_by:"父母会",updated_at:"2026-04-01"}],Rs=[{id:"link-official-site",title:"Arinko Ants 公式ホームページ",url:"https://arinkoants.sakura.ne.jp/",category:"公式・連盟",description:"チームの公式Webサイト。チーム紹介、選手募集案内、活動予定などが掲載されています。",icon:"🐜",display_order:1},{id:"link-google-calendar",title:"チーム活動予定 Googleカレンダー",url:"https://calendar.google.com/",category:"スケジュール・連絡",description:"練習日、遠征試合、大会スケジュールが登録されているGoogleカレンダーです。",icon:"📅",display_order:2},{id:"link-nagareyama-league",title:"流山市少年野球連盟 公式サイト",url:"http://nagareyama-baseball.jp/",category:"公式・連盟",description:"市内大会の組み合わせトーナメント表、試合日程、グラウンド規程などが確認できます。",icon:"⚾",display_order:3},{id:"link-chiba-league",title:"千葉県少年野球連盟",url:"http://chiba-baseball.jp/",category:"公式・連盟",description:"千葉県大会の要項、大会結果、競技規則の最新情報が掲載されています。",icon:"🏆",display_order:4},{id:"link-ground-map",title:"主な活動グラウンド案内 (Google Maps)",url:"https://maps.google.com/",category:"グラウンド・施設",description:"ホームグラウンド（東小学校）および近隣の流山市内グラウンドへのアクセス地図一覧です。",icon:"📍",display_order:5},{id:"link-weather-forecast",title:"流山市のピンポイント天気予報 (tenki.jp)",url:"https://tenki.jp/forecast/3/15/4510/12220/",category:"便利ツール",description:"当日の雨雲レーダー、1時間ごとの降水確率、風速、WBGT（熱中症指数）を確認できます。",icon:"☀️",display_order:6},{id:"link-baseball-rules",title:"公認野球規則 & 少年野球特別規則",url:"https://japan-baseball.jp/",category:"便利ツール",description:"全日本軟式野球連盟（JSBB）による少年野球特別規程および公認野球規則の解説です。",icon:"📖",display_order:7}],fr=["すべて","配車マニュアル","チーム運営・規約","野球ルール・スコア","その他"],br=["すべて","公式・連盟","グラウンド・施設","スケジュール・連絡","便利ツール","その他"];async function Wn({supabaseClient:e,currentUser:t,currentUserRole:n}){Te=e,ze=t,Ha=n,kt=n==="admin"||(t==null?void 0:t.can_edit_info)===!0||n==="leader"&&(t==null?void 0:t.can_edit_info)!==!1,Gi(),await Wi(),await Ei({supabaseClient:Te,currentUser:ze,currentUserRole:Ha,canManageInfo:kt}),ld(),xr(),vr()}function Gi(){document.querySelectorAll(".info-admin-only").forEach(n=>{kt?n.classList.remove("hidden"):n.classList.add("hidden")});const t=document.getElementById("info-user-role-badge");t&&(Ha==="admin"?(t.className="px-2.5 py-0.5 text-xs font-bold rounded-full bg-purple-100 text-purple-700 border border-purple-200",t.textContent="管理者 (編集可)"):kt?(t.className="px-2.5 py-0.5 text-xs font-bold rounded-full bg-teal-100 text-teal-700 border border-teal-200",t.textContent="編集可"):(t.className="px-2.5 py-0.5 text-xs font-bold rounded-full bg-gray-100 text-gray-700 border border-gray-200",t.textContent="閲覧専用"))}async function Wi(){let e=!1,t=!1,n=!1;if(Te){try{const{data:a,error:s}=await Te.from("info_documents").select("*").order("updated_at",{ascending:!1});!s&&a&&a.length>0&&(Ce=a,e=!0)}catch(a){console.warn("Supabase info_documents select failed:",a)}try{const{data:a,error:s}=await Te.from("info_links").select("*").order("display_order",{ascending:!0});!s&&a&&a.length>0&&(Be=a,t=!0)}catch(a){console.warn("Supabase info_links select failed:",a)}try{const{data:a,error:s}=await Te.from("info_pages").select("*").eq("id","top_announcement").single();!s&&a&&a.content&&(Xt=a.content,n=!0)}catch(a){console.warn("Supabase info_pages select failed:",a)}}if(!e)try{const a=localStorage.getItem(ta);a?Ce=JSON.parse(a):(Ce=[...Ms],localStorage.setItem(ta,JSON.stringify(Ce)))}catch{Ce=[...Ms]}if(!t)try{const a=localStorage.getItem(na);a?Be=JSON.parse(a):(Be=[...Rs],localStorage.setItem(na,JSON.stringify(Be)))}catch{Be=[...Rs]}if(!n){const a=localStorage.getItem(gr);Xt=a!==null?a:Vi}}async function yr(e,t=!1){const n=Ce.findIndex(a=>a.id===e.id);n>=0?Ce[n]=e:Ce.unshift(e);try{localStorage.setItem(ta,JSON.stringify(Ce))}catch(a){console.error(a)}if(Te)try{t?await Te.from("info_documents").insert([e]):await Te.from("info_documents").update(e).eq("id",e.id)}catch(a){console.warn("Supabase document save failed:",a)}}async function zi(e){Ce=Ce.filter(t=>t.id!==e);try{localStorage.setItem(ta,JSON.stringify(Ce))}catch(t){console.error(t)}if(Te)try{await Te.from("info_documents").delete().eq("id",e)}catch(t){console.warn("Supabase document delete failed:",t)}}async function hr(e,t=!1){const n=Be.findIndex(a=>a.id===e.id);n>=0?Be[n]=e:Be.push(e);try{localStorage.setItem(na,JSON.stringify(Be))}catch(a){console.error(a)}if(Te)try{t?await Te.from("info_links").insert([e]):await Te.from("info_links").update(e).eq("id",e.id)}catch(a){console.warn("Supabase link save failed:",a)}}async function Ji(e){Be=Be.filter(t=>t.id!==e);try{localStorage.setItem(na,JSON.stringify(Be))}catch(t){console.error(t)}if(Te)try{await Te.from("info_links").delete().eq("id",e)}catch(t){console.warn("Supabase link delete failed:",t)}}async function Yi(e){Xt=e;try{localStorage.setItem(gr,e)}catch(t){console.error(t)}if(Te)try{await Te.from("info_pages").upsert([{id:"top_announcement",content:e,updated_at:new Date().toISOString(),updated_by:(ze==null?void 0:ze.name)||"管理者"}])}catch(t){console.warn("Supabase top_announcement save failed:",t)}xr()}function xr(){const e=document.getElementById("info-top-announcement-rendered"),t=document.getElementById("info-top-announcement-wrap");!e||!t||(Xt&&Xt.trim()?(t.classList.remove("hidden"),e.innerHTML=ma(Xt)):t.classList.add("hidden"))}function vr(){wr(),rt==="docs"?_n():rt==="surveys"?sn():In()}function wr(){const e=document.getElementById("info-category-filters");if(!e)return;if(rt==="surveys"){e.classList.add("hidden");return}e.classList.remove("hidden");const t=rt==="docs"?fr:br,n=rt==="docs"?Xn:ea;e.innerHTML=t.map(a=>`<button class="info-cat-btn px-3.5 py-1.5 text-xs rounded-full transition-all cursor-pointer ${n===a?"bg-teal-600 text-white shadow-sm font-bold":"bg-white text-gray-600 hover:bg-gray-100 border border-gray-200"}" data-category="${a}">${a}</button>`).join(""),e.querySelectorAll(".info-cat-btn").forEach(a=>{a.addEventListener("click",()=>{const s=a.getAttribute("data-category");rt==="docs"?(Xn=s,_n()):(ea=s,In()),wr()})})}function _n(){const e=document.getElementById("info-docs-container"),t=document.getElementById("info-docs-empty");if(!e)return;let n=Ce;if(Xn!=="すべて"&&(n=n.filter(a=>a.category===Xn)),Bn.trim()){const a=Bn.toLowerCase().trim();n=n.filter(s=>s.title&&s.title.toLowerCase().includes(a)||s.summary&&s.summary.toLowerCase().includes(a)||s.content&&s.content.toLowerCase().includes(a)||s.category&&s.category.toLowerCase().includes(a))}if(n.length===0){e.innerHTML="",t&&t.classList.remove("hidden");return}t&&t.classList.add("hidden"),e.innerHTML=n.map(a=>{const s=ds(a.category),r=a.files?a.files.length:0;return`
        ...
        `}).join(""),e.querySelectorAll(".btn-view-doc").forEach(a=>{a.addEventListener("click",()=>{const s=a.getAttribute("data-id"),r=Ce.find(o=>o.id===s);r&&Zi(r)})}),kt&&(e.querySelectorAll(".btn-duplicate-doc").forEach(a=>{a.addEventListener("click",async()=>{const s=a.getAttribute("data-id");await Ki(s)})}),e.querySelectorAll(".btn-edit-doc").forEach(a=>{a.addEventListener("click",()=>{const s=a.getAttribute("data-id"),r=Ce.find(o=>o.id===s);r&&Er(r)})}),e.querySelectorAll(".btn-delete-doc").forEach(a=>{a.addEventListener("click",async()=>{const s=a.getAttribute("data-id"),r=Ce.find(o=>o.id===s);r&&confirm(`ドキュメント「${r.title}」を削除しますか？`)&&(await zi(s),_n())})}))}async function Ki(e){const t=Ce.find(a=>a.id===e);if(!t)return;const n=JSON.parse(JSON.stringify(t));n.id=`doc_${Date.now()}`,n.title=`(複製) ${t.title}`,n.created_at=new Date().toISOString(),n.updated_at=new Date().toISOString(),n.author_name=(ze==null?void 0:ze.name)||"管理者",await yr(n,!0),_n(),alert(`ドキュメント「${n.title}」を作成（複製）しました！`)}function In(){const e=document.getElementById("info-links-container"),t=document.getElementById("info-links-empty");if(!e)return;let n=Be;if(ea!=="すべて"&&(n=n.filter(a=>a.category===ea)),Bn.trim()){const a=Bn.toLowerCase().trim();n=n.filter(s=>s.title&&s.title.toLowerCase().includes(a)||s.description&&s.description.toLowerCase().includes(a)||s.url&&s.url.toLowerCase().includes(a)||s.category&&s.category.toLowerCase().includes(a))}if(n.length===0){e.innerHTML="",t&&t.classList.remove("hidden");return}t&&t.classList.add("hidden"),e.innerHTML=n.map(a=>{const s=ds(a.category),r=a.icon||"🔗",o=a.url?a.url.replace(/^https?:\/\//,"").replace(/\/$/,""):"";return`
            <div class="bg-white rounded-2xl border border-gray-200/80 shadow-sm hover:shadow-md transition-all p-5 flex flex-col justify-between group">
                <div>
                    <div class="flex items-center justify-between gap-2 mb-3">
...
`),m=new Blob([new Uint8Array([239,187,191]),c],{type:"text/csv;charset=utf-8;"}),u=document.createElement("a");u.href=URL.createObjectURL(m),u.download=`members_${new Date().toISOString().split("T")[0]}.csv`,u.click()}catch(e){console.error(e),alert("エクスポートに失敗しました: "+e.message)}finally{q()}}async function ol(e){const t=e.target.files[0];if(!t)return;H("CSVファイルを解析中...");const n=new FileReader;n.onload=async a=>{try{const s=a.target.result,r=Ar(s);if(r.length<2){alert("有効なデータがありません。"),q();return}const o=r[0].map(B=>B.trim()),i=o.indexOf("メールアドレス"),d=o.indexOf("氏名"),l=o.indexOf("役割"),c=o.indexOf("所属グループ名"),m=o.indexOf("ユーザー属性名"),u=o.findIndex(B=>B.includes("配車")),f=o.findIndex(B=>B.includes("成績")),b=o.findIndex(B=>B.includes("出欠")),y=o.findIndex(B=>B.includes("シミュレータ")),g=o.findIndex(B=>(B.includes("Info")||B.includes("インフォ"))&&!B.includes("編集")),p=o.findIndex(B=>(B.includes("Info")||B.includes("インフォ"))&&B.includes("編集")),v=o.findIndex(B=>B.includes("代行入力先")),x=o.findIndex(B=>B.includes("代行専用")),h=o.findIndex(B=>B.includes("初期パスワード")),k=o.findIndex(B=>B.includes("削除"));if(i===-1||d===-1){alert("「メールアドレス」および「氏名」列は必須です。"),q();return}const{data:L}=await S.from("app_users").select("*"),{data:w}=await S.from("user_groups").select("*"),{data:I}=await S.from("groups").select("*"),{data:E}=await S.from("user_attributes").select("*");let $=window.adminDelegations;if(!$){const{data:B}=await S.from("master_data").select("*").eq("key","ATTENDANCE_DELEGATIONS").single();$=B&&B.data?B.data:{},window.adminDelegations=$}const _=new Map(I.map(B=>[B.name,B.id])),C=new Map(E.map(B=>[B.name,B.id])),O=new Map(L.map(B=>[B.email,B])),P=new Map(w.map(B=>[B.user_email,B.group_id])),M=[],F=[],D=[];for(let B=1;B<r.length;B++){const A=r[B];if(A.length<2)continue;const T=(A[d]||"").trim();if(!T)continue;let N=(A[i]||"").trim();const G=x!==-1?A[x]==="1"||A[x]==="true"||A[x]==="代行専用":!1,U=N.endsWith("@local.dummy"),W=G||U;if(N)W&&!U&&(N.includes("@")?N=N.split("@")[0]+"@local.dummy":N=N+"@local.dummy");else if(W)N=`dummy_${Date.now()}_${B}@local.dummy`;else continue;N=pa(N);const R=(A[l]||"").trim();let z="user";R==="管理者"||R==="admin"?z="admin":(R==="リーダー"||R==="leader")&&(z="leader");const J=c!==-1?(A[c]||"").trim():"",be=J&&_.get(J)||null,ne=m!==-1?(A[m]||"").trim():"",de=ne&&C.get(ne)||null,_e=u!==-1?!(A[u]==="0"||A[u]==="false"):!0,Je=f!==-1?!(A[f]==="0"||A[f]==="false"):!0,se=b!==-1?!(A[b]==="0"||A[b]==="false"):!0,Ke=y!==-1?!(A[y]==="0"||A[y]==="false"):!0,Pe=g!==-1?!(A[g]==="0"||A[g]==="false"):!0,Vt=p!==-1?A[p]==="1"||A[p]==="true"||A[p]==="可":z==="admin"||z==="leader"&&!0,St=k!==-1?A[k]==="1"||A[k]==="削除":!1;let lt=null;v!==-1&&(lt=(A[v]||"").split(",").map(Oe=>Oe.trim()).filter(Boolean));let ct="",nt=null;if(!W){const $e=h!==-1?(A[h]||"").trim():"";if(O.has(N))$e.length>=6&&(nt=$e);else if($e.length>=6)ct=$e;else{const Oe="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";let ut="pw-";for(let Fn=0;Fn<6;Fn++)ut+=Oe.charAt(Math.floor(Math.random()*Oe.length));ct=ut}}const V={email:N,name:T,role:z,group_id:be,group_name:J,attribute_id:de,attribute_name:ne,can_use_dispatch:_e,can_use_dashboard:Je,can_use_attendance:se,can_use_simulator:Ke,can_use_info:Pe,can_edit_info:Vt,delegations:lt,is_dummy:W,initial_password:nt!==null?nt:ct},re=O.get(N);if(re)if(St)D.push(V);else{const $e=P.get(N)!==be,Oe=re.role!==z,ut=re.name!==T,Fn=re.attribute_id!==de,Cr=re.can_use_dispatch!==!1!==_e,Dr=re.can_use_dashboard!==!1!==Je,Nr=re.can_use_attendance!==!1!==se,Pr=re.can_use_simulator!==!1!==Ke,Or=re.can_use_info!==!1!==Pe,Mr=re.can_edit_info===!0!==Vt;let hs=!1;if(lt!==null){const Rr=$[N]||[],qr=[...lt].sort().join(","),Fr=[...Rr].sort().join(",");qr!==Fr&&(hs=!0)}let xs=!1;nt!==null&&(xs=!0),($e||Oe||ut||Fn||Cr||Dr||Nr||Pr||Or||Mr||hs||xs)&&F.push(V)}else St||(V.delegations===null&&(V.delegations=[]),M.push(V))}q(),Tr("users",M,F,D,"メンバーCSVインポート確認")}catch(s){console.error(s),alert("CSVの解析に失敗しました: "+s.message),q()}},n.readAsText(t)}async function il(){_t.type==="users"?await dl():_t.type==="events"&&typeof window.executeEventsImport=="function"&&await window.executeEventsImport()}async function dl(){H("インポートデータを保存中...");try{const{add:e,update:t,delete:n}=_t;let a=window.adminDelegations;if(!a){const{data:d}=await S.from("master_data").select("*").eq("key","ATTENDANCE_DELEGATIONS").single();a=d&&d.data?d.data:{},window.adminDelegations=a}if(n.length>0){const d=n.map(c=>c.email),{error:l}=await S.from("app_users").delete().in("email",d);if(l)throw l;d.forEach(c=>{delete window.adminDelegations[c]}),Object.keys(window.adminDelegations).forEach(c=>{window.adminDelegations[c]=(window.adminDelegations[c]||[]).filter(m=>!d.includes(m))}),await ae("IMPORT_USERS_DELETE",`${n.length}件のユーザーをインポートで削除しました`)}if(e.length>0){const d=[];for(let u of e)if(!u.is_dummy)try{const f=window.supabase.createClient(us,ms,{auth:{persistSession:!1,autoRefreshToken:!1}}),{error:b}=await f.auth.signUp({email:u.email,password:u.initial_password});if(b)throw new Error(`Auth作成失敗: ${b.message}`)}catch(f){console.error(`Sign up error for ${u.email}:`,f),d.push(`${u.email}: ${f.message}`)}if(d.length>0&&!confirm(`一部のアカウント払い出し（Auth）に失敗しました。データベース登録を続行しますか？

エラー内容:
${d.join(`
`)}`))throw new Error("インポート処理を中断しました。");const l=e.map(u=>({email:u.email,name:u.name,role:u.role,attribute_id:u.attribute_id,can_use_dispatch:u.can_use_dispatch,can_use_dashboard:u.can_use_dashboard,can_use_attendance:u.can_use_attendance,can_use_simulator:u.can_use_simulator,can_use_info:u.can_use_info,can_edit_info:u.can_edit_info}));let{error:c}=await S.from("app_users").insert(l);if(c&&c.message&&c.message.includes("can_edit_info")){const u=l.map(b=>{const y={...b};return delete y.can_edit_info,y});c=(await S.from("app_users").insert(u)).error}if(c)throw c;const m=e.filter(u=>u.group_id).map(u=>({user_email:u.email,group_id:u.group_id}));if(m.length>0){const{error:u}=await S.from("user_groups").insert(m);if(u)throw u}e.forEach(u=>{u.delegations&&(window.adminDelegations[u.email]=u.delegations)}),await ae("IMPORT_USERS_ADD",`${e.length}件のユーザーをインポートで追加しました`)}if(t.length>0){const d=[];for(let l of t){if(l.initial_password)try{const{error:f}=await S.rpc("admin_update_user_password",{user_email:l.email,new_password:l.initial_password});if(f)throw f;await ae("ADMIN_CHANGE_PASSWORD",`ユーザー「${l.email}」のパスワードをインポートで変更しました`)}catch(f){console.error(`Failed to update password for ${l.email}:`,f),d.push(`${l.email}: ${f.message}`)}const c={name:l.name,role:l.role,attribute_id:l.attribute_id,can_use_dispatch:l.can_use_dispatch,can_use_dashboard:l.can_use_dashboard,can_use_attendance:l.can_use_attendance,can_use_simulator:l.can_use_simulator,can_use_info:l.can_use_info,can_edit_info:l.can_edit_info};let{error:m}=await S.from("app_users").update(c).eq("email",l.email);if(m&&m.message&&m.message.includes("can_edit_info")){const f={...c};delete f.can_edit_info,m=(await S.from("app_users").update(f).eq("email",l.email)).error}if(m)throw m;const{data:u}=await S.from("user_groups").select("*").eq("user_email",l.email);if(l.group_id)if(u&&u.length>0){const{error:f}=await S.from("user_groups").update({group_id:l.group_id}).eq("user_email",l.email);if(f)throw f}else{const{error:f}=await S.from("user_groups").insert([{user_email:l.email,group_id:l.group_id}]);if(f)throw f}else if(u&&u.length>0){const{error:f}=await S.from("user_groups").delete().eq("user_email",l.email);if(f)throw f}l.delegations!==null&&(window.adminDelegations[l.email]=l.delegations)}d.length>0&&alert(`一部のパスワード更新に失敗しました。詳細は開発者コンソールを確認してください。
